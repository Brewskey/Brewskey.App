						...
						post: function($scope, $element, $attrs, controller) {
							promise.then(function() {
								...
								function parseViewValue(value) {
									...
									// used on model
									var viewValueWithoutDivisors = viewValue.withoutDivisors(
										true
									);
									...
									try {
										...
										var diffMaskAndViewValueIsGreaterThanZero =
											maskWithoutOptionals.length -
												viewValueWithDivisors.length >
											0;
								...
								var watcher = $scope.$watch($attrs.ngModel, function(
									newValue,
									oldValue
								) {
			...
			function create() {
				...
				function insertDivisors(array, combination) {
					...
					var divs = divisors.filter(function(it) {
						var optionalDivisorsKeys = Object.keys(
							optionalDivisors
						).map(function(it) {
							return parseInt(it);
						});
