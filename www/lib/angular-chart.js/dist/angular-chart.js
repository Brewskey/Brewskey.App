(function(factory) {
	...
	var usingExcanvas =
		typeof window.G_vmlCanvasManager === "object" &&
		...
		return function chart(type) {
           ...
           * having an alias point from the new names to the old names. */
					...
					function createChart(type) {
						if (
							isResponsive(type, scope) &&
							elem[0].clientHeight === 0 &&
						...
						scope.getColour =
							typeof scope.getColour === "function"
								? scope.getColour
								: getRandomColour;
						...
						// Bind events
						cvs.onclick = scope.click
							? getEventHandler(scope, chart, "click", false)
							: angular.noop;
						cvs.onmousemove = scope.hover
							? getEventHandler(scope, chart, "hover", true)
							: angular.noop;
		...
		function getEventHandler(scope, chart, action, triggerOnlyOnChange) {
			...
			return function(evt) {
				var atEvent =
					...
					chart.getSegmentsAtEvent;
				if (atEvent) {
					var activePoints = atEvent.call(chart, evt);
					if (
						triggerOnlyOnChange === false ||
						angular.equals(lastState, activePoints) === false
					) {
