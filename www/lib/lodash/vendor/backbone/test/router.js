(function(QUnit) {
	...
	ExternalObject.routingFunction = _.bind(
		ExternalObject.routingFunction,
		ExternalObject
	);
	...
	QUnit.test("routes via navigate for backwards-compatibility", function(
		assert
	) {
		assert.expect(2);
	...
	QUnit.test("routes via navigate with {replace: true}", function(assert) {
		...
		location.replace = function(href) {
			assert.strictEqual(
				href,
				new Location("http://example.com#end_here").href
			);
		};
	...
	QUnit.test("fires event when router doesn't have callback on it", function(
		assert
	) {
		assert.expect(1);
	...
	QUnit.test("No events are triggered if #execute returns false.", function(
		assert
	) {
		assert.expect(1);
	...
	QUnit.test("#967 - Route callback gets passed encoded values.", function(
		assert
	) {
		assert.expect(3);
	...
	QUnit.test("#1185 - Use pathname when hashChange is not wanted.", function(
		assert
	) {
		assert.expect(1);
		Backbone.history.stop();
	...
	QUnit.test("#1206 - Strip leading slash before location.assign.", function(
		assert
	) {
		assert.expect(1);
		Backbone.history.stop();
	...
	QUnit.test("#1366 - History does not prepend root to fragment.", function(
		assert
	) {
		assert.expect(2);
		Backbone.history.stop();
	...
	QUnit.test('#2062 - Trigger "route" event on router instance.', function(
		assert
	) {
		assert.expect(2);
	...
	QUnit.test("#2255 - Extend routes by making routes a function.", function(
		assert
	) {
		assert.expect(1);
	...
	QUnit.test(
		"#2538 - hashChange to pushState only if both requested.",
		function(assert) {
			assert.expect(0);
			Backbone.history.stop();
	...
	QUnit.test("pushState to hashChange with only search params.", function(
		assert
	) {
		assert.expect(1);
		Backbone.history.stop();
	...
	QUnit.test("#3123 - History#navigate decodes before comparison.", function(
		assert
	) {
		assert.expect(1);
		Backbone.history.stop();
		...
		assert.strictEqual(
			...
			"shop/search?keyword=short dress"
		);
	});
	...
	QUnit.test(
		"#3358 - pushState to hashChange transition with search params",
		function(assert) {
			assert.expect(1);
			Backbone.history.stop();
