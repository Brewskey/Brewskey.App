(function(QUnit) {
	...
	QUnit.test("add; at should have preference over comparator", function(
		assert
	) {
		assert.expect(1);
	...
	QUnit.test("add model to collection with sort()-style comparator", function(
		assert
	) {
		assert.expect(3);
		var collection = new Backbone.Collection();
	...
	QUnit.test("fetch with an error response triggers an error event", function(
		assert
	) {
		assert.expect(1);
		var collection = new Backbone.Collection();
	...
	QUnit.test(
		"#3283 - fetch with an error response calls error with context",
		function(assert) {
			assert.expect(1);
			var collection = new Backbone.Collection();
			var obj = {};
	...
	QUnit.test("create will pass extra options to success callback", function(
		assert
	) {
		assert.expect(1);
		var Model = Backbone.Model.extend({
		...
		var success = function(model, response, options) {
			assert.ok(
				options.specialSync,
				"Options were passed correctly to callback"
			);
		};
	...
	QUnit.test("preinitialize occurs before the collection is set up", function(
		assert
	) {
		assert.expect(2);
		var Collection = Backbone.Collection.extend({
	...
	QUnit.test(
		"Underscore methods with object-style and property-style iteratee",
		function(assert) {
			assert.expect(26);
	...
	QUnit.test("reset passes caller options", function(assert) {
		...
		collection.reset(
			[{ astring: "green", anumber: 1 }, { astring: "blue", anumber: 2 }],
	...
	QUnit.test("#714: access `model.collection` in a brand new model.", function(
		assert
	) {
		assert.expect(2);
		var collection = new Backbone.Collection();
	...
	QUnit.test("#574, remove its own reference to the .models array.", function(
		assert
	) {
		assert.expect(2);
		var collection = new Backbone.Collection([
	...
	QUnit.test("Invalid models are discarded with validate:true.", function(
		assert
	) {
		assert.expect(5);
		var collection = new Backbone.Collection();
	...
	QUnit.test("#1112 - passing options.model sets collection.model", function(
		assert
	) {
		assert.expect(2);
		var Model = Backbone.Model.extend({});
	...
	QUnit.test("#1355 - `options` is passed to success callbacks", function(
		assert
	) {
		assert.expect(2);
	...
	QUnit.test("#1412 - Trigger 'request' and 'sync' events.", function(assert) {
		...
		collection.on("request", function(obj, xhr, options) {
			assert.ok(
				obj === collection,
				"collection has correct 'request' event after fetching"
			);
		});
		collection.on("sync", function(obj, response, options) {
			assert.ok(
				obj === collection,
				"collection has correct 'sync' event after fetching"
			);
		});
		collection.fetch();
		collection.off();
	...
	QUnit.test("#3283 - fetch, create calls success with context", function(
		assert
	) {
		assert.expect(2);
		var collection = new Backbone.Collection();
	...
	QUnit.test("#1655 - groupBy can be used with a string argument.", function(
		assert
	) {
		assert.expect(3);
	...
	QUnit.test("#1655 - sortBy can be used with a string argument.", function(
		assert
	) {
		assert.expect(1);
	...
	QUnit.test("#1638 - `sort` during `add` triggers correctly.", function(
		assert
	) {
		var collection = new Backbone.Collection();
		collection.comparator = function(model) {
	...
	QUnit.test(
		"#1407 parse option on constructor parses collection and models",
		function(assert) {
			assert.expect(2);
			var model = {
				namespace: [{ id: 1 }, { id: 2 }]
	...
	QUnit.test(
		"#1407 parse option on reset parses collection and models",
		function(assert) {
			assert.expect(2);
			var model = {
				namespace: [{ id: 1 }, { id: 2 }]
	...
	QUnit.test("Reset includes previous models in triggered event.", function(
		assert
	) {
		assert.expect(1);
		var model = new Backbone.Model();
	...
	QUnit.test(
		"`set` matches input order in the absence of a comparator",
		function(assert) {
	...
	QUnit.test("#2428 - push duplicate models, return the correct one", function(
		assert
	) {
		assert.expect(1);
		var collection = new Backbone.Collection();
	...
	QUnit.test("#1915 - `parse` data in the right order in `set`", function(
		assert
	) {
	...
	QUnit.test("fetch will pass extra options to success callback", function(
		assert
	) {
		assert.expect(1);
	...
	QUnit.test(
		"`add` only `sort`s when necessary with comparator function",
		function(assert) {
			assert.expect(3);
			var collection = new (Backbone.Collection.extend({
				comparator: function(m1, m2) {
	...
	QUnit.test("Pass falsey for `models` for empty Col with `options`", function(
		assert
	) {
		assert.expect(9);
		var opts = { a: 1, b: 2 };
	...
	QUnit.test("#2612 - nested `parse` works with `Collection#set`", function(
		assert
	) {
	...
	QUnit.test("Do not allow duplicate models to be `add`ed or `set`", function(
		assert
	) {
		var collection = new Backbone.Collection();
	...
	QUnit.test("#3020: #set with {add: false} should not throw.", function(
		assert
	) {
		assert.expect(2);
		var collection = new Backbone.Collection();
	...
	QUnit.test('Polymorphic models work with "simple" constructors', function(
		assert
	) {
	...
	QUnit.test('Polymorphic models work with "advanced" constructors', function(
		assert
	) {
	...
	QUnit.test("#3039 #3951: adding at index fires with correct at", function(
		assert
	) {
		assert.expect(4);
	...
	QUnit.test("#3039: index is not sent when at is not specified", function(
		assert
	) {
		assert.expect(2);
	...
	QUnit.test("#3199 - Order not changing should not trigger a sort", function(
		assert
	) {
		assert.expect(0);
		var one = new Backbone.Model({ id: 1 });
	...
	QUnit.test("adding multiple models triggers `update` event once", function(
		assert
	) {
		assert.expect(1);
		var collection = new Backbone.Collection();
	...
	QUnit.test("remove does not trigger `update` when nothing removed", function(
		assert
	) {
		assert.expect(0);
		var collection = new Backbone.Collection([{ id: 1 }, { id: 2 }]);
	...
	QUnit.test("#3871 - falsy parse result creates empty collection", function(
		assert
	) {
		var collection = new (Backbone.Collection.extend({
			parse: function(data, options) {}
	...
	QUnit.test(
		...
		function(assert) {
			...
			collection.on("update", function(context, options) {
				...
				assert.ok(
					_.indexOf(changed.removed, model) > -1 &&
						_.indexOf(changed.removed, model2) > -1
				);
			});
	...
	QUnit.test("#3711 - set's `update` event returns one added model", function(
		assert
	) {
		var model = new Backbone.Model({ id: 1, title: "First Post" });
		var collection = new Backbone.Collection();
	...
	QUnit.test("#3711 - set's `update` event returns one merged model", function(
		assert
	) {
		var model = new Backbone.Model({ id: 1, title: "First Post" });
		var model2 = new Backbone.Model({ id: 2, title: "Second Post" });
		...
		collection.on("update", function(context, options) {
			...
			assert.strictEqual(
				mergedModels[0].get("title"),
				model2Update.get("title")
			);
		});
	...
	QUnit.test(
		...
		function(assert) {
			...
			collection.on("update", function(context, options) {
				...
				assert.strictEqual(
					mergedModels[1].get("title"),
					modelUpdate.get("title")
				);
			});
