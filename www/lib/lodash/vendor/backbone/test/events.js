(function(QUnit) {
	...
	QUnit.test("stopListening cleans up references from listenToOnce", function(
		assert
	) {
		assert.expect(12);
		var a = _.extend({}, Backbone.Events);
	...
	QUnit.test("listenTo and stopListening cleaning up references", function(
		assert
	) {
		assert.expect(2);
		var a = _.extend({}, Backbone.Events);
	...
	QUnit.test("listenToOnce with event maps cleans up references", function(
		assert
	) {
		assert.expect(2);
		var a = _.extend({}, Backbone.Events);
	...
	QUnit.test("listenToOnce with event maps binds the correct `this`", function(
		assert
	) {
		assert.expect(1);
		var a = _.extend({}, Backbone.Events);
	...
	QUnit.test("listenTo with empty callback doesn't throw an error", function(
		assert
	) {
		assert.expect(1);
		var e = _.extend({}, Backbone.Events);
	...
	QUnit.test("#3448 - listenToOnce with space-separated events", function(
		assert
	) {
		assert.expect(2);
