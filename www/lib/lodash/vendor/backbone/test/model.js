(function(QUnit) {
	...
	QUnit.test("preinitialize occurs before the model is set up", function(
		assert
	) {
		assert.expect(6);
		var Model = Backbone.Model.extend({
	...
	QUnit.test("isNew", function(assert) {
		...
		assert.ok(
			!new Backbone.Model({ id: 2 }).isNew(),
			"is false for a positive integer"
		);
		assert.ok(
			!new Backbone.Model({ id: -5 }).isNew(),
			"is false for a negative integer"
		);
	});
	...
	QUnit.test("set and unset", function(assert) {
		...
		assert.equal(
			a.id,
			undefined,
			"Unsetting the id should remove the id property."
		);
	});
	...
	QUnit.test("#3283 - save, fetch, destroy calls error with context", function(
		assert
	) {
		assert.expect(3);
		var model = new Backbone.Model();
	...
	QUnit.test("save will pass extra options to success callback", function(
		assert
	) {
		assert.expect(1);
	...
	QUnit.test("destroy will pass extra options to success callback", function(
		assert
	) {
		assert.expect(1);
		var SpecialSyncModel = Backbone.Model.extend({
	...
	QUnit.test("Inherit class properties", function(assert) {
		...
		assert.notEqual(
			Child.prototype.instancePropDiff,
			Parent.prototype.instancePropDiff
		);
		assert.notEqual(Child.prototype.instancePropDiff, undefined);
	});

	QUnit.test("Nested change events don't clobber previous attributes", function(
		assert
	) {
		assert.expect(4);
		new Backbone.Model()
	...
	QUnit.test("unset does not fire a change for undefined attributes", function(
		assert
	) {
		assert.expect(0);
	...
	QUnit.test("save without `wait` doesn't set invalid attributes", function(
		assert
	) {
		var model = new Backbone.Model();
	...
	QUnit.test("#1365 - Destroy: New models execute success callback.", function(
		assert
	) {
	...
	QUnit.test("#1433 - Save: An invalid model cannot be persisted.", function(
		assert
	) {
		assert.expect(1);
		var model = new Backbone.Model();
	...
	QUnit.test(
		"silent changes in last `change` event back to original triggers change",
		function(assert) {
			assert.expect(2);
			var changes = [];
			var model = new Backbone.Model();
	...
	QUnit.test("#1943 change calculations should use _.isEqual", function(
		assert
	) {
	...
	QUnit.test("toJSON receives attrs during save(..., {wait: true})", function(
		assert
	) {
		assert.expect(1);
		var Model = Backbone.Model.extend({
	...
	QUnit.test(
		"#2034 - nested set with silent only triggers one change",
		function(assert) {
			assert.expect(1);
			var model = new Backbone.Model();
			model.on("change", function() {
