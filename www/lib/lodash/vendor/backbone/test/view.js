(function(QUnit) {
	...
	QUnit.test("preinitialize occurs before the view is set up", function(
		assert
	) {
		assert.expect(2);
		var View = Backbone.View.extend({
	...
	QUnit.test("delegate", function(assert) {
		...
		assert.equal(
			myView.delegate(),
			myView,
			"#delegate returns the view instance"
		);
	});
	...
	QUnit.test("undelegateEvents", function(assert) {
		...
		assert.equal(
			myView.undelegateEvents(),
			myView,
			"#undelegateEvents returns the view instance"
		);
	});
	...
	QUnit.test("undelegate", function(assert) {
		...
		assert.equal(
			myView.undelegate(),
			myView,
			"#undelegate returns the view instance"
		);
	});
