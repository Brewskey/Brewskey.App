(function(QUnit) {
	...
	QUnit.test("update", function(assert) {
		...
		library
			.first()
			.save({ id: "1-the-tempest", author: "William Shakespeare" });
	...
	QUnit.test("update with emulateHTTP and emulateJSON", function(assert) {
		assert.expect(7);
		library.first().save(
			{ id: "2-the-tempest", author: "Tim Shakespeare" },
			{
				emulateHTTP: true,
				emulateJSON: true
			}
	...
	QUnit.test("destroy with emulateHTTP", function(assert) {
		...
		assert.equal(this.ajaxSettings.type, "POST");
		assert.equal(
			JSON.stringify(this.ajaxSettings.data),
			'{"_method":"DELETE"}'
		);
	});
	...
	QUnit.test("#1756 - Call user provided beforeSend function.", function(
		assert
	) {
		assert.expect(4);
		Backbone.emulateHTTP = true;
	...
	QUnit.test("#2928 - Pass along `textStatus` and `errorThrown`.", function(
		assert
	) {
		assert.expect(2);
		var model = new Backbone.Model();
