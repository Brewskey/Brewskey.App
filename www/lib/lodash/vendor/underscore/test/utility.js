(function() {
	...
	QUnit.test("identity", function(assert) {
		...
		assert.equal(
			_.identity(stooge),
			stooge,
			"stooge is the same as his identity"
		);
	});
	...
	QUnit.test("constant", function(assert) {
		...
		assert.equal(
			_.constant(stooge)(),
			stooge,
			"should create a function that returns stooge"
		);
	});

	QUnit.test("noop", function(assert) {
		assert.strictEqual(
			_.noop("curly", "larry", "moe"),
			void 0,
	...
	QUnit.test("property", function(assert) {
		...
		assert.equal(
			...
			void 0,
			"should return undefined for null values"
		);
		assert.equal(
			_.property("name")(void 0),
			void 0,
	...
	QUnit.test("propertyOf", function(assert) {
		...
		assert.equal(
			moreStoogeRanks("curly"),
			2,
			"should return properties from further up the prototype chain"
		);

		var nullPropertyOf = _.propertyOf(null);
		assert.equal(
			nullPropertyOf("curly"),
			void 0,
			"should return undefined when obj is null"
		);

		var undefPropertyOf = _.propertyOf(void 0);
		assert.equal(
			undefPropertyOf("curly"),
			void 0,
			"should return undefined when obj is undefined"
		);
	});
	...
	QUnit.test("mixin", function(assert) {
		...
		assert.equal(
			_("champ").myReverse(),
			"pmahc",
			"mixed in a function to the OOP wrapper"
		);
	});
	...
	QUnit.test("_.escape & unescape", function(assert) {
		...
		_.each(escapeCharacters, function(escapeChar) {
			...
			assert.equal(
				...
				-1,
				"can escape multiple occurances of " + escapeChar
			);
			assert.equal(
				_.unescape(e),
				s,
		...
		assert.ok(
			_.every(escapeCharacters, function(escapeChar) {
	...
	QUnit.test("template", function(assert) {
		...
		var quoteInStatementAndBody = _.template(
			"<% " + "  if(foo == 'bar'){ " + "%>Statement quotes and 'quotes'.<% } %>"
		);
		assert.equal(
		...
		var withNewlinesAndTabs = _.template(
			"This\n\t\tis: <%= x %>.\n\tok.\nend."
		);
		assert.equal(
		...
		assert.equal(
			template({ data: 12345 }).replace(/\s/g, ""),
			"<li>24690</li>"
		);
		...
		var custom = _.template(
			"<ul>{{ for (var key in people) { }}<li>{{= people[key] }}</li>{{ } }}</ul>"
		);
		...
		assert.equal(
			mustache({ planet: "World" }),
			"Hello World!",
			"can mimic mustache.js"
		);

		var templateWithNull = _.template("a null undefined {{planet}}");
		assert.equal(
			templateWithNull({ planet: "world" }),
			"a null undefined world",
	...
	QUnit.test(
		...
		function(assert) {
			...
			assert.strictEqual(
				_.result("".match("missing"), 1, "default"),
				"default"
			);
		}
	);
	...
	QUnit.test("#779 - delimeters are applied to unescaped text.", function(
		assert
	) {
		assert.expect(1);
