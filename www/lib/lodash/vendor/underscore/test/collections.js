(function() {
	...
	QUnit.test("Iterating objects with sketchy length properties", function(
		assert
	) {
		var functions = [
			"each",
		...
		_.each(tricks, function(trick) {
			...
			assert.strictEqual(
				_.size(trick),
				1,
				"size on obj with length: " + length
			);
			assert.deepEqual(
				_.toArray(trick),
				[length],
				"toArray on obj with length: " + length
			);
			assert.deepEqual(
				_.shuffle(trick),
				[length],
				"shuffle on obj with length: " + length
			);
			assert.deepEqual(
				_.sample(trick),
				length,
				"sample on obj with length: " + length
			);
	...
	QUnit.test(
		...
		function(assert) {
			...
			var array = ["findIndex", "findLastIndex"];
			var object = ["mapObject", "findKey", "pick", "omit"];
	...
	QUnit.test("reduce", function(assert) {
		...
		assert.strictEqual(
			_.reduce(null, _.noop, 138),
			138,
			"handles a null (with initial value) properly"
		);
		assert.equal(
			_.reduce([], _.noop, void 0),
			void 0,
			"undefined can be passed as a special case"
		);
		assert.equal(
			_.reduce([_], _.noop),
			_,
			"collection of length one with no initial value returns the first item"
		);
		assert.equal(
			_.reduce([], _.noop),
			void 0,
			"returns undefined when collection is empty and no initial value"
		);
	});
	...
	QUnit.test("reduceRight", function(assert) {
		...
		assert.strictEqual(
			_.reduceRight(null, _.noop, 138),
			138,
			"handles a null (with initial value) properly"
		);
		assert.equal(
			_.reduceRight([_], _.noop),
			_,
			"collection of length one with no initial value returns the first item"
		);

		assert.equal(
			_.reduceRight([], _.noop, void 0),
			void 0,
			"undefined can be passed as a special case"
		);
		assert.equal(
			_.reduceRight([], _.noop),
			void 0,
			"returns undefined when collection is empty and no initial value"
		);
		...
		var expected =
			lastKey === "a" ? [init, 1, "a", object] : [init, 2, "b", object];
		...
		expected =
			lastKey === "2" ? [init, "a", "2", object] : [init, "b", "1", object];
	...
	QUnit.test("filter", function(assert) {
		...
		assert.deepEqual(_.filter(list, { b: 2 }), [
			{ a: 1, b: 2 },
			{ a: 2, b: 2 }
		]);
		assert.deepEqual(
	...
	QUnit.test("reject", function(assert) {
		...
		assert.deepEqual(
			_.reject([odds, { one: 1, two: 2, three: 3 }], "two"),
			[odds],
			"predicate string map to object properties"
		);
		...
		assert.deepEqual(_.reject(list, { b: 2 }), [
			{ a: 1, b: 3 },
			{ a: 1, b: 4 }
		]);
		assert.deepEqual(
			_.reject(list, {}),
			[],
			"Returns empty list given empty object"
		);
		assert.deepEqual(
			_.reject(list, []),
			[],
	...
	QUnit.test("every", function(assert) {
		...
		assert.ok(
			_.every([0, 10, 28], function(num) {
				return num % 2 === 0;
			}),
			"even numbers"
		);
		assert.notOk(
			_.every([0, 11, 28], function(num) {
				return num % 2 === 0;
			}),
			"an odd number"
		);
		assert.strictEqual(
			_.every([1], _.identity),
			true,
			"cast to boolean - true"
		);
		assert.strictEqual(
			_.every([0], _.identity),
			false,
			"cast to boolean - false"
		);
		assert.notOk(
			_.every([void 0, void 0, void 0], _.identity),
			"works with arrays of undefined"
		);
		...
		assert.notOk(
			_.every({ a: 1, b: 2, c: 3, d: 4 }, _.isObject),
			"takes objects"
		);
		assert.ok(
			_.every(["a", "b", "c", "d"], _.hasOwnProperty, {
				a: 1,
				b: 2,
				c: 3,
				d: 4
		...
		assert.notOk(
			_.every(["a", "b", "c", "d", "f"], _.hasOwnProperty, {
				a: 1,
				b: 2,
				c: 3,
				d: 4
	...
	QUnit.test("some", function(assert) {
		...
		assert.notOk(
			_.some([1, 11, 29], function(num) {
				return num % 2 === 0;
			}),
			"all odd numbers"
		);
		assert.ok(
			_.some([1, 10, 29], function(num) {
				return num % 2 === 0;
			}),
			"an even number"
		);
		...
		assert.ok(
			_.some({ a: "1", b: "2", c: "3", d: "4", e: 6 }, _.isNumber),
			"takes objects"
		);
		assert.notOk(
			_.some({ a: 1, b: 2, c: 3, d: 4 }, _.isObject),
			"takes objects"
		);
		assert.ok(
			_.some(["a", "b", "c", "d"], _.hasOwnProperty, {
				a: 1,
				b: 2,
				c: 3,
				d: 4
		...
		assert.notOk(
			_.some(["x", "y", "z"], _.hasOwnProperty, { a: 1, b: 2, c: 3, d: 4 }),
			"context works"
		);
	});
	...
	QUnit.test("includes", function(assert) {
		...
		assert.strictEqual(
			_.includes([5, 4, 3, 2, 1], 5, true),
			true,
			"doesn't delegate to binary search"
		);

		assert.strictEqual(
			_.includes({ moe: 1, larry: 3, curly: 9 }, 3),
			true,
			"_.includes on objects checks their values"
		);
		...
		assert.ok(
			_.every([1, 2, 3], _.partial(_.includes, numbers)),
			"fromIndex is guarded"
		);
	});
	...
	QUnit.test("includes with NaN", function(assert) {
		assert.strictEqual(
			_.includes([1, 2, NaN, NaN], NaN),
			true,
			"Expected [1, 2, NaN] to contain NaN"
		);
		assert.strictEqual(
			_.includes([1, 2, Infinity], NaN),
			false,
			"Expected [1, 2, NaN] to contain NaN"
		);
	});
	...
	QUnit.test("invoke", function(assert) {
		...
		assert.deepEqual(
			_.invoke([{ a: null }, {}, { a: _.constant(1) }], "a"),
			[null, void 0, 1],
	...
	QUnit.test("findWhere", function(assert) {
		...
		assert.deepEqual(
			_.findWhere([{ y: 5, b: 6 }, expect], new TestClass()),
			expect,
			"uses class instance properties"
		);
	});
	...
	QUnit.test("max", function(assert) {
		...
		assert.equal(
			_.max([1, 2, 3, null]),
			3,
			"Finds correct max in array starting with num and containing a `null`"
		);
		assert.equal(
			_.max([null, 1, 2, 3]),
			3,
			"Finds correct max in array starting with a `null`"
		);

		assert.equal(
			_.max([1, 2, 3, ""]),
			3,
			"Finds correct max in array starting with num and containing an empty string"
		);
		assert.equal(
			_.max(["", 1, 2, 3]),
			3,
			"Finds correct max in array starting with an empty string"
		);

		assert.equal(
			_.max([1, 2, 3, false]),
			3,
			"Finds correct max in array starting with num and containing a false"
		);
		assert.equal(
			_.max([false, 1, 2, 3]),
			3,
			"Finds correct max in array starting with a false"
		);

		assert.equal(
			_.max([0, 1, 2, 3, 4]),
			4,
			"Finds correct max in array containing a zero"
		);
		assert.equal(
			_.max([-3, -2, -1, 0]),
			0,
			"Finds correct max in array containing negative numbers"
		);

		assert.deepEqual(
			_.map([[1, 2, 3], [4, 5, 6]], _.max),
			[3, 6],
			"Finds correct max in array when mapping through multiple arrays"
		);
	...
	QUnit.test("min", function(assert) {
		...
		assert.deepEqual(
			_.map([[1, 2, 3], [4, 5, 6]], _.min),
			[1, 4],
			"Finds correct min in array when mapping through multiple arrays"
		);
		...
		assert.equal(
			_.min([1, 2, 3, "test"]),
			1,
			"Finds correct min in array starting with num and containing a NaN"
		);
		assert.equal(
			_.min(["test", 1, 2, 3]),
			1,
		...
		assert.equal(
			_.min([1, 2, 3, null]),
			1,
			"Finds correct min in array starting with num and containing a `null`"
		);
		assert.equal(
			_.min([null, 1, 2, 3]),
			1,
			"Finds correct min in array starting with a `null`"
		);

		assert.equal(
			_.min([0, 1, 2, 3, 4]),
			0,
			"Finds correct min in array containing a zero"
		);
		assert.equal(
			_.min([-3, -2, -1, 0]),
			-3,
			"Finds correct min in array containing negative numbers"
		);
	...
	QUnit.test("sortBy", function(assert) {
		...
		assert.deepEqual(
			_.sortBy(list, _.identity),
			[1, 2, 3, 4, void 0, void 0],
		...
		assert.deepEqual(
			_.sortBy(stableArray, "x"),
			stableArray,
			"sortBy accepts property string"
		);
		...
		assert.deepEqual(
			actual,
			stableArray,
			"sortBy should be stable for objects"
		);
	...
	QUnit.test("groupBy", function(assert) {
		...
		assert.deepEqual(_.groupBy(matrix, 0), {
			1: [[1, 2], [1, 3]],
			2: [[2, 3]]
		});
		assert.deepEqual(_.groupBy(matrix, 1), {
			2: [[1, 2]],
			3: [[1, 3], [2, 3]]
		});
	});
	...
	QUnit.test("sample", function(assert) {
		assert.strictEqual(
			_.sample([1]),
			1,
			"behaves correctly when no second parameter is given"
		);
		assert.deepEqual(
			_.sample([1, 2, 3], -2),
			[],
		...
		assert.ok(
			_.contains(numbers, _.sample(numbers)),
			"sampling a single element returns something from the array"
		);
		assert.strictEqual(
			_.sample([]),
			void 0,
		...
		assert.ok(
			_.contains([1, 2, 3], _.sample({ a: 1, b: 2, c: 3 })),
			"sample one value from an object"
		);
		...
		assert.notDeepEqual(
			partialSampleSorted,
			_.range(10),
			"samples from the whole array, not just the beginning"
		);
	});
	...
	QUnit.test("toArray", function(assert) {
		...
		assert.deepEqual(
			_.toArray(expected.join("")),
			expected,
			"maintains astral characters"
		);
		...
		if (typeof document != "undefined") {
			...
			assert.deepEqual(
				actual,
				_.map(document.childNodes, _.identity),
				"works on NodeList"
			);
		}
	});
	...
	QUnit.test("partition", function(assert) {
		...
		assert.deepEqual(
			_.partition({ a: 1, b: 2, c: 3 }, function(x) {
				return x > 1;
			}),
			[[2, 3], [1]],
			"handles objects"
		...
		assert.deepEqual(
			_.partition(list, function(x, index) {
				return index % 2;
			}),
			[[1, 3, 5], [0, 2, 4]],
		...
		assert.deepEqual(
			_.partition(list, function(x, index, arr) {
		...
		// Default iterator
		assert.deepEqual(
			_.partition([1, false, true, ""]),
			[[1, true], [false, ""]],
			"Default iterator"
		);
		assert.deepEqual(
			_.partition([{ x: 1 }, { x: 0 }, { x: 1 }], "x"),
			[[{ x: 1 }, { x: 1 }], [{ x: 0 }]],
			"Takes a string"
		);
		...
		assert.deepEqual(
			_.partition([1, 2, 3], predicate, { x: 2 }),
			[[2], [1, 3]],
			"partition takes a context argument"
		);

		assert.deepEqual(
			_.partition([{ a: 1 }, { b: 2 }, { a: 1, b: 2 }], { a: 1 }),
			[[{ a: 1 }, { a: 1, b: 2 }], [{ b: 2 }]],
			"predicate can be object"
		);
	...
	if (typeof document != "undefined") {
		QUnit.test("Can use various collection methods on NodeLists", function(
			assert
		) {
			...
			assert.equal(
				_.max(parent.childNodes, compareNode),
				_.last(parent.childNodes)
			);
			assert.equal(
				_.min(parent.childNodes, compareNode),
				_.first(parent.childNodes)
			);
		});
	}
})();
