(function() {
	...
	var testElement =
		typeof document === "object" ? document.createElement("div") : void 0;
	...
	QUnit.test("allKeys", function(assert) {
		assert.deepEqual(
			_.allKeys({ one: 1, two: 2 }),
			["one", "two"],
			"can extract the allKeys from an object"
		);
		// the test above is not safe because it relies on for-in enumeration order
		var a = [];
		a[1] = 0;
		...
		_.each(
			[null, void 0, 1, "a", true, NaN, {}, [], new Number(5), new Date(0)],
			function(val) {
	...
	QUnit.test("pairs", function(assert) {
		...
		assert.deepEqual(
			_.pairs({ one: 1, two: 2, length: 3 }),
			[["one", 1], ["two", 2], ["length", 3]],
			'... even when one of them is "length"'
		);
	});
	...
	QUnit.test("invert", function(assert) {
		...
		assert.equal(
			_.invert(obj)["3"],
			"length",
			'can invert an object with "length"'
		);
	});
	...
	QUnit.test("extend", function(assert) {
		...
		assert.equal(
			_.extend({ a: "x" }, { a: "b" }).a,
			"b",
			"properties in source override destination"
		);
		assert.equal(
			_.extend({ x: "x" }, { a: "b" }).x,
			"x",
			"properties not in source don't get overriden"
		);
		...
		assert.deepEqual(
			_.keys(result),
			["a", "b"],
			"extend copies undefined values"
		);
		...
		_.extend(subObj, {});
		assert.notOk(
			subObj.hasOwnProperty("a"),
			"extend does not convert destination object's 'in' properties to 'own' properties"
		);
	...
	QUnit.test("extendOwn", function(assert) {
		...
		assert.equal(
			_.extendOwn({ a: "x" }, { a: "b" }).a,
			"b",
			"properties in source override destination"
		);
		assert.equal(
			_.extendOwn({ x: "x" }, { a: "b" }).x,
			"x",
			"properties not in source don't get overriden"
		);
		...
		assert.deepEqual(
			_.extendOwn({}, subObj),
			{ c: "d" },
			"copies own properties from source"
		);

		result = {};
		assert.deepEqual(
			_.extendOwn(result, null, void 0, { a: 1 }),
			{ a: 1 },
			"should not error on `null` or `undefined` sources"
		);
	...
	QUnit.test("pick", function(assert) {
		...
		_.each([null, void 0], function(val) {
			assert.deepEqual(
				_.pick(val, "hasOwnProperty"),
				{},
				"Called with null/undefined"
			);
		...
		assert.notOk(
			_.has(_.pick({}, "foo"), "foo"),
			"does not set own property if property not in object"
		);
		_.pick(data, function(value, key, obj) {
			assert.equal(
				obj,
				data,
				"passes same object as third parameter of iteratee"
			);
		});
	});
	...
	QUnit.test("omit", function(assert) {
		...
		assert.deepEqual(
			_.omit(null, "a", "b"),
			{},
			"non objects return empty object"
		);
		assert.deepEqual(
			_.omit(void 0, "toString"),
			{},
		...
		assert.deepEqual(
			_.omit(5, "toString", "b"),
			{},
			"returns empty object for primitives"
		);
	...
	QUnit.test("clone", function(assert) {
		...
		assert.equal(
			_.last(moe.lucky),
			101,
			"changes to deep attributes are shared with the original"
		);
	...
	QUnit.test("create", function(assert) {
		...
		_.each(["foo", null, void 0, 1], function(val) {
			assert.deepEqual(
				_.create(val),
				{},
				"should return empty object when a non-object is provided"
			);
		});

		assert.ok(
			_.create([]) instanceof Array,
			"should return new instance of array when array is provided"
		);
		...
		assert.strictEqual(
			...
			func,
			"properties should be added to object"
		);
		...
		assert.notOk(
			created.hasOwnProperty("foo"),
			"should only add own properties"
		);
	});
	...
	QUnit.test("isEqual", function(assert) {
		...
		assert.ok(
			_.isEqual(new Number(NaN), NaN),
			"Object(`NaN`) is equal to `NaN`"
		);
		assert.notOk(
			_.isEqual(61, NaN),
		...
		assert.notOk(
			_.isEqual(new Number(63), new String(63)),
		...
		assert.notOk(
			_.isEqual(new Date("Curly"), new Date("Curly")),
		...
		assert.ok(_.isEqual(/(?:)/gi, /(?:)/gi), "Flag order is not significant");
		assert.notOk(
			_.isEqual(/(?:)/g, /(?:)/gi),
			"RegExps with equivalent patterns and different flags are not equal"
		);
		assert.notOk(
			_.isEqual(/Moe/gim, /Curly/gim),
		...
		assert.ok(
			_.isEqual([1, "Larry", true], [1, "Larry", true]),
		...
		// Sparse arrays.
		assert.ok(
			_.isEqual(Array(3), Array(3)),
			"Sparse arrays of identical lengths are equal"
		);
		assert.notOk(
			_.isEqual(Array(3), Array(6)),
		...
		// Simple objects.
		assert.ok(
			_.isEqual({ a: "Curly", b: 1, c: true }, { a: "Curly", b: 1, c: true }),
		...
		// Chaining.
		assert.notOk(
			_.isEqual(_({ x: 1, y: void 0 }).chain(), _({ x: 1, z: 2 }).chain()),
		...
		b = _({ x: 1, y: 2 }).chain();
		assert.equal(
			_.isEqual(a.isEqual(b), _(true)),
			true,
			"`isEqual` can be chained"
		);
		...
		if (typeof Symbol !== "undefined") {
			var symbol = Symbol("x");
			assert.strictEqual(
				_.isEqual(symbol, symbol),
				true,
				"A symbol is equal to itself"
			);
			assert.strictEqual(
				_.isEqual(symbol, Object(symbol)),
				true,
				"Even when wrapped in Object()"
			);
			assert.strictEqual(
				_.isEqual(symbol, null),
				false,
	...
	QUnit.test("isEmpty", function(assert) {
		...
		assert.ok(
			...
			"objects with prototype properties are empty"
		);
		...
		assert.notOk(
			_.isEmpty(args("")),
			"non-empty arguments object is not empty"
		);

		// covers collecting non-enumerable properties in IE < 9
		var nonEnumProp = { toString: 5 };
		assert.notOk(
			_.isEmpty(nonEnumProp),
			"non-enumerable property is not empty"
		);
	});
	...
	QUnit.test("isArguments", function(assert) {
		...
		assert.notOk(
			_.isArguments("string"),
			"a string is not an arguments object"
		);
		assert.notOk(
			_.isArguments(_.isArguments),
			"a function is not an arguments object"
		);
		assert.ok(
			_.isArguments(args),
			"but the arguments object is an arguments object"
		);
		assert.notOk(
			_.isArguments(_.toArray(args)),
			"but not when it's converted into an array"
		);
	...
	QUnit.test("isString", function(assert) {
		...
		assert.strictEqual(
			_.isString("I am a string literal"),
			true,
			"string literals are"
		);
	...
	QUnit.test("isSymbol", function(assert) {
		...
		if (typeof Symbol === "function") {
			...
			assert.ok(
				_.isSymbol(Symbol("description")),
				"described symbols are symbols"
			);
	...
	QUnit.test("isBoolean", function(assert) {
		...
		assert.notOk(
			_.isBoolean(arguments),
			"the arguments object is not a boolean"
		);
	...
	if (typeof Int8Array !== "undefined") {
		QUnit.test("#1929 Typed Array constructors are functions", function(
			assert
		) {
			_.chain([
				"Float32Array",
				...
				.each(function(TypedArray) {
					...
					// as a function
					assert.strictEqual(
						_.isFunction(TypedArray),
						Object.prototype.toString.call(TypedArray) === "[object Function]"
					);
				});
		});
	}
	...
	QUnit.test("isFinite", function(assert) {
		...
		if (typeof Symbol === "function") {
			...
			assert.notOk(
				_.isFinite(Symbol("description")),
				"described symbols are not numbers"
			);
			assert.notOk(
				_.isFinite(Object(Symbol())),
				"boxed symbols are not numbers"
			);
		}
	});
	...
	QUnit.test("has", function(assert) {
		...
		assert.ok(
			_.has(obj, "foo"),
			"has() checks that the object has a property."
		);
		assert.notOk(
			_.has(obj, "baz"),
			"has() returns false if the object doesn't have the property."
		);
		assert.ok(_.has(obj, "func"), "has() works for functions too.");
		obj.hasOwnProperty = null;
		assert.ok(
			_.has(obj, "foo"),
			"has() works even when the hasOwnProperty method is deleted."
		);
		var child = {};
		child.prototype = obj;
		assert.notOk(
		...
		assert.strictEqual(
			_.has(null, "foo"),
			false,
			"has() returns false for null"
		);
		assert.strictEqual(
			_.has(void 0, "foo"),
			false,
			"has() returns false for undefined"
		);
	});
	...
	QUnit.test("isMatch", function(assert) {
		...
		assert.equal(
			_.isMatch(5, { __x__: void 0 }),
			false,
			"can match undefined props on primitives"
		);
		assert.equal(
			_.isMatch({ __x__: void 0 }, { __x__: void 0 }),
			true,
			"can match undefined props"
		);

		assert.equal(
			_.isMatch(null, {}),
			true,
			"Empty spec called with null object returns true"
		);
		assert.equal(
			_.isMatch(null, { a: 1 }),
			false,
			"Non-empty spec called with null object returns false"
		);
		...
		assert.equal(
			_.isMatch({ x: 2 }, specObj),
			true,
			"spec is restricted to own properties"
		);
		...
		assert.deepEqual(
			_.map([null, void 0, 5, {}], _.partial(_.isMatch, _, oCon)),
	...
	QUnit.test("matcher", function(assert) {
		...
		assert.equal(
			_.matcher({ __x__: void 0 })(5),
			false,
			"can match undefined props on primitives"
		);
		assert.equal(
			_.matcher({ __x__: void 0 })({ __x__: void 0 }),
			true,
			"can match undefined props"
		);
		...
		assert.strictEqual(
			_.find(stooges, _.matcher({ hair: false })),
			curly,
			"returns a predicate that can be used by finding functions."
		);
		assert.strictEqual(
			_.find(stooges, _.matcher(moe)),
			moe,
			"can be used to locate an object exists in a collection."
		);
		assert.deepEqual(
			_.filter([null, void 0], _.matcher({ a: 1 })),
			[],
			"Do not throw on null values."
		);

		assert.deepEqual(
			_.filter([null, void 0], _.matcher(null)),
			[null, void 0],
			"null matches null"
		);
		assert.deepEqual(
			_.filter([null, void 0], _.matcher({})),
			[null, void 0],
			"null matches {}"
		);
		assert.deepEqual(
			_.filter([{ b: 1 }], _.matcher({ a: void 0 })),
			[],
			"handles undefined values (1683)"
		);
		...
		assert.equal(
			m({ b: 1 }),
			true,
			"changing spec object doesnt change matches result"
		);

		//null edge cases
		var oCon = _.matcher({ constructor: Object });
		assert.deepEqual(
			_.map([null, void 0, 5, {}], oCon),
			[false, false, false, true],
			"doesnt falsey match constructor on undefined/null"
		);
	});
	...
	QUnit.test("findKey", function(assert) {
		...
		assert.equal(
			_.findKey(objects, function(obj) {
				return obj.a === 0;
			}),
			"a"
		);
		...
		assert.strictEqual(
			_.findKey([1, 2, 3, 4, 5, 6], function(obj) {
				return obj === 3;
			}),
			"2",
			"Keys are strings"
		...
		array.match = 55;
		assert.strictEqual(
			_.findKey(array, function(x) {
				return x === 55;
			}),
			"match",
			"matches array-likes keys"
		);
	});
	...
	QUnit.test("mapObject", function(assert) {
		...
		assert.deepEqual(
			_.mapObject(objects, function(val) {
				return _.reduce(
					val,
					function(memo, v) {
						return memo + v;
		...
		assert.deepEqual(
			_.mapObject(obj, function(val, key, o) {
				return o[key] * 2;
			}),
			{ a: 2, b: 4 },
			"correct keys"
		...
		assert.deepEqual(
			_.mapObject(protoObj, _.identity),
			{ a: 1 },
