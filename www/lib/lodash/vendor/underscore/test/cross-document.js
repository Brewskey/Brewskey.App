(function() {
	...
	QUnit.test("isEqual", function(assert) {
		...
		assert.ok(
			_.isEqual({}, iObject),
			"Objects with equivalent members created in different documents are equal"
		);

		// Array from another frame.
		assert.ok(
			_.isEqual([1, 2, 3], iArray),
			"Arrays with equivalent elements created in different documents are equal"
		);
	});
	...
	if (typeof ActiveXObject != "undefined") {
		...
		QUnit.test(
			"#1621 IE 11 compat mode DOM elements are not functions",
			function(assert) {
				var fn = function() {};
				var xml = new ActiveXObject("Msxml2.DOMDocument.3.0");
