(function() {
	...
	QUnit.test("first", function(assert) {
		...
		assert.equal(
			_.first(null),
			void 0,
			"returns undefined when called on null"
		);
		...
		assert.equal(
			_.first([]),
			void 0,
			"return undefined when called on a empty array"
		);
	...
	QUnit.test("rest", function(assert) {
		...
		assert.deepEqual(
			_.rest(numbers),
			[2, 3, 4],
			"fetches all but the first element"
		);
		assert.deepEqual(
			_.rest(numbers, 0),
			[1, 2, 3, 4],
	...
	QUnit.test("compact", function(assert) {
		assert.deepEqual(
			_.compact([1, false, null, 0, "", void 0, NaN, 2]),
			[1, 2],
	...
	QUnit.test("flatten", function(assert) {
		...
		assert.equal(
			_.flatten([_.range(10), _.range(10), 5, 1, 3], true).length,
			23,
			"can flatten medium length arrays"
		);
		assert.equal(
			_.flatten([_.range(10), _.range(10), 5, 1, 3]).length,
			23,
			"can shallowly flatten medium length arrays"
		);
		assert.equal(
			_.flatten([new Array(1000000), _.range(56000), 5, 1, 3]).length,
			1056003,
			"can handle massive arrays"
		);
		assert.equal(
			_.flatten([new Array(1000000), _.range(56000), 5, 1, 3], true).length,
			1056003,
	...
	QUnit.test("without", function(assert) {
		...
		assert.deepEqual(
			_.without(list, 0, 1),
			[2, 3, 4],
			"removes all instances of the given values"
		);
		var result = (function() {
		...
		list = [{ one: 1 }, { two: 2 }];
		assert.deepEqual(
			_.without(list, { one: 1 }),
			list,
			"compares objects by reference (value case)"
		);
		assert.deepEqual(
			_.without(list, list[0]),
			[{ two: 2 }],
			"compares objects by reference (reference case)"
		);
	});
	...
	QUnit.test("sortedIndex", function(assert) {
		...
		assert.equal(
			indexFor35,
			3,
			"finds the index at which a value should be inserted to retain order"
		);
		var indexFor30 = _.sortedIndex(numbers, 30);
		assert.equal(
			indexFor30,
			2,
			"finds the smallest index at which a value could be inserted to retain order"
		);
		...
		assert.strictEqual(
			_.sortedIndex(objects, { x: 25 }, iterator),
			2,
			"uses the result of `iterator` for order comparisons"
		);
		assert.strictEqual(
			_.sortedIndex(objects, { x: 35 }, "x"),
			3,
			"when `iterator` is a string, uses that key for order comparisons"
		);
		...
		assert.strictEqual(
			_.sortedIndex([1, 3], 2, iterator, context),
			1,
			"can execute its iterator in the given context"
		);
		...
		assert.equal(
			_.sortedIndex(largeArray, 2147483648),
			2147483648,
			"works with large indexes"
		);
	});
	...
	QUnit.test("uniq", function(assert) {
		...
		assert.deepEqual(
			_.uniq([a, b, a, b, c]),
			[a, b, c],
			"works on values that can be tested for equivalency but not ordered"
		);

		assert.deepEqual(
			_.uniq(null),
			[],
			"returns an empty array when `array` is not iterable"
		);
		...
		_.uniq(
			...
			function(value, index, array) {
				assert.strictEqual(
					this,
					context,
					"executes its iterator in the given context"
				);
	...
	QUnit.test("intersection", function(assert) {
		var stooges = ["moe", "curly", "larry"],
			leaders = ["moe", "groucho"];
		assert.deepEqual(
			_.intersection(stooges, leaders),
			["moe"],
		...
		assert.deepEqual(
			_.intersection(theSixStooges, leaders),
			["moe"],
		...
		assert.deepEqual(
			result,
			[],
			"returns an empty array when passed null as the first argument"
		);
		result = _.intersection([1, 2, 3], null);
		assert.deepEqual(
			result,
			[],
			"returns an empty array when passed null as an argument beyond the first"
		);
	});
	...
	QUnit.test("union", function(assert) {
		...
		assert.deepEqual(
			result,
			[1, 2, 3, 30, 40],
			"can find the union of a list of arrays"
		);
		...
		assert.deepEqual(
			result,
			[1, 2, 3, 30, 40, [1]],
			"can find the union of a list of nested arrays"
		);
		...
		assert.deepEqual(
			result,
			[10, 20, 1, 30, 0, 40],
			"orders values by their first encounter"
		);
		...
		assert.deepEqual(
			_.union([1, 2, 3], 4),
			[1, 2, 3],
			"restricts the union to arrays only"
		);
	});
	...
	QUnit.test("difference", function(assert) {
		...
		assert.deepEqual(
			result,
			[1, 2, 3],
			"restrict the difference to arrays only"
		);
	});
	...
	QUnit.test("zip", function(assert) {
		...
		assert.deepEqual(
			...
			[["moe", 30, true], ["larry", 40, void 0], ["curly", 50, void 0]],
		...
		assert.deepEqual(
			stooges,
			[["moe", "larry", "curly"], [30, 40, 50], [void 0, void 0, "extra data"]],
	...
	QUnit.test("unzip", function(assert) {
		...
		assert.deepEqual(
			_.unzip(zipped),
			[["moe", 30, void 0], ["larry", 40, void 0], ["curly", 50, "extra data"]],
	...
	QUnit.test("indexOf", function(assert) {
		...
		assert.strictEqual(
			_.indexOf(array, 1, -3),
			3,
			"neg `fromIndex` starts at the right index"
		);
		assert.strictEqual(
			_.indexOf(array, 1, -2),
			-1,
	...
	QUnit.test("indexOf with NaN", function(assert) {
		assert.strictEqual(
			_.indexOf([1, 2, NaN, NaN], NaN),
			2,
			"Expected [1, 2, NaN] to contain NaN"
		);
		assert.strictEqual(
			_.indexOf([1, 2, Infinity], NaN),
			-1,
			"Expected [1, 2, NaN] to contain NaN"
		);

		assert.strictEqual(
			_.indexOf([1, 2, NaN, NaN], NaN, 1),
			2,
			"startIndex does not affect result"
		);
		assert.strictEqual(
			_.indexOf([1, 2, NaN, NaN], NaN, -2),
			2,
			"startIndex does not affect result"
		);
		...
		(function() {
			assert.strictEqual(
				_.indexOf(arguments, NaN),
				2,
				"Expected arguments [1, 2, NaN] to contain NaN"
			);
		})(1, 2, NaN, NaN);
	});
	...
	QUnit.test("lastIndexOf", function(assert) {
		...
		assert.strictEqual(
			_.lastIndexOf(array, 1, 0),
			0,
			"starts at the correct from idx"
		);
		assert.strictEqual(
			_.lastIndexOf(array, 3),
			5,
			"should return the index of the last matched value"
		);
		assert.strictEqual(
			_.lastIndexOf(array, 4),
			-1,
			"should return `-1` for an unmatched value"
		);

		assert.strictEqual(
			_.lastIndexOf(array, 1, 2),
			0,
			"should work with a positive `fromIndex`"
		);
		...
		assert.strictEqual(
			...
			1,
			"should work with a negative `fromIndex`"
		);
		assert.strictEqual(
			_.lastIndexOf(array, 1, -3),
			3,
			"neg `fromIndex` starts at the right index"
		);

		assert.deepEqual(
			_.map([-6, -8, -Infinity], function(fromIndex) {
	...
	QUnit.test("lastIndexOf with NaN", function(assert) {
		assert.strictEqual(
			_.lastIndexOf([1, 2, NaN, NaN], NaN),
			3,
			"Expected [1, 2, NaN] to contain NaN"
		);
		assert.strictEqual(
			_.lastIndexOf([1, 2, Infinity], NaN),
			-1,
			"Expected [1, 2, NaN] to contain NaN"
		);

		assert.strictEqual(
			_.lastIndexOf([1, 2, NaN, NaN], NaN, 2),
			2,
			"fromIndex does not affect result"
		);
		assert.strictEqual(
			_.lastIndexOf([1, 2, NaN, NaN], NaN, -2),
			2,
	...
	QUnit.test("findIndex", function(assert) {
		...
		assert.equal(
			_.findIndex(objects, function(obj) {
				return obj.a === 0;
			}),
			0
		);
		...
		assert.strictEqual(
			_.findIndex(objects, function(a) {
				return a.foo === null;
			}),
			-1
		);
		...
		sparse[20] = { a: 2, b: 2 };
		assert.equal(
			_.findIndex(sparse, function(obj) {
	...
	QUnit.test("findLastIndex", function(assert) {
		...
		assert.equal(
			_.findLastIndex(objects, function(obj) {
				return obj.a === 0;
			}),
			3
		);
		...
		assert.strictEqual(
			_.findLastIndex(objects, function(a) {
				return a.foo === null;
			}),
			-1
		);
		...
		sparse[20] = { a: 2, b: 2 };
		assert.equal(
			_.findLastIndex(sparse, function(obj) {
				return obj && obj.b * obj.a === 4;
			}),
			20,
			"Works with sparse arrays"
		...
		array.match = 55;
		assert.strictEqual(
			_.findLastIndex(array, function(x) {
				return x === 55;
			}),
			-1,
			"doesn't match array-likes keys"
	...
	QUnit.test("range", function(assert) {
		...
		assert.deepEqual(
			_.range(8, 5),
			[8, 7, 6],
			"negative range generates descending array"
		);
		assert.deepEqual(
			_.range(-3),
			[0, -1, -2],
	...
	QUnit.test("chunk", function(assert) {
		assert.deepEqual(
			_.chunk([], 2),
			[],
			"chunk for empty array returns an empty array"
		);

		assert.deepEqual(
			_.chunk([1, 2, 3], 0),
			[],
			"chunk into parts of 0 elements returns empty array"
		);
		assert.deepEqual(
			_.chunk([1, 2, 3], -1),
			[],
			"chunk into parts of negative amount of elements returns an empty array"
		);
		assert.deepEqual(
			_.chunk([1, 2, 3]),
			[],
		...
		assert.deepEqual(
			_.chunk([1, 2, 3], 1),
			[[1], [2], [3]],
			"chunk into parts of 1 elements returns original array"
		);

		assert.deepEqual(
			_.chunk([1, 2, 3], 3),
			[[1, 2, 3]],
			"chunk into parts of current array length elements returns the original array"
		);
		assert.deepEqual(
			_.chunk([1, 2, 3], 5),
			[[1, 2, 3]],
			"chunk into parts of more then current array length elements returns the original array"
		);

		assert.deepEqual(
			_.chunk([10, 20, 30, 40, 50, 60, 70], 2),
			[[10, 20], [30, 40], [50, 60], [70]],
			"chunk into parts of less then current array length elements"
		);
		assert.deepEqual(
			_.chunk([10, 20, 30, 40, 50, 60, 70], 3),
			[[10, 20, 30], [40, 50, 60], [70]],
			"chunk into parts of less then current array length elements"
		);
	});
