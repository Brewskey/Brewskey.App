(function() {
	...
	var filePath = (function() {
		...
		result =
			result.length > min && !/perf(?:\.js)?$/.test(last)
				? last
				: "../lodash.js";
	...
	var load = typeof require == "function" && !amd ? require : noop;

	/** Load lodash. */
	var lodash =
		root.lodash ||
		(root.lodash = ((lodash = load(filePath) || root._),
	...
	var _ =
		root.underscore ||
		(root.underscore = ((_ =
			load("../vendor/underscore/underscore.js") || root._),
		_._ || _));
	...
	var Benchmark =
		...
		(root.Benchmark = ((Benchmark =
			load("../node_modules/benchmark/benchmark.js") || root.Benchmark),
   ...
   */
	function basename(filePath, extension) {
		...
		return arguments.length < 2
			? result
			: result.replace(
					RegExp(extension.replace(reSpecialChars, "\\$&") + "$"),
					""
				);
	}
   ...
   */
	function run() {
		fbPanel =
			(fbPanel = root.document && document.getElementById("FirebugUI")) &&
			(fbPanel =
				(fbPanel = fbPanel.contentWindow || fbPanel.contentDocument).document ||
				fbPanel) &&
				...
				if (fastest.length > 1) {
					...
					log(
						fastest[0].name +
							" is " +
							formatNumber(
								percent < 1 ? percent.toFixed(2) : Math.round(percent)
							) +
							"% faster."
					);
				}
			...
			if (suites.length) {
				...
				var aMeanHz = getGeometricMean(score.a),
					...
					message =
						...
						(xFaster == 1
							? ""
							: "(" + formatNumber(xFaster.toFixed(2)) + "x) ") +
						"faster than";
	...
	suites.push(
		Benchmark.Suite(
			"`_.invokeMap` with a function for `path` iterating an array"
		)
			.add(
				buildName,
				"\
	...
	suites.push(
		Benchmark.Suite(
			"`_.isEqual` comparing primitives and their object counterparts (edge case)"
		)
			.add(buildName, {
				fn:
					"\
	...
	suites.push(
		Benchmark.Suite("`_.partial` (slow path)")
			.add(buildName, {
				fn:
					'lodash.partial(function(greeting) { return greeting + " " + this.name; }, "hi")',
				teardown: "function partial(){}"
			})
			.add(otherName, {
				fn:
					'_.partial(function(greeting) { return greeting + " " + this.name; }, "hi")',
				teardown: "function partial(){}"
			})
	);
