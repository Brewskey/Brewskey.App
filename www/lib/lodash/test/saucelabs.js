...
if (env.TRAVIS_SECURE_ENV_VARS == "false") {
	console.log(
		"Skipping Sauce Labs jobs; secure environment variables are unavailable"
	);
	process.exit(0);
}
...
var advisor = getOption("advisor", false),
	...
	customData = Function(
		"return {" + getOption("customData", "").replace(/^\{|}$/g, "") + "}"
	)(),
	...
	port =
		ports[
			Math.min(_.sortedIndex(ports, getOption("port", 9001)), ports.length - 1)
		],
...
function optionToArray(name, string) {
	return _.compact(
		_.invokeMap((optionToValue(name, string) || "").split(/, */), "trim")
	);
}
...
function onJobStart(error, res, body) {
	...
	if (error || !taskId || statusCode != 200) {
		...
		logInline();
		console.error(
			"Failed to start job; status: %s, body: %s",
			statusStr,
			bodyStr
		);
		if (error) {
...
function onJobStatus(error, res, body) {
	...
	if (!completed && !expired) {
		this._pollerId = _.delay(
			_.bind(this.status, this),
	...
	var description =
			...
			platform[2] +
			" on " +
			_.startCase(platform[0]),
		errored =
			!jobResult ||
			!jobResult.passed ||
			reError.test(message) ||
			reError.test(jobStatus),
...
Job.prototype.remove = function(callback) {
	...
	return this.stop(function() {
		...
		request.del(
			_.template("https://saucelabs.com/rest/v1/${user}/jobs/${id}")(this),
			{
...
Job.prototype.start = function(callback) {
	...
	this.starting = true;
	request.post(
		_.template("https://saucelabs.com/rest/v1/${user}/js-tests")(this),
		{
			auth: { user: this.user, pass: this.pass },
			json: this.options
		},
...
Job.prototype.status = function(callback) {
	...
	this.checking = true;
	request.post(
		_.template("https://saucelabs.com/rest/v1/${user}/js-tests/status")(this),
		{
			auth: { user: this.user, pass: this.pass },
...
Job.prototype.stop = function(callback) {
	...
	request.put(
		_.template("https://saucelabs.com/rest/v1/${user}/jobs/${id}/stop")(this),
		{
			auth: { user: this.user, pass: this.pass }
		},
		onStop
...
function Tunnel(properties) {
	...
	_.invokeMap(all, "on", "restart", function() {
		...
		if (
			tunnel.attempts < tunnel.retries &&
			active.length >= threshold &&
			_.isEmpty(_.difference(active, restarted))
		) {
			tunnel.restart();
		}
	});
	...
	this.connection = new SauceTunnel(
		this.user,
		this.pass,
		this.id,
		this.tunneled,
