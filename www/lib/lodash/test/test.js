(function() {
	...
	var isNpm =
		isModularize && /\bnpm\b/.test([ui.buildPath, ui.urlParams.build]);
	...
	if (phantom && isModularize) {
		...
		page.onCallback = function(details) {
			...
			if (coverage) {
				...
				fs.write(
					[cwd, "coverage", "coverage.json"].join(sep),
		...
		console.log(
			"test.js invoked with arguments: " + JSON.stringify(slice.call(params))
		);
		return;
	}
   ...
   */
	...
	lodashStable.attempt(function() {
		...
		if (isModularize && !(amd || isNpm)) {
			lodashStable.each(["baseEach", "isIndex", "isIterateeCall"], function(
				funcName
			) {
	...
	(function() {
		...
		QUnit.test(
			"should support loading " +
				basename +
				' with the Require.js "shim" configuration option',
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"should not add `Function.prototype` extensions to lodash",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should avoid non-native built-ins", function(assert) {
			...
			function message(lodashMethod, nativeMethod) {
				return (
					"`" +
					lodashMethod +
					"` should avoid overwritten native `" +
					nativeMethod +
					"`"
				);
			}
			...
			if (lodashBizarro) {
				...
				label = message(
					"_.difference`, `_.intersection`, and `_.uniq",
					"Object.create` and `Map"
				);
	...
	(function() {
		...
		QUnit.test("should return `false` for non-iteratee calls", function(
			assert
		) {
			assert.expect(4);
	...
	(function() {
		...
		lodashStable.forOwn(createCaches(pairs), function(cache, kind) {
			QUnit.test("should implement a `Map` interface for " + kind, function(
				assert
			) {
				assert.expect(82);
	...
	(function() {
		...
		QUnit.test(
			"should convert foreign wrapped values to `lodash` instances",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should use `func.length` if `n` is not given", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work as an iteratee for methods like `_.map`", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"should work when combined with other methods that use metadata",
			function(assert) {
				assert.expect(2);
	...
	lodashStable.each(["assign", "assignIn"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should accept multiple sources",
			function(assert) {
				assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should return `undefined` for nonexistent keys", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work with non-index keys on array values", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should return an empty array when no keys are given", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			"should work with a falsey `object` argument when keys are given",
			function(assert) {
				assert.expect(1);

				var expected = lodashStable.map(
					falsey,
					lodashStable.constant(Array(4))
				);
		...
		QUnit.test("should work with an `arguments` object for `object`", function(
			assert
		) {
			assert.expect(1);
			...
			if (!isNpm) {
				...
				lodashStable.each([[2], ["2"], [2, 1]], function(paths) {
					lodashStable.times(2, function(index) {
						...
						assert.deepEqual(
							wrapped.value(),
							_.at(_.map(array, identity), paths)
						);
					});
				});
			} else {
		...
		QUnit.test("work with an object for `object` when chaining", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should provide additional arguments to `func`", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work with an error object from another realm", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			...
			function(assert) {
				...
				if (!isNpm) {
					assert.ok(
						_(lodashStable.constant("x"))
							.chain()
							.attempt() instanceof _
					);
				} else {
					skipAssert(assert);
				}
	...
	(function() {
		...
		QUnit.test("should create a function with a `length` of `0`", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			"should ignore binding when called with the `new` operator",
			function(assert) {
				assert.expect(3);
			...
			function(assert) {
				...
				var thisArg = { a: 1 },
					...
					expected = lodashStable.times(
						count,
						lodashStable.constant([undefined, undefined])
					);
		...
		QUnit.test(
			"should append array arguments to partially applied arguments",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should not error when instantiating bound built-ins", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			...
			function(assert) {
				...
				var createCtor = lodashStable.attempt(
					Function,
					'"use strict";return class A{}'
				);
	...
	(function() {
		...
		QUnit.test(
			"should work with `arguments` objects as secondary arguments",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		QUnit.test("should work when the target function is overwritten", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			"should ensure `new bound` is an instance of `object[key]`",
			function(assert) {
				assert.expect(2);
	...
	lodashStable.each(
		...
		function(caseName) {
			...
			QUnit.test("`_." + methodName + "` should deburr letters", function(
				assert
			) {
				assert.expect(1);
			...
			QUnit.test(
				"`_." + methodName + "` should remove latin-1 mathematical operators",
				function(assert) {
					assert.expect(1);
			...
			QUnit.test(
				"`_." +
					methodName +
					"` should return an unwrapped value implicitly when chaining",
				function(assert) {
					assert.expect(1);
	...
	(function() {
		QUnit.test("should capitalize the first character of a string", function(
			assert
		) {
			assert.expect(3);
	...
	(function() {
		...
		QUnit.test("should return the last chunk as remaining elements", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should coerce `min` and `max` of `NaN` to `0`", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test(
			"`_.cloneDeep` should deep clone objects with circular references",
			function(assert) {
				assert.expect(1);
		...
		lodashStable.each(["clone", "cloneDeep"], function(methodName) {
			...
			lodashStable.forOwn(objects, function(object, kind) {
				QUnit.test("`_." + methodName + "` should clone " + kind, function(
					assert
				) {
					assert.expect(2);
			...
			QUnit.test("`_." + methodName + "` should clone array buffers", function(
				assert
			) {
				assert.expect(2);
			...
			QUnit.test("`_." + methodName + "` should clone buffers", function(
				assert
			) {
				assert.expect(4);
			...
			QUnit.test(
				"`_." + methodName + "` should clone prototype objects",
				function(assert) {
					assert.expect(2);
			...
			QUnit.test("`_." + methodName + "` should clone symbol objects", function(
				assert
			) {
				assert.expect(4);
					...
					var objects = lodashStable.transform(
						...
						function(result, value, key) {
							if (
								...
								props.push(
									lodashStable.capitalize(lodashStable.camelCase(key))
								);
								result.push(value);
							}
						},
			...
			lodashStable.forOwn(uncloneable, function(value, key) {
				QUnit.test("`_." + methodName + "` should not clone " + key, function(
					assert
				) {
					assert.expect(3);
					...
					if (value) {
						var object = { a: value, b: { c: value } },
							actual = func(object),
							expected =
								typeof value == "function" && !!value.c ? { c: Foo.c } : {};
	...
	(function() {
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.take(_.compact(_.slice(largeArray, 1)).reverse())
				);
			} else {
				skipAssert(assert);
			}
				...
				if (!isModularize) {
					...
					try {
						var actual = _(largeArray)
							...
							.compact()
							.value();
						pass = lodashStable.isEqual(
							actual,
							_.compact(_.slice(largeArray, 1))
						);
					} catch (e) {
						console.log(e);
					}
	...
	(function() {
		...
		QUnit.test("should cast non-array `array` values to arrays", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should work with predicate shorthands", function(assert) {
			...
			var cond = _.cond([[{ a: 1 }, stubA], [["b", 1], stubB], ["c", stubC]]);
		...
		QUnit.test("should return `undefined` when no condition is met", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should use `this` binding of function for `pairs`", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		QUnit.test("should not change behavior if `source` is modified", function(
			assert
		) {
			assert.expect(2);
	...
	lodashStable.each(["conforms", "conformsTo"], function(methodName) {
		...
		QUnit.test(
			...
			function(assert) {
				...
				var objects = [{ a: 1, b: 8 }, { a: 2, b: 4 }, { a: 3, b: 16 }];
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should not match by inherited `source` properties",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should not invoke `source` predicates for missing `object` properties",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should return `true` when comparing an empty `source`",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		QUnit.test("should create a function that returns `value`", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should use `_.identity` when `iteratee` is nullish", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				var array = lodashStable
					...
					.concat(
						lodashStable.range(
							Math.floor(LARGE_ARRAY_SIZE / 2),
							LARGE_ARRAY_SIZE
						),
						lodashStable.range(
							Math.floor(LARGE_ARRAY_SIZE / 1.5),
							LARGE_ARRAY_SIZE
						)
					);
				...
				assert.deepEqual(
					actual,
					_.take(_.filter(_.map(_.countBy(array), square), isEven))
				);
			} else {
				skipAssert(assert);
			}
	...
	(function() {
		...
		QUnit.test("should assign `properties` to the created object", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test(
			"should ignore primitive `prototype` arguments and use an empty object instead",
			function(assert) {
				assert.expect(1);
				...
				var actual = lodashStable.map(primitives, function(value, index) {
					return lodashStable.isPlainObject(
						index ? _.create(value) : _.create()
					);
				});
	...
	(function() {
		...
		QUnit.test("should curry based on the number of arguments given", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should ensure `new curried` is an instance of `func`", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should use `this` binding of function", function(assert) {
			...
			assert.deepEqual(
				_.curryRight(_.bind(fn, object), 3)("c")("b")("a"),
				expected
			);
			assert.deepEqual(
				_.curryRight(_.bind(fn, object), 3)("b", "c")("a"),
				expected
			);
			assert.deepEqual(
				_.curryRight(_.bind(fn, object), 3)("a", "b", "c"),
				expected
			);

			assert.deepEqual(
				_.bind(_.curryRight(fn), object)("c")("b")("a"),
				Array(3)
			);
			assert.deepEqual(
				_.bind(_.curryRight(fn), object)("b", "c")("a"),
				Array(3)
			);
			assert.deepEqual(
				_.bind(_.curryRight(fn), object)("a", "b", "c"),
				expected
			);
	...
	lodashStable.each(["curry", "curryRight"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should not error on functions with the same name as lodash methods",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should work for function names that shadow those on `Object.prototype`",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		QUnit.test(
			"should convert latin-1 supplementary letters to basic latin",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should not deburr latin-1 mathematical operators", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should accept multiple sources", function(assert) {
			...
			var expected = { a: 1, b: 2, c: 3 };
			assert.deepEqual(
				_.defaults({ a: 1, b: 2 }, { b: 3 }, { c: 3 }),
				expected
			);
			assert.deepEqual(
				_.defaults({ a: 1, b: 2 }, { b: 3, c: 3 }, { c: 2 }),
				expected
			);
		});
	...
	(function() {
		...
		QUnit.test("should accept multiple sources", function(assert) {
			...
			assert.deepEqual(
				_.defaultsDeep({ a: { b: 2 } }, source1, source2),
				expected
			);
			assert.deepEqual(
				_.defaultsDeep({ a: { b: 2 } }, source3, source4),
				expected
			);
		});
		...
		QUnit.test("should not convert function properties to objects", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should merge sources containing circular references", function(
			assert
		) {
			assert.expect(2);
	...
	lodashStable.each(["difference", "differenceBy", "differenceWith"], function(
		methodName
	) {
		var args = (function() {
				return arguments;
		...
		QUnit.test("`_." + methodName + "` should treat `-0` as `0`", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("`_." + methodName + "` should work with large arrays", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return an empty array when `n` >= `length`", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.dropRight(_.drop(_.dropRight(_.drop(array, 2))), 2)
				);
				...
				assert.deepEqual(
					actual,
					_.dropRight(
						_.drop(_.dropRight(_.drop(_.filter(_.drop(array), predicate), 2))),
						2
					)
				);
			} else {
	...
	(function() {
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.dropRight(_.dropRight(_.filter(array, predicate), 2))
				);
				...
				assert.deepEqual(
					actual,
					_.drop(_.dropRight(_.drop(_.dropRight(array, 2))), 2)
				);
				...
				assert.deepEqual(
					...
					_.drop(
						_.dropRight(
							_.drop(_.dropRight(_.filter(_.dropRight(array), predicate), 2))
						),
						2
					)
				);
	...
	(function() {
		...
		var objects = [{ a: 0, b: 0 }, { a: 1, b: 1 }, { a: 2, b: 2 }];

		QUnit.test(
			"should drop elements while `predicate` returns truthy",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should provide correct `predicate` arguments", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work with `_.matches` shorthands", function(assert) {
			...
			assert.deepEqual(
				_.dropRightWhile(objects, { b: 2 }),
				objects.slice(0, 2)
			);
		});

		QUnit.test("should work with `_.matchesProperty` shorthands", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		var objects = [{ a: 2, b: 2 }, { a: 1, b: 1 }, { a: 0, b: 0 }];
	...
	(function() {
		...
		QUnit.test("should return `true` if a string ends with `target`", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should handle strings with nothing to escape", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should escape values", function(assert) {
			...
			assert.strictEqual(
				_.escapeRegExp(unescaped + unescaped),
				escaped + escaped
			);
		});
		...
		QUnit.test("should return an empty string for empty values", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.strictEqual(
					_.every([undefined, undefined, undefined], identity),
					false
				);
			}
		);

		QUnit.test("should use `_.identity` when `predicate` is nullish", function(
			assert
		) {
			assert.expect(2);
	...
	lodashStable.each(["assign", "assignIn", "bindAll", "defaults"], function(
		methodName
	) {
		var func = _[methodName],
	...
	(function() {
		...
		QUnit.test("should use `undefined` for `value` if not given", function(
			assert
		) {
			assert.expect(2);
	...
	_.each(["find", "findIndex", "findLast", "findLastIndex"], function(
		methodName
	) {
		var func = _[methodName];
	...
	lodashStable.each(["includes", "find"], function(methodName) {
		var args = (function() {
			...
			resolve =
				methodName == "find" ? lodashStable.curry(lodashStable.eq) : identity;
			...
			function(collection, key) {
				...
				QUnit.test(
					"`_." +
						...
						"` should work with " +
						key +
						" and coerce `fromIndex` to an integer",
					function(assert) {
						assert.expect(1);
			...
			function(collection, key) {
				...
				QUnit.test(
					...
					function(assert) {
						...
						var expected = [values[2], undefined];
	...
	(function() {
		...
		QUnit.test("should treat a `depth` of < `1` as a shallow clone", function(
			assert
		) {
			assert.expect(2);
	...
	lodashStable.each(["flatMap", "flatMapDeep", "flatMapDepth"], function(
		methodName
	) {
		var func = _[methodName],
			array = [1, 2, 3, 4];
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should map values in `array` to a new flattened array",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." + methodName + "` should accept a falsey `collection` argument",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should work with objects with non-number length properties",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["flow", "flowRight"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should return a new function", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should return an identity function when no arguments are given",
			function(assert) {
				assert.expect(6);
			...
			function(assert) {
				...
				lodashStable.times(2, function(index) {
					var filter1 = _.filter,
						...
						filter3 = ((_.filter = index ? filter2 : filter1),
						filter2(predicate));
	...
	(function() {
		...
		lodashStable.each(lodashStable.difference(methods, objectMethods), function(
			methodName
		) {
			var array = [1, 2, 3],
				func = _[methodName],
		...
		lodashStable.each(iterationMethods, function(methodName) {
			...
			QUnit.test(
				"`_." + methodName + "` should return the collection",
				function(assert) {
					assert.expect(1);
		...
		lodashStable.each(methods, function(methodName) {
			...
			QUnit.test(
				"`_." + methodName + "` should ignore changes to `length`",
				function(assert) {
					assert.expect(1);
	...
	lodashStable.each(["assign", "assignIn", "defaults", "merge"], function(
		methodName
	) {
		var func = _[methodName],
	...
	lodashStable.each(["assignWith", "assignInWith", "mergeWith"], function(
		methodName
	) {
		var func = _[methodName],
	...
	(function() {
		QUnit.test(
			"internal data objects should work with the `__proto__` key",
			function(assert) {
				assert.expect(4);
				...
				assert.deepEqual(
					_.without.apply(_, [largeArray].concat(largeArray)),
					[]
				);
			}
		);
	...
	(function() {
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.take(_.filter(_.map(_.fromPairs(array), square), isEven))
				);
			} else {
				skipAssert(assert);
			}
	...
	(function() {
		QUnit.test("should return the function names of an object", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.take(
						_.filter(lodashStable.map(_.groupBy(array), iteratee), predicate)
					)
				);
			} else {
				skipAssert(assert);
	...
	(function() {
		...
		QUnit.test("should return `false` if `value` is <= `other`", function(
			assert
		) {
			assert.expect(4);
	...
	lodashStable.each(["has", "hasIn"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should not use the `hasOwnProperty` method of the object",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("`_." + methodName + "` should support deep paths", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test(
			...
			function(assert) {
				...
				var actual = lodashStable.transform(objects, function(
					result,
					object,
					index
				) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should return `true` for index values within bounds for arrays, `arguments` objects, and strings",
			function(assert) {
				assert.expect(2);
				...
				expected = lodashStable.map(
					values,
					lodashStable.constant([true, true])
				);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should return `false` for deep paths when `object` is nullish",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should return `false` for nullish values of nested objects",
			function(assert) {
				assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should return `undefined` when querying empty arrays", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		lodashStable.each(
			...
			function(collection, key) {
				...
				QUnit.test(
					"should work with " +
						key +
						" and  return `false` for unmatched values",
					function(assert) {
						assert.expect(1);
				...
				QUnit.test(
					"should work with " +
						key +
						" and return an unwrapped value implicitly when chaining",
					function(assert) {
						assert.expect(1);
				...
				QUnit.test(
					"should work with " +
						key +
						" and return a wrapped value when explicitly chaining",
					function(assert) {
						assert.expect(1);
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.ok(
					lodashStable.every(values, lodashStable.partial(_.includes, array))
				);
			}
		);
	})(1, 2, 3, 4);
	...
	(function() {
		...
		QUnit.test("should return an empty when querying empty arrays", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with `start` and `end` arguments", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should treat falsey `start` arguments as `0`", function(
			assert
		) {
			assert.expect(13);
		...
		QUnit.test("should swap `start` and `end` when `start` > `end`", function(
			assert
		) {
			assert.expect(2);
	...
	lodashStable.each(
		...
		function(methodName) {
			...
			QUnit.test(
				"`_." + methodName + "` should work with a single array",
				function(assert) {
					assert.expect(1);
			...
			QUnit.test(
				...
				function(assert) {
					...
					assert.deepEqual(func(lodashStable.range(LARGE_ARRAY_SIZE), [1]), [
						1
					]);
				}
			);
	...
	(function() {
		QUnit.test(
			"should invoke a methods on each element of `collection`",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should work with a function for `methodName`", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should treat number values for `collection` as empty", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should not error on elements with missing properties", function(
			assert
		) {
			assert.expect(1);

			var objects = lodashStable.map([null, undefined, stubOne], function(
				value
			) {
				return { a: value };
			});
	...
	(function() {
		...
		QUnit.test("should return `true` for `arguments` objects", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should return `false` for non `arguments` objects", function(
			assert
		) {
			assert.expect(12);
			...
			assert.strictEqual(
				_.isArguments({ "0": 1, callee: noop, length: 1 }),
				false
			);
	...
	(function() {
		...
		QUnit.test("should work with an array from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with array buffers from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with a boolean from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return `false` if `Buffer` is not defined", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with a date object from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with a DOM element from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should compare objects regardless of key order", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should compare objects with constructor properties", function(
			assert
		) {
			assert.expect(5);

			assert.strictEqual(
				_.isEqual({ constructor: 1 }, { constructor: 1 }),
				true
			);
			assert.strictEqual(
				_.isEqual({ constructor: 1 }, { constructor: "1" }),
		...
		QUnit.test("should compare arrays with circular references", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should compare objects with circular references", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should compare objects with shared property values", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"should treat objects created by `Object.create(null)` like a plain object",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test("should avoid common type coercions", function(assert) {
			...
			assert.strictEqual(
				_.isEqual(1337756400000, new Date(2012, 4, 23)),
				false
			);
		...
		QUnit.test(
			"should treat `arguments` objects like `Object` objects",
			function(assert) {
				assert.expect(4);
		...
		QUnit.test("should compare array views", function(assert) {
			...
			lodashStable.times(2, function(index) {
				...
				var expected = lodashStable.map(
					pairs,
					lodashStable.constant([true, false, false])
				);
		...
		QUnit.test("should compare date objects", function(assert) {
			...
			assert.strictEqual(
				_.isEqual(date, { getTime: lodashStable.constant(+date) }),
				false
			);
		});
		...
		QUnit.test("should compare maps with circular references", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should compare promises by reference", function(assert) {
			...
			if (promise) {
				lodashStable.each(
					[[promise, Promise.resolve(1)], [promise, realm.promise]],
					function(promises) {
		...
		QUnit.test("should compare sets with circular references", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should compare wrapped and non-wrapped values", function(
			assert
		) {
			assert.expect(4);
	...
	(function() {
		QUnit.test("should provide correct `customizer` arguments", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.strictEqual(_.isEqualWith(["a"], ["b"], customizer), true);
				assert.strictEqual(
					_.isEqualWith({ "0": "a" }, { "0": "b" }, customizer),
					true
				);
			}
		);
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.strictEqual(_.isEqualWith(["a"], ["a"], customizer), false);
				assert.strictEqual(
					_.isEqualWith({ "0": "a" }, { "0": "a" }, customizer),
					false
				);
			}
		);
		...
		QUnit.test("should call `customizer` for values maps and sets", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should return `false` for non-numeric values", function(
			assert
		) {
			assert.expect(10);
		...
		QUnit.test("should return `false` for numeric string values", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return `true` for generator functions", function(
			assert
		) {
			assert.expect(1);

			assert.strictEqual(
				_.isFunction(generator),
				typeof generator == "function"
			);
		});

		QUnit.test("should return `true` for array view constructors", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should return `false` for non-functions", function(assert) {
			...
			if (document) {
				assert.strictEqual(
					_.isFunction(document.getElementsByTagName("body")),
					false
				);
			} else {
				skipAssert(assert);
		...
		QUnit.test(
			"should work with host objects in IE 8 document mode (test in IE 11)",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test("should work with a function from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	lodashStable.each(["isInteger", "isSafeInteger"], function(methodName) {
		...
		QUnit.test("should return `false` for non-integer number values", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.strictEqual(
					_.isMatchWith({ "0": "a" }, { "0": "b" }, customizer),
					true
				);
			}
		);
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.strictEqual(
					_.isMatchWith({ "0": "a" }, { "0": "a" }, customizer),
					false
				);
			}
		);
		...
		QUnit.test(
			...
			function(assert) {
				...
				lodashStable.each(falsey, function(value) {
					actual.push(
						_.isMatchWith(object, { a: 2 }, lodashStable.constant(value))
					);
				});
	...
	(function() {
		...
		QUnit.test("should return `false` for non-native methods", function(
			assert
		) {
			assert.expect(12);
		...
		QUnit.test("should work with native functions from another realm", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should throw an error if core-js is detected", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should detect methods masquerading as native", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should return `false` for non-nullish values", function(
			assert
		) {
			assert.expect(13);
	...
	(function() {
		...
		QUnit.test(
			"should avoid `[xpconnect wrapped native prototype]` in Firefox",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			"should return `true` for plain objects with a custom `valueOf` property",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			'should return `false` for Object objects without a `toStringTag` of "Object"',
			function(assert) {
				assert.expect(3);
	...
	(function() {
		...
		QUnit.test("should work with weak sets from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with typed arrays from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return `false` for non `undefined` values", function(
			assert
		) {
			assert.expect(13);
		...
		QUnit.test("should work with `undefined` from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with weak maps from another realm", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should not error on host objects (test in IE)", function(
			assert
		) {
			assert.expect(26);
	...
	(function() {
		...
		QUnit.test("should return `_.identity` when `func` is nullish", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"should use internal `iteratee` if external is unavailable",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		var objects = [{ a: 0, b: 0 }, { a: 1, b: 0 }, { a: 1, b: 1 }];

		QUnit.test("`_.countBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.differenceBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"`_.dropRightWhile` should use `_.iteratee` internally",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("`_.dropWhile` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.every` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.filter` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.findIndex` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.findLast` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.findLastIndex` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.findKey` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.findLastKey` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.groupBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"`_.intersectionBy` should use `_.iteratee` internally",
			function(assert) {
				assert.expect(1);

				if (!isModularize) {
					_.iteratee = getPropA;
					assert.deepEqual(_.intersectionBy(objects, [objects[2]]), [
						objects[1]
					]);
					_.iteratee = iteratee;
				} else {
		...
		QUnit.test("`_.keyBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.mapKeys` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.mapValues` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.maxBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.meanBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.minBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.partition` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.pullAllBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.reduce` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.reduceRight` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.reject` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.remove` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.sortBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.sortedIndexBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"`_.sortedLastIndexBy` should use `_.iteratee` internally",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("`_.sumBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"`_.takeRightWhile` should use `_.iteratee` internally",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("`_.takeWhile` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
			...
			if (!isModularize) {
				_.iteratee = getPropB;
				assert.deepEqual(
					_.takeWhile(objects.reverse()),
					objects.reverse().slice(2)
				);
				_.iteratee = iteratee;
			} else {
				skipAssert(assert);
		...
		QUnit.test("`_.transform` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
			...
			if (!isModularize) {
				...
				assert.deepEqual(_.transform(objects, undefined, { sum: 0 }), {
					sum: 2
				});
				_.iteratee = iteratee;
			} else {
		...
		QUnit.test("`_.uniqBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.unionBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_.xorBy` should use `_.iteratee` internally", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return join all array elements into a string", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.take(_.filter(_.map(_.keyBy(array), square), isEven))
				);
			} else {
				skipAssert(assert);
			}
	...
	(function() {
		QUnit.test("should return `true` if `value` is less than `other`", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		QUnit.test("should return `true` if `value` is <= `other`", function(
			assert
		) {
			assert.expect(4);
	...
	lodashStable.each(
		...
		function(methodName) {
			...
			QUnit.test(
				"`_." + methodName + "` should accept a falsey `array` argument",
				function(assert) {
					assert.expect(1);
			...
			QUnit.test("`_." + methodName + "` should match `-0` as `0`", function(
				assert
			) {
				assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should map values in `collection` to a new array", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should accept a falsey `collection` argument", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should treat a nodelist as an array-like object", function(
			assert
		) {
			assert.expect(1);

			if (document) {
				var actual = _.map(document.getElementsByTagName("body"), function(
					element
				) {
	...
	(function() {
		...
		QUnit.test("should map values in `object` to a new object", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should use `_.identity` when `iteratee` is nullish", function(
			...
			var object = { a: 1, b: 2 },
				...
				expected = lodashStable.map(
					values,
					lodashStable.constant([true, false])
				);
	...
	lodashStable.each(["mapKeys", "mapValues"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should accept a falsey `object` argument",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["matches", "isMatch"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should perform a deep comparison between `source` and `object`",
			function(assert) {
				assert.expect(5);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should match inherited string keyed `object` properties",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should compare a variety of `source` property values",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." + methodName + "` should compare functions by reference",
			function(assert) {
				assert.expect(3);
		...
		QUnit.test("`_." + methodName + "` should partial match arrays", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should partial match arrays of objects", function(assert) {
			...
			var actual = lodashStable.filter(
				objects,
				matches({ a: [{ b: 1 }, { b: 4, c: 5 }] })
			);
			assert.deepEqual(actual, [objects[0]]);
		});

		QUnit.test("`_." + methodName + "` should partial match maps", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("`_." + methodName + "` should partial match sets", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test(
			"`_." + methodName + "` should match `undefined` values",
			function(assert) {
				assert.expect(3);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` hould return `true` when comparing an empty `source`",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return `false` when `object` is nullish", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should return `false` if parts of `path` are missing", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test(
			"should match inherited string keyed `srcValue` properties",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test("should not match by inherited `srcValue` properties", function(
			assert
		) {
			assert.expect(2);
			...
			lodashStable.each(["a", ["a"]], function(path) {
				assert.deepEqual(
					lodashStable.map(objects, _.matchesProperty(path, new Foo())),
					expected
				);
			});
		});
		...
		QUnit.test("should partial match arrays with duplicate values", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should partial match arrays of objects", function(assert) {
			...
			var actual = lodashStable.filter(
				objects,
				_.matchesProperty("a", [{ a: 1 }, { a: 4, b: 5 }])
			);
			assert.deepEqual(actual, [objects[0]]);
		});
		...
		QUnit.test("should match `undefined` values", function(assert) {
			...
			actual = lodashStable.map(
				objects,
				_.matchesProperty("a", { b: undefined })
			);

			assert.deepEqual(actual, expected);
		});

		QUnit.test("should match `undefined` values of nested objects", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should match `undefined` values on primitives", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should not change behavior if `srcValue` is modified", function(
			assert
		) {
			assert.expect(9);
	...
	(function() {
		QUnit.test("should return the largest value from a collection", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should return `undefined` for empty collections", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work with non-numeric collection values", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		QUnit.test("should return the mean of an array of numbers", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should use `this` binding of function for `resolver`", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should not error if `resolver` is falsey", function(assert) {
			...
			var actual = lodashStable.map(falsey, function(resolver, index) {
				try {
					return _.isFunction(
						index ? _.memoize(noop, resolver) : _.memoize(noop)
					);
				} catch (e) {}
			});
		...
		QUnit.test("should allow `_.memoize.Cache` to be customized", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should works with an immutable `_.memoize.Cache` ", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should not merge onto function values of sources", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should merge typed arrays", function(assert) {
			...
			var actual = lodashStable.map(typedArrays, function(type) {
				var Ctor = root[type];
				return Ctor
					? _.merge({ value: new Ctor(buffer) }, { value: [1] })
					: false;
			});
			...
			actual = lodashStable.map(typedArrays, function(type, index) {
				...
				array.push(1);
				return Ctor
					? _.merge({ value: array }, { value: new Ctor(buffer) })
					: false;
			});
		...
		QUnit.test(
			"should assign non array/typed-array/plain-object sources directly",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"should deep clone array/typed-array/plain-object sources",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should merge plain-objects onto non plain-objects", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test(
			"should skip `undefined` values in array sources if a destination value exists",
			function(assert) {
				assert.expect(2);
	...
	(function() {
		...
		QUnit.test(
			"should clone sources when `customizer` returns `undefined`",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"should overwrite primitives with source object clones",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return `undefined` when `object` is nullish", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			"should return `undefined` with deep paths when `object` is nullish",
			function(assert) {
				assert.expect(2);
	...
	(function() {
		QUnit.test("should return the smallest value from a collection", function(
			assert
		) {
			assert.expect(1);
	...
	lodashStable.each(["max", "maxBy", "min", "minBy"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should work with Date objects", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"`_." + methodName + "` should work with extremely large arrays",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should work when chaining on an array with only one value",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["maxBy", "minBy"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should work with an `iteratee` argument",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should not assign inherited `source` methods", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should accept an `options` argument", function(assert) {
			...
			lodashStable.each([_, Wrapper], function(func) {
				lodashStable.each([{ chain: false }, { chain: true }], function(
					options
				) {
					if (!isNpm) {
						if (func === _) {
		...
		QUnit.test("should not error for non-object `options` values", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should produce methods that work in a lazy sequence", function(
			assert
		) {
			assert.expect(1);
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.take(_.b(_.map(_.a(array), square), isEven))
				);
	...
	(function() {
		...
		QUnit.test(
			"should sort by a single property by a specified order",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"should sort by multiple properties by specified orders",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should use `_.identity` when a predicate is nullish", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work with a negative `n`", function(assert) {
			...
			var actual = lodashStable.map(
				lodashStable.range(1, args.length + 1),
				function(n) {
					var func = _.nthArg(-n);
					return func.apply(undefined, args);
				}
	...
	(function() {
		...
		QUnit.test("should work with a primitive `object` argument", function(
			assert
		) {
			assert.expect(1);
	...
	lodashStable.each(["omit", "omitBy"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should include symbol properties",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should create an object with omitted symbol properties",
			function(assert) {
				assert.expect(6);
		...
		QUnit.test(
			"`_." + methodName + "` should work with an array `object` argument",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			"should return `false` as soon as a predicate returns falsey",
			function(assert) {
				assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should truncate pad characters to fit the pad length", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test(
			"should parse strings with leading whitespace (test in Chrome and Firefox)",
			function(assert) {
				assert.expect(2);
	...
	lodashStable.each(["partial", "partialRight"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` partially applies arguments", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_." + methodName + "` should support placeholders", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should work with placeholders and curried functions", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		QUnit.test("should work as a deep `_.defaults`", function(assert) {
			...
			var defaultsDeep = _.partialRight(_.mergeWith, function deep(
				value,
				other
			) {
				return lodashStable.isObject(value)
					? _.mergeWith(value, other, deep)
					: value;
			});
	...
	(function() {
		...
		QUnit.test("should work with combinations of partial functions", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should ensure `new combo` is an instance of `func`", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should work with recursively bound functions", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work when hot", function(assert) {
			...
			lodashStable.each(["partial", "partialRight"], function(
				methodName,
				index
			) {
				var func = _[methodName],
	...
	(function() {
		...
		QUnit.test("should split elements into two groups by `predicate`", function(
			assert
		) {
			assert.expect(3);
	...
	lodashStable.each(["pick", "pickBy"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should pick symbol properties", function(
			assert
		) {
			assert.expect(2);
	...
	lodashStable.each(["pull", "pullAll", "pullAllWith"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should modify and return the array",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." + methodName + "` should preserve holes in arrays",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." + methodName + "` should treat holes as `undefined`",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		QUnit.test("should modify the array and return removed elements", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should use `undefined` for nonexistent indexes", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			"should return an empty array when no indexes are given",
			function(assert) {
				assert.expect(4);
		...
		QUnit.test(
			"should work with a falsey `array` argument when keys are given",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should return `0` or `1` when no arguments are given", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should support not providing a `max` argument", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should swap `min` and `max` when `min` > `max`", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should support providing a `floating` argument", function(
			assert
		) {
			assert.expect(3);
	...
	lodashStable.each(["range", "rangeRight"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should support a `step` of `0`",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." + methodName + "` should support `start` of `-0`",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." + methodName + "` should treat falsey `start` arguments as `0`",
			function(assert) {
				assert.expect(13);
	...
	(function() {
		...
		QUnit.test(
			"should not rearrange arguments when no indexes are given",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test("should work with fewer indexes than arguments", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work on functions that have been rearged", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			...
			function(assert) {
				...
				var expected =
					firstKey == "a" ? [0, 1, "a", object] : [0, 2, "b", object];
	...
	lodashStable.each(["reduce", "reduceRight"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should support empty collections with an initial `accumulator` value",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should handle an initial `accumulator` value of `undefined`",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["filter", "reject"], function(methodName) {
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.deepEqual(func(objects, objects[1]), [
	...
	(function() {
		...
		QUnit.test("should return an empty string if `n` is <= `0`", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("should coerce `string` to a string", function(assert) {
			...
			assert.strictEqual(
				_.repeat({ toString: lodashStable.constant("*") }, 3),
				"***"
			);
		});
	...
	lodashStable.each(["get", "result"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should get a key over a path", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test("`_." + methodName + "` should handle empty paths", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("`_." + methodName + "` should handle complex paths", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			...
			function(assert) {
				...
				var actual = lodashStable.transform(values, function(result, value) {
					lodashStable.each(["a.b", ["a", "b"]], function(path) {
						result.push(func(object, path, value), func(null, path, value));
					});
				});
	...
	(function() {
		...
		QUnit.test("should track the `__chain__` value of a wrapper", function(
			assert
		) {
			assert.expect(6);
	...
	(function() {
		...
		QUnit.test("should return an empty array when `n` < `1` or `NaN`", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should return all elements when `n` >= `length`", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("should return an empty array for empty collections", function(
			assert
		) {
			assert.expect(1);
	...
	lodashStable.each(["update", "updateWith", "set", "setWith"], function(
		methodName
	) {
		var func = _[methodName],
		...
		QUnit.test("`_." + methodName + "` should set property values", function(
			assert
		) {
			assert.expect(4);
		...
		QUnit.test("`_." + methodName + "` should set a key over a path", function(
			assert
		) {
			assert.expect(4);
	...
	(function() {
		...
		QUnit.test("should shuffle small collections", function(assert) {
			...
			assert.deepEqual(
				lodashStable.sortBy(lodashStable.uniqBy(actual, String), "0"),
				[[1, 2], [2, 1]]
			);
		});
	...
	(function() {
		...
		QUnit.test("should sort in ascending order by `iteratee`", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should work with `_.property` shorthands", function(assert) {
			...
			var actual = lodashStable.map(
				_.sortBy(objects.concat(undefined), "b"),
				"b"
			);
		...
		QUnit.test("should coerce arrays returned from `iteratee`", function(
			assert
		) {
			assert.expect(1);
	...
	lodashStable.each(["orderBy", "sortBy"], function(methodName) {
		...
		var stableObject = lodashStable.zipObject(
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should sort multiple properties in ascending order",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("`_." + methodName + "` should support iteratees", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should perform a stable sort (test in IE > 8 and V8)",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." + methodName + "` should not error on nullish elements",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["sortedIndex", "sortedLastIndex"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should accept a falsey `array` argument and a `value`",
			function(assert) {
				assert.expect(1);

				var expected = lodashStable.map(
					falsey,
					lodashStable.constant([0, 0, 0])
				);
		...
		QUnit.test("`_." + methodName + "` should align with `_.sortBy`", function(
			assert
		) {
			assert.expect(12);
	...
	lodashStable.each(["sortedIndexBy", "sortedLastIndexBy"], function(
		methodName
	) {
		var func = _[methodName],
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should support arrays larger than `MAX_ARRAY_LENGTH / 2`",
			function(assert) {
				assert.expect(12);

				lodashStable.each(
					[Math.ceil(MAX_ARRAY_LENGTH / 2), MAX_ARRAY_LENGTH],
					function(length) {
						var array = [],
						...
						lodashStable.each(values, function(value) {
							...
							var expected = (isSortedIndexBy
							? !lodashStable.isNaN(value)
							: lodashStable.isFinite(value))
								? 0
	...
	lodashStable.each(["sortedIndexOf", "sortedLastIndexOf"], function(
		methodName
	) {
		var func = _[methodName],
	...
	(function() {
		QUnit.test("should return unique values of a sorted array", function(
			assert
		) {
			assert.expect(3);
	...
	(function() {
		...
		QUnit.test("should treat falsey `position` values as `0`", function(
			assert
		) {
			assert.expect(1);
	...
	lodashStable.each(
		...
		function(methodName) {
			...
			QUnit.test("`_." + methodName + "` should return " + pair[1], function(
				assert
			) {
				assert.expect(1);
	...
	lodashStable.each(["add", "divide", "multiply", "subtract"], function(
		methodName
	) {
		var func = _[methodName],
		...
		QUnit.test(
			"`_." + methodName + "` should convert objects to `NaN`",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			"`_." + methodName + "` should convert symbols to `NaN`",
			function(assert) {
				assert.expect(2);
	...
	lodashStable.each(["sum", "sumBy"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should return `0` when passing empty `array` values",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("`_." + methodName + "` should not skip `NaN` values", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.takeRight(_.take(_.takeRight(_.take(array, 6), 4), 2))
				);
				...
				assert.deepEqual(
					...
					_.takeRight(
						_.take(
							_.takeRight(
								_.take(_.filter(_.take(array, array.length - 1), predicate), 6),
								4
							),
							2
						)
	...
	(function() {
		...
		QUnit.test("should work in a lazy sequence", function(assert) {
			...
			if (!isNpm) {
				...
				assert.deepEqual(
					actual,
					_.takeRight(_.takeRight(_.filter(array, predicate), 2))
				);
				...
				assert.deepEqual(
					actual,
					_.take(_.takeRight(_.take(_.takeRight(array, 6), 4), 2))
				);
				...
				assert.deepEqual(
					...
					_.take(
						_.takeRight(
							_.take(_.takeRight(_.filter(array, predicate), 6), 4),
							2
						)
					)
				);
	...
	(function() {
		...
		QUnit.test('should evaluate JavaScript in "evaluate" delimiters', function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test('should support complex "interpolate" delimiters', function(
			assert
		) {
			assert.expect(22);
		...
		QUnit.test("should work with statements containing quotes", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"should not error with IE conditional comments enabled (test with development build)",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should match delimiters before escaping text", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should resolve nullish values to an empty string", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should expose the source on compiled templates", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should not truncate if `string` is <= `length`", function(
			assert
		) {
			assert.expect(2);
			...
			assert.strictEqual(
				_.truncate(string, { length: string.length + 2 }),
				string
			);
		});
		...
		QUnit.test("should coerce nullish `omission` values to strings", function(
			assert
		) {
			assert.expect(2);

			assert.strictEqual(
				_.truncate(string, { omission: null }),
	...
	(function() {
		...
		QUnit.test("should not trigger a trailing call when invoked once", function(
			assert
		) {
			assert.expect(2);
		...
		QUnit.test(
			"should trigger a second throttled call as soon as possible",
			function(assert) {
				assert.expect(3);
	...
	lodashStable.each(["debounce", "throttle"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` supports recursive calls", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test(
			"should return an empty array for falsey and negative `n` arguments",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["slice", "toArray"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should return a dense array", function(
			assert
		) {
			assert.expect(3);
	...
	lodashStable.each(["toPairs", "toPairsIn"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should convert strings", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should coerce array elements to strings", function(assert) {
			...
			lodashStable.each([array, lodashStable.map(array, Object)], function(
				value
			) {
		...
		QUnit.test("should not coerce symbols to strings", function(assert) {
			...
			if (Symbol) {
				var object = Object(symbol);
				lodashStable.each([symbol, object, [symbol], [object]], function(
					value
				) {
					var actual = _.toPath(value);
		...
		QUnit.test("should handle consecutive empty brackets and dots", function(
			assert
		) {
			assert.expect(12);
	...
	(function() {
		...
		QUnit.test("should flatten inherited string keyed properties", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should convert `arguments` objects to plain objects", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		QUnit.test("should treat nullish values as empty strings", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			...
			function(assert) {
				...
				expected = lodashStable.map(
					accumulators,
					lodashStable.constant({ a: 1, b: 4, c: 9 })
				);
		...
		QUnit.test("should create regular arrays from typed arrays", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"should create an empty object when given a falsey `object` argument",
			function(assert) {
				assert.expect(1);
		...
		lodashStable.each(
			...
			function(object, key) {
				QUnit.test(
					"should provide correct `iteratee` arguments when transforming an " +
						key,
					function(assert) {
						assert.expect(2);
		...
		QUnit.test(
			"should create an object from the same realm as `object`",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["trim", "trimStart", "trimEnd"], function(
		methodName,
		index
	) {
		var func = _[methodName],
	...
	(function() {
		...
		QUnit.test("should account for combining diacritical marks", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should handle strings with nothing to unescape", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			"should unescape the same characters escaped by `_.escape`",
			function(assert) {
				assert.expect(1);
	...
	lodashStable.each(["union", "unionBy", "unionWith"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should return the union of two arrays",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." + methodName + "` should not flatten nested arrays",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should output values from the first possible array", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should preserve the sign of `0`", function(assert) {
			...
			var actual = lodashStable.map(arrays, function(array) {
				return lodashStable.map(
					_.uniqWith(array, lodashStable.eq),
					lodashStable.toString
				);
			});
	...
	(function() {
		...
		QUnit.test(
			"should return a string value when not providing a prefix argument",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test("should coerce the prefix argument to a string", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test(
			...
			function(assert) {
				...
				var array = [[1, 3], [2, 4]],
					...
					expected = lodashStable.map(
						values,
						lodashStable.constant(_.unzip(array))
					);
	...
	(function() {
		QUnit.test("should work with a `customizer` callback", function(assert) {
			...
			var actual = _.updateWith({ "0": {} }, "[0][1][2]", stubThree, function(
				value
			) {
				return lodashStable.isObject(value) ? undefined : {};
			});
	...
	lodashStable.each(["update", "updateWith"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should invoke `updater` with the value on `path` of `object`",
			function(assert) {
				assert.expect(4);
	...
	(function() {
		QUnit.test("should treat latin-1 supplementary letters as words", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("should not treat mathematical operators as words", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should use `_.identity` when `wrapper` is nullish", function(
			assert
		) {
			assert.expect(1);
	...
	lodashStable.each(["zipObject", "zipObjectDeep"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should ignore extra `values`", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		QUnit.test(
			"should zip arrays combining grouped elements with `iteratee`",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test(
			...
			function(assert) {
				...
				var array1 = [1, 2],
					...
					expected = lodashStable.map(
						values,
						lodashStable.constant(_.zip(array1, array2))
					);

				var actual = lodashStable.map(values, function(value, index) {
					return index
						? _.zipWith(array1, array2, value)
						: _.zipWith(array1, array2);
				});
	...
	lodashStable.each(["unzip", "zip"], function(methodName, index) {
		...
		lodashStable.forOwn(object, function(pair, key) {
			QUnit.test("`_." + methodName + "` should work with " + key, function(
				assert
			) {
				assert.expect(2);
	...
	lodashStable.each([false, true], function(implict) {
		...
		QUnit.test("should follow the iterator protocol " + chainType, function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should reset the iterator correctly " + chainType, function(
			assert
		) {
			assert.expect(4);
	...
	(function() {
		...
		QUnit.test("should reset iterator data on cloned sequences", function(
			assert
		) {
			assert.expect(3);
	...
	(function() {
		QUnit.test("should remove elements from the end of `array`", function(
			assert
		) {
			assert.expect(5);
	...
	(function() {
		QUnit.test("should remove elements from the front of `array`", function(
			assert
		) {
			assert.expect(5);
	...
	(function() {
		QUnit.test("should support removing and inserting elements", function(
			assert
		) {
			assert.expect(5);
	...
	(function() {
		...
		lodashStable.each(funcs, function(methodName) {
			QUnit.test(
				"`_(...)." + methodName + "` should return a new wrapper",
				function(assert) {
					assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should work with `arguments` objects", function(assert) {
			...
			assert.deepEqual(
				_.dropWhile(args, identity),
				[null, [3], null, 5],
				message("dropWhile")
			);
			assert.deepEqual(_.findIndex(args, identity), 0, message("findIndex"));
			assert.deepEqual(
				_.findLastIndex(args, identity),
				4,
				message("findLastIndex")
			);
			assert.deepEqual(
				_.flatten(args),
				[1, null, 3, null, 5],
			...
			assert.deepEqual(
				_.sortedIndexOf(sortedArgs, 5),
				2,
				message("sortedIndexOf")
			);
			assert.deepEqual(
				_.sortedLastIndex(sortedArgs, 5),
				3,
				message("sortedLastIndex")
			);
			assert.deepEqual(
				_.sortedLastIndexOf(sortedArgs, 1),
				0,
				message("sortedLastIndexOf")
			);
			...
			assert.deepEqual(
				_.takeRightWhile(args, identity),
				[5],
				message("takeRightWhile")
			);
			...
			assert.deepEqual(
				_.zip(args, args),
				[[1, 1], [null, null], [[3], [3]], [null, null], [5, 5]],
				message("zip")
			);
		});
		...
		QUnit.test("should accept falsey secondary arguments", function(assert) {
			...
			function message(methodName) {
				return (
					"`_." + methodName + "` should accept falsey secondary arguments"
				);
			}

			assert.deepEqual(_.difference(array, null), array, message("difference"));
			assert.deepEqual(
				_.intersection(array, null),
				[],
				message("intersection")
			);
	...
	(function() {
		var allMethods = lodashStable.reject(_.functions(_).sort(), function(
			methodName
		) {
		...
		QUnit.test("should accept falsey arguments", function(assert) {
			...
			lodashStable.each(["chain", "noConflict", "runInContext"], function(
				methodName
			) {
				if (!_[methodName]) {
					skipAssert(assert);
			...
			function(assert) {
				...
				assert.ok(
					lodashStable.every(_.functions(_), function(methodName) {
						return (
							methodName.length > 2 ||
							lodashStable.includes(shortNames, methodName)
						);
					})
				);
			}
