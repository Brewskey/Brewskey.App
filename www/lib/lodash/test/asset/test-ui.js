(function(window) {
	...
	var build =
		(build = /build=([^&]+)/.exec(location.search)) &&
		decodeURIComponent(build[1]);

	/** The module loader to use. */
	var loader =
		(loader = /loader=([^&]+)/.exec(location.search)) &&
		decodeURIComponent(loader[1]);
	...
	addEventListener("load", function() {
		function eventHandler(event) {
			...
			location.href =
				...
				(search ? search + "&" : "") +
				"build=" +
				(buildIndex < 0 ? build : buildList[buildIndex].value) +
				"&" +
				"loader=" +
				(loaderIndex < 0 ? loader : loaderList[loaderIndex].value);
		}
		...
		span1.style.cssText = span2.style.cssText =
			"display:inline-block;float:right;line-height:2.1em;margin-left:1em;margin-top:0;";

		span1.firstChild.style.cssText = span2.firstChild.style.cssText =
	...
	ui.isModularize = /\b(?:amd|commonjs|es|node|npm|(index|main)\.js)\b/.test([
		location.pathname,
		location.search
	]);
