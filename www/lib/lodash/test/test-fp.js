(function() {
   ...
   */
	...
	(function() {
		...
		QUnit.test("should work with `name`, `func`, and `options`", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should work with `runInContext` and `options`", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("should only add a `placeholder` property if needed", function(
			assert
		) {
			assert.expect(2);
	...
	_.each(["fp.convert", "method.convert"], function(methodName) {
		...
		QUnit.test("`" + methodName + "` should work with an object", function(
			assert
		) {
			assert.expect(3);
		...
		QUnit.test("`" + methodName + "` should extend existing configs", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		...
		QUnit.test("should have >= arity of `aryMethod` designation", function(
			assert
		) {
			assert.expect(4);
			...
			_.times(4, function(index) {
				...
				var methodNames = _.filter(mapping.aryMethod[aryCap], function(
					methodName
				) {
	...
	(function() {
		QUnit.test("should have correct aliases", function(assert) {
			...
			var actual = _.transform(
				mapping.aliasToReal,
				function(result, realName, alias) {
	...
	(function() {
		QUnit.test("should have a cap of 1", function(assert) {
			...
			var actual = _.map(mapping.aryMethod[1], function(methodName) {
				...
				if (
					_.includes(exceptions, methodName)
		...
		QUnit.test("should have a cap of 2", function(assert) {
			...
			var exceptions = _.without(
					funcMethods.concat("matchesProperty"),
					"delay"
				),
				expected = _.map(mapping.aryMethod[2], _.constant(true));

			var actual = _.map(mapping.aryMethod[2], function(methodName) {
				var args = _.includes(funcMethods, methodName)
						? [methodName == "curryN" ? 1 : _.noop, _.noop]
						: [1, []],
					result = _.attempt(function() {
	...
	(function() {
		QUnit.test("should provide the correct `iteratee` arguments", function(
			assert
		) {
			assert.expect(4);
	...
	(function() {
		...
		_.forOwn(mapping.placeholder, function(truthy, methodName) {
			...
			QUnit.test(
				"`_." + methodName + "` should have a `placeholder` property",
				function(assert) {
					assert.expect(2);
	...
	_.each(["add", "subtract"], function(methodName) {
		...
		QUnit.test(
			"`fp." + methodName + "` should not have `rearg` applied",
			function(assert) {
				assert.expect(1);
	...
	_.each(["assign", "assignIn", "defaults", "defaultsDeep", "merge"], function(
		methodName
	) {
		var func = fp[methodName];

		QUnit.test("`fp." + methodName + "` should not mutate values", function(
			assert
		) {
			assert.expect(2);
	...
	_.each(
		["assignAll", "assignInAll", "defaultsAll", "defaultsDeepAll", "mergeAll"],
		function(methodName) {
			var func = fp[methodName];
	...
	_.each(["assignWith", "assignInWith", "extendWith"], function(methodName) {
		...
		QUnit.test(
			"`fp." +
				methodName +
				"` should provide the correct `customizer` arguments",
			function(assert) {
				assert.expect(1);
	...
	_.each(["assignAllWith", "assignInAllWith", "extendAllWith"], function(
		methodName
	) {
		var func = fp[methodName];
	...
	_.each(["curry", "curryRight"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should only accept a `func` param",
			function(assert) {
				assert.expect(1);
	...
	_.each(["curryN", "curryRightN"], function(methodName) {
		...
		QUnit.test(
			"`_." + methodName + "` should accept an `arity` param",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		QUnit.test(
			"should return the elements of the first array not included in the second array",
			function(assert) {
				assert.expect(1);
	...
	_.each(
		["findFrom", "findIndexFrom", "findLastFrom", "findLastIndexFrom"],
		function(methodName) {
			var func = fp[methodName];
			...
			QUnit.test(
				"`_." +
					methodName +
					"` should provide the correct `predicate` arguments",
				function(assert) {
					assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			...
			function(assert) {
				...
				assert.strictEqual(
					fp.findLastFrom(resolve(2))(-2)(objects),
					objects[1]
				);
			}
		);
	...
	(function() {
		QUnit.test(
			"should have an argument order of `iteratee`, `depth`, then `collection`",
			function(assert) {
				assert.expect(2);
	...
	_.each(["gt", "gte"], function(methodName) {
		...
		QUnit.test("`fp." + methodName + "` should have `rearg` applied", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		QUnit.test("should provide the correct `customizer` arguments", function(
			assert
		) {
			assert.expect(1);
			...
			var args,
				...
				objects = [{ a: 1 }, { a: 2 }],
				stack = {
					__data__: { __data__: [objects, objects.slice().reverse()] }
				},
	...
	_.each(["maxBy", "minBy"], function(methodName) {
		...
		QUnit.test(
			"`fp." + methodName + "` should work with an `iteratee` argument",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`fp." + methodName + "` should provide the correct `iteratee` arguments",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test(
			"should mixin static methods but not prototype methods",
			function(assert) {
				assert.expect(2);
		...
		QUnit.test("should not remove existing prototype methods", function(
			assert
		) {
			assert.expect(2);
	...
	_.each(["partial", "partialRight"], function(methodName) {
		...
		QUnit.test("`_." + methodName + "` should accept an `args` param", function(
			assert
		) {
			assert.expect(1);
		...
		QUnit.test("`_." + methodName + "` should convert by name", function(
			assert
		) {
			assert.expect(2);
	...
	(function() {
		QUnit.test("should have an argument order of `start` then `end`", function(
			assert
		) {
			assert.expect(1);
	...
	_.each(["reduce", "reduceRight"], function(methodName) {
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should provide the correct `iteratee` arguments when iterating an array",
			function(assert) {
				assert.expect(1);
		...
		QUnit.test(
			"`_." +
				methodName +
				"` should provide the correct `iteratee` arguments when iterating an object",
			function(assert) {
				assert.expect(1);
	...
	_.each(["trimChars", "trimCharsStart", "trimCharsEnd"], function(
		methodName,
		index
	) {
		var func = fp[methodName],
	...
	(function() {
		QUnit.test(
			"should have an argument order of `iteratee`, `array`, then `other`",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		...
		QUnit.test("should provide the correct `comparator` arguments", function(
			assert
		) {
			assert.expect(1);
	...
	(function() {
		QUnit.test("should not convert end of `path` to an object", function(
			assert
		) {
			assert.expect(1);
	...
	_.each(["differenceWith", "intersectionWith", "xorWith"], function(
		methodName
	) {
		var func = fp[methodName];
		...
		QUnit.test(
			"`fp." +
				methodName +
				"` should provide the correct `comparator` arguments",
			function(assert) {
				assert.expect(1);
	...
	(function() {
		QUnit.test("should zip together an array of arrays", function(assert) {
			...
			assert.deepEqual(fp.zipAll([[1, 2], [3, 4], [5, 6]]), [
				[1, 3, 5],
				[2, 4, 6]
			]);
		});
	...
	(function() {
		QUnit.test("should zip together key/value arrays into an object", function(
			assert
		) {
			assert.expect(1);
