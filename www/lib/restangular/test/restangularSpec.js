describe("Restangular", function() {
	...
	beforeEach(
		inject(function($injector) {
			...
			accountsHalModel = [
				{
					...
					amount: 42,
					transaction: [],
					_links: { self: "/accountsHAL/martin" }
				},
				{
					id: 1,
					user: "Paul",
					...
					transaction: [
						{
							...
							amount: 3,
							id: 0,
							_links: { self: "/accountsHAL/paul/transactions/0" }
						},
						{
							from: "Anonymous",
							amount: 0.1416,
							id: 1,
							_links: { self: "/accountsHAL/paul/transactions/1" }
						}
					],
			...
			$httpBackend
				.whenGET("/accounts/1/transactions")
				.respond(accountsModel[1].transactions);
			$httpBackend
				.whenGET("/accounts/1/transactions/1")
				.respond(accountsModel[1].transactions[1]);
			...
			$httpBackend
				.whenPOST("/accounts/1/transactions")
				.respond(function(method, url, data, headers) {
					return [201, "", ""];
				});

			$httpBackend
				.whenDELETE("/accounts/1/transactions/1")
				.respond(function(method, url, data, headers) {
					return [200, "", ""];
				});

			$httpBackend
				.whenDELETE("/accounts/1")
				.respond(function(method, url, data, headers) {
					return [200, "", ""];
				});
	...
	describe("ALL", function() {
		it("getList() should return an array of items", function() {
			restangularAccounts.getList().then(function(accounts) {
				expect(Restangular.stripRestangular(accounts)).toEqual(
					Restangular.stripRestangular(accountsModel)
				);
			});
		...
		it("several getList() should return an array of items", function() {
			...
			Restangular.several("accounts", 0, 1)
				.getList()
		...
		it("get(id) should return the item with given id", function() {
			restangularAccounts.get(0).then(function(account) {
				expect(Restangular.stripRestangular(account)).toEqual(
					Restangular.stripRestangular(accountsModel[0])
				);
			});
		...
		it("Custom GET methods should work", function() {
			restangularAccounts.customGETLIST("messages").then(function(msgs) {
				expect(Restangular.stripRestangular(msgs)).toEqual(
					Restangular.stripRestangular(messages)
				);
			});
	...
	describe("Scoped Service", function() {
		...
		it("should correctly work with children", function() {
			var Transactions = Restangular.service(
				"transactions",
				restangularAccount1
			);
	...
	describe("ONE", function() {
		it("get() should return a JSON item", function() {
			restangularAccount1.get().then(function(account) {
				expect(Restangular.stripRestangular(account)).toEqual(
					Restangular.stripRestangular(accountsModel[1])
				);
			});
		...
		it("Should make RequestLess connections with one", function() {
			restangularAccount1
				...
				.get()
				.then(function(transaction) {
					expect(Restangular.stripRestangular(transaction)).toEqual(
						Restangular.stripRestangular(accountsModel[1].transactions[1])
					);
				});
		...
		it("Should make RequestLess connections with all", function() {
			restangularAccount1
				...
				.then(function(transactions) {
					expect(Restangular.stripRestangular(transactions)).toEqual(
						Restangular.stripRestangular(accountsModel[1].transactions)
					);
				});
		...
		it("Custom GET methods should work", function() {
			restangularAccount1.customGET("message").then(function(msg) {
				expect(Restangular.stripRestangular(msg)).toEqual(
					Restangular.stripRestangular(messages[0])
				);
			});
	...
	describe("getRestangularUrl", function() {
		it("should return the generated URL when you chain Restangular methods together", function() {
			var restangularSpaces = Restangular.one("accounts", 123)
				.one("buildings", 456)
				.all("spaces");
			expect(restangularSpaces.getRestangularUrl()).toEqual(
				"/accounts/123/buildings/456/spaces"
			);
		});
	});
	...
	describe("getRestangularUrl with useCannonicalId set to true", function() {
		it("should return the generated URL when you chain Restangular methods together", function() {
			...
			var restangularSpaces = R.one("accounts", 123)
				.one("buildings", 456)
				.all("spaces");
			expect(restangularSpaces.getRestangularUrl()).toEqual(
				"/accounts/123/buildings/456/spaces"
	...
	describe("withConfig", function() {
		it("should create new service with scoped configuration", function() {
			var childRestangular = Restangular.withConfig(function(
				RestangularConfigurer
			) {
		...
		it("should allow nested configurations", function() {
			...
			var grandchildRestangular = childRestangular.withConfig(function(
				RestangularConfigurer
			) {
				RestangularConfigurer.setRequestSuffix(".json");
			});
	...
	describe("Self linking", function() {
		it("Should request the link in HAL format", function() {
			var linkRestangular = Restangular.withConfig(function(
				RestangularConfigurer
			) {
