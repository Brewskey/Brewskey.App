...
(function() {
	...
	angular
		.module("ngCordova.plugins.actionSheet", [])
		.factory("$cordovaActionSheet", [
			"$q",
			"$window",
			function($q, $window) {
				return {
	...
	angular.module("ngCordova.plugins.adMob", []).factory("$cordovaAdMob", [
		"$q",
		"$window",
		function($q, $window) {
			return {
	...
	angular
		.module("ngCordova.plugins.appAvailability", [])
		.factory("$cordovaAppAvailability", [
			"$q",
			function($q) {
				return {
	...
	angular.module("ngCordova.plugins.appRate", []).provider("$cordovaAppRate", [
		function() {
			this.setPreferences = function(defaults) {
				...
				AppRate.preferences.promptAgainForEachNewVersion =
					defaults.promptForNewVersion || true;
				...
				AppRate.preferences.useCustomRateDialog =
					defaults.useCustomRateDialog || false;
	...
	angular
		.module("ngCordova.plugins.appVersion", [])
		.factory("$cordovaAppVersion", [
			"$q",
			function($q) {
				return {
	...
	angular
		.module("ngCordova.plugins.backgroundGeolocation", [])
		.factory("$cordovaBackgroundGeolocation", [
			"$q",
			"$window",
			function($q, $window) {
				return {
	...
	angular.module("ngCordova.plugins.badge", []).factory("$cordovaBadge", [
		"$q",
		function($q) {
			return {
	...
	angular
		.module("ngCordova.plugins.barcodeScanner", [])
	...
	angular
		.module("ngCordova.plugins.batteryStatus", [])
		.factory("$cordovaBatteryStatus", [
			"$rootScope",
			"$window",
			"$timeout",
	...
	angular.module("ngCordova.plugins.ble", []).factory("$cordovaBLE", [
		"$q",
		function($q) {
			return {
	...
	angular
		.module("ngCordova.plugins.bluetoothSerial", [])
		.factory("$cordovaBluetoothSerial", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					readRSSI: function() {
						var q = $q.defer();
						$window.bluetoothSerial.readRSSI(
							function(data) {
								q.resolve(data);
							},
							function(error) {
	...
	angular
		.module("ngCordova.plugins.brightness", [])
		.factory("$cordovaBrightness", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					set: function(data) {
						...
						$window.cordova.plugins.brightness.setBrightness(
							data,
							function(result) {
								q.resolve(result);
							},
					...
					setKeepScreenOn: function(bool) {
						...
						$window.cordova.plugins.brightness.setKeepScreenOn(
							bool,
							function(result) {
								q.resolve(result);
							},
	...
	angular.module("ngCordova.plugins.calendar", []).factory("$cordovaCalendar", [
		"$q",
		"$window",
		function($q, $window) {
			return {
	...
	angular.module("ngCordova.plugins.camera", []).factory("$cordovaCamera", [
		"$q",
		function($q) {
			return {
	...
	angular.module("ngCordova.plugins.capture", []).factory("$cordovaCapture", [
		"$q",
		function($q) {
			return {
	...
	angular
		.module("ngCordova.plugins.clipboard", [])
		.factory("$cordovaClipboard", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					paste: function() {
						...
						$window.cordova.plugins.clipboard.paste(
							function(text) {
								q.resolve(text);
							},
							function() {
	...
	angular.module("ngCordova.plugins.contacts", []).factory("$cordovaContacts", [
		"$q",
		function($q) {
			return {
	...
	angular
		.module("ngCordova.plugins.datePicker", [])
		.factory("$cordovaDatePicker", [
			"$window",
			"$q",
			function($window, $q) {
				return {
	...
	angular
		.module("ngCordova.plugins.deviceMotion", [])
		.factory("$cordovaDeviceMotion", [
			"$q",
			function($q) {
				return {
	...
	angular
		.module("ngCordova.plugins.emailComposer", [])
		.factory("$cordovaEmailComposer", [
			"$q",
			function($q) {
				return {
	...
	angular
		.module("ngCordova.plugins.facebook", [])
	...
	angular
		.module("ngCordova.plugins.facebookAds", [])
		.factory("$cordovaFacebookAds", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					showBannerAtXY: function(x, y) {
						...
						$window.FacebookAds.showBannerAtXY(
							x,
							y,
							function() {
								d.resolve();
					...
					prepareInterstitial: function(options) {
						...
						$window.FacebookAds.prepareInterstitial(
							options,
							function() {
								d.resolve();
							},
					...
					showInterstitial: function() {
						...
						$window.FacebookAds.showInterstitial(
							function() {
								d.resolve();
							},
							function() {
	...
	angular.module("ngCordova.plugins.file", []).factory("$cordovaFile", [
		"$q",
		"$window",
		"$log",
		"$timeout",
		function($q, $window, $log, $timeout) {
			return {
				...
				listDir: function(filePath) {
					...
					getDirectory(filePath, { create: false }).then(
						function(parent) {
				...
				readFile: function(filePath) {
					/// now deprecated in new ng-cordova version
					$log.log(
						"readFile is now deprecated as of v0.1.4-alpha, use readAsText instead"
					);
				...
				downloadFile: function(source, filePath, options, trustAllHosts) {
					console.warn(
						"This method is deprecated as of v0.1.11-alpha, please refer to $cordovaFileTransfer"
					);
					var q = $q.defer();
     ...
     */
			function getPromisedFileReader(deferred) {
				...
				reader.onloadend = function() {
					if (this.error) deferred.reject(this.error);
					else deferred.resolve(this.result);
				};
				return reader;
			}
	...
	angular
		.module("ngCordova.plugins.fileOpener2", [])
		.factory("$cordovaFileOpener2", [
			"$q",
			function($q) {
				return {
	...
	angular
		.module("ngCordova.plugins.fileTransfer", [])
			...
			function($q, $timeout) {
				return {
					download: function(source, filePath, options, trustAllHosts) {
						...
						var ft = new FileTransfer();
						var uri =
							options && options.encodeURI === false
								? source
								: encodeURI(source);
	...
	angular
		.module("ngCordova.plugins.flashlight", [])
		.factory("$cordovaFlashlight", [
			"$q",
			"$window",
			function($q, $window) {
				return {
	...
	angular
		.module("ngCordova.plugins.flurryAds", [])
		.factory("$cordovaFlurryAds", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					showBannerAtXY: function(x, y) {
						...
						$window.FlurryAds.showBannerAtXY(
							x,
							y,
							function() {
								d.resolve();
					...
					prepareInterstitial: function(options) {
						...
						$window.FlurryAds.prepareInterstitial(
							options,
							function() {
								d.resolve();
							},
					...
					showInterstitial: function() {
						...
						$window.FlurryAds.showInterstitial(
							function() {
								d.resolve();
							},
							function() {
	...
	angular.module("ngCordova.plugins.ga", []).factory("$cordovaGA", [
		"$q",
		"$window",
		function($q, $window) {
			return {
	...
	angular
		.module("ngCordova.plugins.geolocation", [])
	...
	angular
		.module("ngCordova.plugins.globalization", [])
		.factory("$cordovaGlobalization", [
			"$q",
			function($q) {
				return {
	...
	angular
		.module("ngCordova.plugins.googleAds", [])
		.factory("$cordovaGoogleAds", [
			"$q",
			"$window",
			function($q, $window) {
				return {
	...
	angular
		.module("ngCordova.plugins.googleAnalytics", [])
		.factory("$cordovaGoogleAnalytics", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					addTransaction: function(
						...
						$window.analytics.addTransaction(
							transactionId,
							affiliation,
							revenue,
							tax,
	...
	angular
		.module("ngCordova.plugins.googleMap", [])
		.factory("$cordovaGoogleMap", [
			"$q",
			"$window",
			function($q, $window) {
				var map = null;
	...
	angular
		.module("ngCordova.plugins.healthKit", [])
		.factory("$cordovaHealthKit", [
			"$q",
			"$window",
			function($q, $window) {
				return {
	...
	angular.module("ngCordova.plugins.httpd", []).factory("$cordovaHttpd", [
		"$q",
		function($q) {
			return {
				...
				getLocalPath: function() {
					...
					cordova.plugins.CorHttpd.getLocalPath(
						function(path) {
							d.resolve(path);
						},
						function() {
	...
	angular.module("ngCordova.plugins.iAd", []).factory("$cordovaiAd", [
		"$q",
		"$window",
		function($q, $window) {
			return {
				...
				prepareInterstitial: function(options) {
					...
					$window.iAd.prepareInterstitial(
						options,
						function() {
							d.resolve();
						},
	...
	angular
		.module("ngCordova.plugins.imagePicker", [])
		.factory("$cordovaImagePicker", [
			"$q",
			"$window",
			function($q, $window) {
				return {
	...
	angular
		.module("ngCordova.plugins.inAppBrowser", [])
	...
	angular.module("ngCordova.plugins.keyboard", []).factory("$cordovaKeyboard", [
		function() {
			return {
	...
	angular.module("ngCordova.plugins.keychain", []).factory("$cordovaKeychain", [
		"$q",
		"$window",
		function($q, $window) {
	...
	angular
		.module("ngCordova.plugins.localNotification", [])
		.factory("$cordovaLocalNotification", [
			"$q",
			"$window",
			"$rootScope",
			"$timeout",
			function($q, $window, $rootScope, $timeout) {
				if ($window.plugin && $window.plugin.notification) {
					$window.plugin.notification.local.oncancel = function(
						...
						$timeout(function() {
							$rootScope.$broadcast(
								"$cordovaLocalNotification:canceled",
								notification
							);
						});
					};
					...
					$window.plugin.notification.local.ontrigger = function(
						id,
						state,
						json
					) {
						...
						$timeout(function() {
							$rootScope.$broadcast(
								"$cordovaLocalNotification:triggered",
								notification
							);
						});
					};
					...
					$window.plugin.notification.local.onadd = function(id, state, json) {
						...
						$timeout(function() {
							$rootScope.$broadcast(
								"$cordovaLocalNotification:added",
								notification
							);
						});
					};
	...
	angular
		.module("ngCordova.plugins.mMediaAds", [])
		.factory("$cordovaMMediaAds", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					prepareInterstitial: function(options) {
						...
						$window.mMedia.prepareInterstitial(
							options,
							function() {
								d.resolve();
							},
					...
					showInterstitial: function() {
						...
						$window.mMedia.showInterstitial(
							function() {
								d.resolve();
							},
							function() {
	...
	angular.module("ngCordova.plugins.media", []).factory("$cordovaMedia", [
		"$q",
		function($q) {
			return {
	...
	angular
		.module("ngCordova.plugins.mobfoxAds", [])
		.factory("$cordovaMobFoxAds", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					prepareInterstitial: function(options) {
						...
						$window.MobFox.prepareInterstitial(
							options,
							function() {
								d.resolve();
							},
					...
					showInterstitial: function() {
						...
						$window.MobFox.showInterstitial(
							function() {
								d.resolve();
							},
							function() {
	...
	angular.module("ngCordova.plugins.mopubAds", []).factory("$cordovaMoPubAds", [
		"$q",
		"$window",
		function($q, $window) {
			return {
				...
				prepareInterstitial: function(options) {
					...
					$window.MoPub.prepareInterstitial(
						options,
						function() {
							d.resolve();
						},
	...
	angular
		.module("ngCordova.plugins.nativeAudio", [])
		.factory("$cordovaNativeAudio", [
			"$q",
			"$window",
			function($q, $window) {
				return {
					...
					loop: function(id) {
						var q = $q.defer();
						$window.plugins.NativeAudio.loop(
							id,
							function(result) {
								q.resolve(result);
							},
	...
	angular
		.module("ngCordova.plugins.network", [])
 ...
 */
	...
	angular
		.module("ngCordova.plugins.oauth", ["ngCordova.plugins.oauthUtility"])
					...
					adfs: function(clientId, adfsServer, relyingPartyId) {
						...
						if (window.cordova) {
							var cordovaMetadata = cordova.require("cordova/plugin_list")
								.metadata;
							if (
								cordovaMetadata.hasOwnProperty(
									"org.apache.cordova.inappbrowser"
								) === true
							) {
								var browserRef = window.open(
									adfsServer +
										"/adfs/oauth2/authorize?response_type=code&client_id=" +
										clientId +
										"&redirect_uri=http://localhost/callback&resource=" +
										relyingPartyId,
									"_blank",
									"location=no"
								);
							...
							if (
								...
								browserRef.addEventListener("loadstart", function(event) {
									if (event.url.indexOf("http://localhost/callback") === 0) {
										...
										if (
											parameterMap.access_token !== undefined &&
							...
							if (
								...
								var browserRef = window.open(
									"https://cloud.digitalocean.com/v1/oauth/authorize?client_id=" +
										clientId +
										"&redirect_uri=http://localhost/callback&response_type=code&scope=read%20write",
									"_blank",
									"location=no,clearsessioncache=yes,clearcache=yes"
								);
								browserRef.addEventListener("loadstart", function(event) {
							...
							if (
								...
								var browserRef = window.open(
									"https://accounts.google.com/o/oauth2/auth?client_id=" +
										clientId +
										"&redirect_uri=http://localhost/callback&scope=" +
										appScope.join(" ") +
							...
							if (
								...
								var browserRef = window.open(
									"https://www.facebook.com/dialog/oauth?client_id=" +
										clientId +
										"&redirect_uri=http://localhost/callback&response_type=token&scope=" +
										appScope.join(","),
									"_blank",
									"location=no,clearsessioncache=yes,clearcache=yes"
								);
					...
					linkedin: function(clientId, clientSecret, appScope, state) {
						...
						if (window.cordova) {
							...
							if (
								...
								var browserRef = window.open(
									"https://www.linkedin.com/uas/oauth2/authorization?client_id=" +
										clientId +
										"&redirect_uri=http://localhost/callback&scope=" +
										appScope.join(" ") +
					...
					box: function(clientId, clientSecret, appState) {
						...
						if (window.cordova) {
							...
							if (
								...
								var browserRef = window.open(
									"https://app.box.com/api/oauth2/authorize/?client_id=" +
										clientId +
					...
					reddit: function(clientId, clientSecret, appScope) {
						...
						if (window.cordova) {
							...
							if (
								...
								var browserRef = window.open(
									"https://ssl.reddit.com/api/v1/authorize?client_id=" +
										clientId +
									...
									$http({
										...
										.success(function(requestTokenResult) {
											...
											for (var i = 0; i < requestTokenParameters.length; i++) {
												parameterMap[
													requestTokenParameters[i].split("=")[0]
												] = requestTokenParameters[i].split("=")[1];
											}
											if (
												parameterMap.hasOwnProperty("oauth_token") === false
											) {
												deferred.reject("Oauth request token was not received");
											}
											var browserRef = window.open(
												"https://api.twitter.com/oauth/authenticate?oauth_token=" +
													parameterMap.oauth_token,
												"_blank",
												"location=no,clearsessioncache=yes,clearcache=yes"
											);
											browserRef.addEventListener("loadstart", function(event) {
												if (
													event.url.indexOf("http://localhost/callback") === 0
												) {
													var callbackResponse = event.url.split("?")[1];
													var responseParameters = callbackResponse.split("&");
													var parameterMap = {};
													...
													if (
														parameterMap.hasOwnProperty("oauth_verifier") ===
														false
													) {
														deferred.reject(
													...
													$http({
														...
														.success(function(result) {
															...
															for (
																var i = 0;
																i < accessTokenParameters.length;
																i++
															) {
																parameterMap[
																	accessTokenParameters[i].split("=")[0]
																] = accessTokenParameters[i].split("=")[1];
															}
															if (
																parameterMap.hasOwnProperty(
																	"oauth_token_secret"
																) === false
															) {
																deferred.reject(
							...
							if (
								...
								browserRef.addEventListener("loadstart", function(event) {
									if (startWith(event.url, redirectUri)) {
										...
										if (
											typeof oauthResponse === "undefined" ||
										...
										.success(function(requestTokenResult) {
											...
											browserRef.addEventListener("loadstart", function(event) {
												if (
													...
													oauthObject.oauth_token = parameterMap.oauth_token;
													oauthObject.oauth_nonce = $cordovaOauthUtility.createNonce(
														5
													);
													oauthObject.oauth_verifier =
														parameterMap.oauth_verifier;
													var signatureObj = $cordovaOauthUtility.createSignature(
														"POST",
														baseUrl + "/oauth/token",
														oauthObject,
							...
							if (
								...
								var browserRef = window.open(
									"https://oauth.vk.com/authorize?client_id=" +
										clientId +
										"&redirect_uri=http://oauth.vk.com/blank.html&response_type=token&scope=" +
										appScope.join(",") +
								...
								browserRef.addEventListener("loadstart", function(event) {
									...
									if (
										tmp[0] == "https://oauth.vk.com/blank.html" ||
										tmp[0] == "http://oauth.vk.com/blank.html"
									) {
										var callbackResponse = event.url.split("#")[1];
										var responseParameters = callbackResponse.split("&");
										var parameterMap = [];
	...
	angular
		.module("ngCordova.plugins.oauthUtility", [])
		.factory("$cordovaOauthUtility", [
			"$q",
			function($q) {
				return {
					/*
					...
					createSignature: function(
						...
						if (typeof jsSHA !== "undefined") {
							...
							for (var i = 0; i < bodyParameterKeys.length; i++) {
								headerAndBodyParameters[
									bodyParameterKeys[i]
								] = encodeURIComponent(bodyParameters[bodyParameterKeys[i]]);
							}
							var signatureBaseString =
								method + "&" + encodeURIComponent(endPoint) + "&";
							var headerAndBodyParameterKeys = Object.keys(
								headerAndBodyParameters
							).sort();
							for (i = 0; i < headerAndBodyParameterKeys.length; i++) {
								if (i == headerAndBodyParameterKeys.length - 1) {
									signatureBaseString += encodeURIComponent(
										headerAndBodyParameterKeys[i] +
											"=" +
							...
							headerParameters.oauth_signature = encodeURIComponent(
								oauthSignatureObject.getHMAC(
									encodeURIComponent(secretKey) + "&" + encodedTokenSecret,
									"TEXT",
									"SHA-1",
									"B64"
								)
							...
							for (i = 0; i < headerParameterKeys.length; i++) {
								if (i == headerParameterKeys.length - 1) {
									authorizationHeader +=
										headerParameterKeys[i] +
										'="' +
										headerParameters[headerParameterKeys[i]] +
										'"';
								} else {
									authorizationHeader +=
										headerParameterKeys[i] +
    ...
    */
	...
	angular
		.module("ngCordova.plugins.pinDialog", [])
	...
	angular.module("ngCordova.plugins.prefs", []).factory("$cordovaPreferences", [
		"$window",
		"$q",
		function($window, $q) {
			return {
				set: function(key, value) {
					...
					$window.appgiraffe.plugins.applicationPreferences.set(
						key,
						value,
						function(result) {
							q.resolve(result);
				...
				get: function(key) {
					...
					$window.appgiraffe.plugins.applicationPreferences.get(
						key,
						function(value) {
							q.resolve(value);
						},
	...
	angular.module("ngCordova.plugins.printer", []).factory("$cordovaPrinter", [
		"$q",
		"$window",
		function($q, $window) {
			return {
	...
	angular
		.module("ngCordova.plugins.progressIndicator", [])
			...
			function($q) {
				return {
					...
					showSimpleWithLabelDetail: function(_dim, _label, _detail) {
						...
						return ProgressIndicator.showSimpleWithLabelDetail(
							dim,
							label,
							detail
						);
					...
					showDeterminateWithLabel: function(_dim, _timeout, _label) {
						...
						return ProgressIndicator.showDeterminateWithLabel(
							dim,
							timeout,
							label
						);
	...
	angular.module("ngCordova.plugins.push", []).factory("$cordovaPush", [
		"$q",
		"$window",
		"$rootScope",
		"$timeout",
				...
				register: function(config) {
					...
					if (config !== undefined && config.ecb === undefined) {
						...
						config.ecb =
							"angular.element(" +
							injector +
							").injector().get('$cordovaPush').onNotification";
					}
	...
	angular.module("ngCordova.plugins.sms", []).factory("$cordovaSms", [
		"$q",
		function($q) {
			return {
	...
	angular
		.module("ngCordova.plugins.socialSharing", [])
		.factory("$cordovaSocialSharing", [
			"$q",
			"$window",
			function($q, $window) {
				return {
	...
	angular
		.module("ngCordova.plugins.spinnerDialog", [])
	...
	angular
		.module("ngCordova.plugins.splashscreen", [])
	...
	angular.module("ngCordova.plugins.sqlite", []).factory("$cordovaSQLite", [
		"$q",
		"$window",
		function($q, $window) {
			return {
	...
	angular
		.module("ngCordova.plugins.statusbar", [])
	...
	angular.module("ngCordova.plugins.toast", []).factory("$cordovaToast", [
		"$q",
		"$window",
		function($q, $window) {
			return {
	...
	angular.module("ngCordova.plugins.touchid", []).factory("$cordovaTouchID", [
		"$q",
		function($q) {
			return {
	...
	angular
		.module("ngCordova.plugins.vibration", [])
	...
	angular
		.module("ngCordova.plugins.videoCapturePlus", [])
	...
	angular.module("ngCordova.plugins.zip", []).factory("$cordovaZip", [
		"$q",
		"$window",
		function($q, $window) {
			return {
