	...
	(function(ionic) {
       ...
       */
					...
					methodNames.forEach(function(methodName) {
						DelegateInstance.prototype[methodName] = instanceMethodCaller(
							methodName
						);
					});
       ...
       */
					...
					DelegateService.prototype._registerInstance = function(
						instance,
						handle,
						filterFn
					) {
					...
					function instanceMethodCaller(methodName) {
						return function caller() {
							...
							this._instances.forEach(function(instance) {
								if (
									(!handle || handle == instance.$$delegateHandle) &&
									instance.$$filterFn(instance)
								) {
									foundInstancesCount++;
			...
			getParentWithClass: function(e, className, depth) {
				...
				while (e.parentNode && depth--) {
					if (
						...
						e.parentNode.classList.contains(className)
					) {
						return e.parentNode;
					}
					e = e.parentNode;
   ...
   */
     ...
     */
		...
		ionic.onGesture = function() {
			return ionic.EventController.onGesture.apply(
				ionic.EventController.onGesture,
				arguments
			);
		};
		ionic.offGesture = function() {
			return ionic.EventController.offGesture.apply(
				ionic.EventController.offGesture,
				arguments
			);
		};
	})(window.ionic);
  ...
  */
   ...
   */
		...
		ionic.Gestures.NO_MOUSEEVENTS =
			ionic.Gestures.HAS_TOUCHEVENTS &&
			window.navigator.userAgent.match(ionic.Gestures.MOBILE_REGEX);
   ...
   */
		function setup() {
			...
			ionic.Gestures.event.onTouch(
				ionic.Gestures.DOCUMENT,
				ionic.Gestures.EVENT_MOVE,
				ionic.Gestures.detection.detect
			);
			ionic.Gestures.event.onTouch(
				ionic.Gestures.DOCUMENT,
				ionic.Gestures.EVENT_END,
				ionic.Gestures.detection.detect
			);
   ...
   */
		ionic.Gestures.Instance = function(element, options) {
			...
			if (this.options.stop_browser_behavior) {
				ionic.Gestures.utils.stopDefaultBrowserBehavior(
					this.element,
   ...
   */
     ...
     */
			onTouch: function onTouch(element, eventType, handler) {
				...
				this.bindDom(
					element,
					ionic.Gestures.EVENT_TYPES[eventType],
					function bindDomOnTouch(ev) {
						...
						if (enable_detect) {
							...
							if (
								...
								count_touches = ionic.Gestures.PointerEvent.updatePointer(
									eventType,
									ev
								);
     ...
     */
			collectEventData: function collectEventData(
				...
				if (
					ev.type.match(/mouse/) ||
					ionic.Gestures.PointerEvent.matchType(
						ionic.Gestures.POINTER_MOUSE,
						ev
					)
				) {
			...
			getDirection: function getDirection(touch1, touch2) {
				...
				if (x >= y) {
					return touch1.pageX - touch2.pageX > 0
						? ionic.Gestures.DIRECTION_LEFT
						: ionic.Gestures.DIRECTION_RIGHT;
				} else {
					return touch1.pageY - touch2.pageY > 0
						? ionic.Gestures.DIRECTION_UP
						: ionic.Gestures.DIRECTION_DOWN;
				}
			},
     ...
     */
			isVertical: function isVertical(direction) {
				return (
					direction == ionic.Gestures.DIRECTION_UP ||
					direction == ionic.Gestures.DIRECTION_DOWN
				);
			},
     ...
     */
			detect: function detect(eventData) {
				...
				if (
					eventData.eventType == ionic.Gestures.EVENT_END &&
     ...
     */
			extendEventData: function extendEventData(ev) {
				...
				if (
					...
					for (var i = 0, len = ev.touches.length; i < len; i++) {
						startEv.touches.push(
							ionic.Gestures.utils.extend({}, ev.touches[i])
						);
					}
				}
				...
				var delta_time = ev.timeStamp - startEv.timeStamp,
					...
					velocity = ionic.Gestures.utils.getVelocity(
						delta_time,
						delta_x,
						delta_y
					);
				...
				ionic.Gestures.utils.extend(ev, {
					...
					rotation: ionic.Gestures.utils.getRotation(
						startEv.touches,
						ev.touches
					),
   ...
   */
		ionic.Gestures.gestures.Tap = {
			...
			handler: function tapGesture(ev, inst) {
				if (
					ev.eventType == ionic.Gestures.EVENT_END &&
   ...
   */
		ionic.Gestures.gestures.Drag = {
			...
			handler: function dragGesture(ev, inst) {
				if (
					ev.srcEvent.type == "touchstart" ||
					ev.srcEvent.type == "touchend"
				) {
					this.preventedFirstMove = false;
				} else if (
					!this.preventedFirstMove &&
					ev.srcEvent.type == "touchmove"
				) {
					...
					case ionic.Gestures.EVENT_MOVE:
						...
						if (ionic.Gestures.detection.current.name != this.name) {
							...
							if (inst.options.correct_for_drag_min_distance) {
								...
								var factor = Math.abs(
									inst.options.drag_min_distance / ev.distance
								);
								ionic.Gestures.detection.current.startEvent.center.pageX +=
									ev.deltaX * factor;
								ionic.Gestures.detection.current.startEvent.center.pageY +=
									ev.deltaY * factor;
						...
						var last_direction =
							ionic.Gestures.detection.current.lastEvent.direction;
						...
						if (
							(inst.options.drag_block_vertical &&
								ionic.Gestures.utils.isVertical(ev.direction)) ||
	...
	(function(document, ionic) {
		...
		(function() {
			...
			ionic.CSS.TRANSITION_DURATION =
				(isWebkit ? "-webkit-" : "") + "transition-duration";

			// To be sure transitionend works everywhere, include *both* the webkit and non-webkit events
			ionic.CSS.TRANSITIONEND =
				(isWebkit ? "webkitTransitionEnd " : "") + "transitionend";
		})();
...
*/
	...
	ionic.tap = {
		...
		ignoreScrollStart: function(e) {
			return (
				...
				(e.target.dataset
					...
					: e.target.getAttribute("data-prevent-scroll")) == "true" || // manually set within an elements attributes
		...
		isTextInput: function(ele) {
			return (
				...
				(ele.tagName == "TEXTAREA" ||
					...
					(ele.tagName == "INPUT" &&
						!/^(radio|checkbox|range|file|submit|reset|color|image|button)$/i.test(
							ele.type
						)))
			);
		},
		...
		cloneFocusedInput: function(container) {
			...
			ionic.requestAnimationFrame(function() {
				...
				if (
					ionic.tap.isTextInput(focusInput) &&
					!ionic.tap.isDateInput(focusInput)
				) {
		...
		removeClonedInputs: function(container) {
			...
			ionic.requestAnimationFrame(function() {
				...
				var previousInputFocus = container.querySelectorAll(
		...
		requiresNativeClick: function(ele) {
			if (
				...
				(ele.tagName == "A" ||
					...
					(ele.tagName == "INPUT" &&
						(ele.type == "button" || ele.type == "submit")))
			) {
	...
	function tapHandleFocus(ele) {
		...
		if (ele.tagName == "SELECT") {
			...
			/^(input|textarea|ion-label)$/i.test(ele.tagName) ||
	...
	function tapFocusOutActive() {
		...
		if (
			ele &&
			(/^(input|textarea|select)$/i.test(ele.tagName) || ele.isContentEditable)
		) {
	...
	(function(document, ionic) {
		...
		ionic.activator = {
			start: function(e) {
				...
				ionic.requestAnimationFrame(function() {
					if (
						(ionic.scroll && ionic.scroll.isScrolling) ||
						ionic.tap.requiresNativeClick(e.target)
					)
						return;
					var ele = e.target;
					var eleToActivate;
 ...
 */
	var debouncedKeyboardNativeShow = ionic.debounce(
		keyboardNativeShow,
		100,
		true
	);
 ...
 */
	function keyboardFocusIn(e) {
		...
		if (
			...
			!(scrollView = ionic.DomUtil.getParentWithClass(
				e.target,
 ...
 */
	function keyboardShow() {
		...
		if (keyboardActiveElement) {
			...
			details.isElementUnderKeyboard =
				details.elementBottom > details.windowHeight;
	...
	function getViewportHeight() {
		...
		if (
			!(ionic.Platform.isAndroid() && ionic.Platform.isFullScreen) &&
  ...
  */
		...
		if (viewportTag) {
			...
			for (x = 0; x < props.length; x++) {
				if (props[x]) {
					...
					viewportProperties[keyValue[0]] =
						keyValue.length > 1 ? keyValue[1] : "_";
				}
			}
			viewportUpdate();
		}
	...
	function viewportUpdate() {
		...
		if (
			initWidth !== viewportProperties.width ||
			initHeight !== viewportProperties.height
		) {
			viewportTagUpdate();
		}
	}
        ...
        when to fade out a scrollbar. */
				...
				self.setScrollStart = function() {
					ionic.scroll.isScrolling =
						Math.abs(ionic.scroll.lastTop - self.__scrollTop) > 1;
					clearTimeout(self.scrollTimer);
					self.scrollTimer = setTimeout(self.setScrollStop, 80);
				};
  ...
  */
     ...
     */
				self.scrollChildIntoView = function(e) {
					...
					// D
					var scrollBottomOffsetToTop = container.getBoundingClientRect()
						.bottom;
					// D - A
      ...
      */
					if (!alreadyShrunk) {
						...
						if (
							...
							// E - D                         E                         D
							var scrollBottomOffsetToBottom =
								e.detail.viewportHeight - scrollBottomOffsetToTop;

							// 0 or D - B if D > B           E - B                     E - D
							var keyboardOffset = Math.max(
								0,
								e.detail.keyboardHeight - scrollBottomOffsetToBottom
							);

							ionic.requestAnimationFrame(function() {
								// D - A or B - A if D > B       D - A             max(0, D - B)
								scrollViewOffsetHeight =
									scrollViewOffsetHeight - keyboardOffset;
       ...
       */
					...
					if (e.detail.isElementUnderKeyboard) {
						ionic.requestAnimationFrame(function() {
							...
							// C
							var inputMidpoint =
								(e.detail.elementBottom + e.detail.elementTop) / 2;

							// distance from middle of input to the bottom of the scroll view
							// C - D                                C               D
							var inputMidpointOffsetToScrollBottom =
								inputMidpoint - scrollBottomOffsetToTop;

							//C - D + (D - A)/2          C - D                     (D - A)/ 2
							var scrollTop =
								inputMidpointOffsetToScrollBottom + scrollMidpointOffset;
				...
				self.touchMove = function(e) {
					if (
						...
						(e.target.tagName === "TEXTAREA" &&
							e.target.parentElement.querySelector(":focus"))
					) {
						return;
					}
					...
					if (self.startCoordinates) {
						...
						if (
							...
							Math.abs(self.startCoordinates.y - currentCoordinates.y) > 10
						) {
				...
				self.mouseWheel = ionic.animationFrameThrottle(function(e) {
					var scrollParent = ionic.DomUtil.getParentOrSelfWithClass(
						e.target,
						"ionic-scroll"
					);
			...
			__resizeScrollbars: function() {
				...
				if (self.__indicatorX) {
					var width = Math.max(
						Math.round(
							self.__clientWidth * self.__clientWidth / self.__contentWidth
						),
						20
					);
					...
					self.__indicatorX.sizeRatio = self.__maxScrollLeft
						? self.__indicatorX.maxPos / self.__maxScrollLeft
						: 1;
				}
				...
				if (self.__indicatorY) {
					var height = Math.max(
						Math.round(
							self.__clientHeight * self.__clientHeight / self.__contentHeight
						),
						20
					);
					...
					self.__indicatorY.sizeRatio = self.__maxScrollTop
				...
				if (self.__indicatorX) {
					...
					if (self.__scrollLeft < 0) {
						widthScale = Math.max(
							...
							(self.__indicatorX.size - Math.abs(self.__scrollLeft)) /
								self.__indicatorX.size
						);
						...
						self.__indicatorX.indicator.style[self.__transformOriginProperty] =
							"left center";
					} else if (widthDiff > 0) {
						widthScale = Math.max(
							self.__indicatorX.minScale,
					...
					var translate3dX =
						"translate3d(" + x + "px, 0, 0) scaleX(" + widthScale + ")";
					if (self.__indicatorX.transformProp !== translate3dX) {
						self.__indicatorX.indicator.style[
							self.__transformProperty
						] = translate3dX;
				...
				if (self.__indicatorY) {
					...
					if (self.__scrollTop < 0) {
						heightScale = Math.max(
							...
							(self.__indicatorY.size - Math.abs(self.__scrollTop)) /
								self.__indicatorY.size
						);
						...
						if (self.__indicatorY.originProp !== "center top") {
							self.__indicatorY.indicator.style[
								self.__transformOriginProperty
							] =
								"center top";
					...
					var translate3dY =
						"translate3d(0," + y + "px, 0) scaleY(" + heightScale + ")";
				...
				if (self.__isDragging) {
					...
					if (self.__enableScrollY) {
						...
						if (scrollTop > maxScrollTop || scrollTop < 0) {
							// Slow down on the edges
							if (
								self.options.bouncing ||
								(self.__refreshHeight && scrollTop < 0)
							) {
  ...
  */
   ...
   */
			__startDeceleration: function() {
				...
				var verify = function() {
					var shouldContinue =
						Math.abs(self.__decelerationVelocityX) >=
							self.__minVelocityToKeepDecelerating ||
						Math.abs(self.__decelerationVelocityY) >=
							self.__minVelocityToKeepDecelerating;
					if (!shouldContinue) {
				...
				self.__isDecelerating = zyngaCore.effect.Animate.start(
					step,
					verify,
					completed
				);
   ...
   */
			__stepThroughDeceleration: function(render) {
				...
				if (self.options.bouncing) {
					...
					if (scrollOutsideX !== 0) {
						var isHeadingOutwardsX =
							scrollOutsideX * self.__decelerationVelocityX <=
							self.__minDecelerationScrollLeft;
					...
					if (scrollOutsideY !== 0) {
						var isHeadingOutwardsY =
							scrollOutsideY * self.__decelerationVelocityY <=
							self.__minDecelerationScrollTop;
   ...
   */
			__getScale: function getScale(start, end) {
				...
				if (start.length >= 2 && end.length >= 2) {
					return (
						this.__getDistance(end[0], end[1]) /
	...
	(function(ionic) {
		...
		SlideDrag.prototype.start = function(e) {
			...
			if (e.target.classList.contains(ITEM_CONTENT_CLASS)) {
				...
				content = ionic.DomUtil.getParentWithClass(
					e.target,
					ITEM_CONTENT_CLASS
				);
			}
   ...
   */
		...
		SlideDrag.prototype.drag = ionic.animationFrameThrottle(function(e) {
			...
			if (this._isDragging) {
				...
				if (newX < -buttonsWidth) {
					...
					newX = Math.min(
						-buttonsWidth,
						-buttonsWidth + (e.gesture.deltaX + buttonsWidth) * 0.4
					);
				}

				this._currentDrag.content.$$ionicOptionsOpen = newX !== 0;

				this._currentDrag.content.style[ionic.CSS.TRANSFORM] =
		...
		SlideDrag.prototype.end = function(e, doneCallback) {
			...
			if (e.gesture.deltaX > -(self._currentDrag.buttonsWidth / 2)) {
				...
				if (
					e.gesture.direction == "left" &&
					Math.abs(e.gesture.velocityX) < 0.3
				) {
					restingPoint = 0;
			...
			ionic.requestAnimationFrame(function() {
				if (restingPoint === 0) {
					...
					self._currentDrag.content.style[ionic.CSS.TRANSFORM] =
						"translate3d(" + restingPoint + "px,0,0)";
				}
		...
		ReorderDrag.prototype.start = function(e) {
			var startIndex = ionic.DomUtil.getChildIndex(
				this.el,
		...
		ReorderDrag.prototype.drag = ionic.animationFrameThrottle(function(e) {
			...
			if (this.scrollView) {
				...
				var pixelsPastTop =
					containerTop - pageY + this._currentDrag.elementHeight / 2;
				var pixelsPastBottom =
					pageY +
					this._currentDrag.elementHeight / 2 -
					containerTop -
			...
			if (
				!this._isDragging &&
				Math.abs(e.gesture.deltaY) > this.dragThresholdY
			) {
				this._isDragging = true;
			}
		...
		ReorderDrag.prototype.end = function(e, doneCallback) {
			...
			this.onReorder &&
				this.onReorder(this.el, this._currentDrag.startIndex, finalIndex);
   ...
   */
		ionic.views.ListView = ionic.views.View.inherit({
			initialize: function(opts) {
				...
				if (!self.itemHeight && self.listEl) {
					self.itemHeight =
						self.listEl.children[0] &&
						parseInt(self.listEl.children[0].style.height, 10);
				}
				...
				if (
					ionic.DomUtil.getParentOrSelfWithClass(self.el, "overflow-scroll")
				) {
			...
			didScroll: function(e) {
				...
				if (self.isVirtual) {
					...
					var highWater = Math.max(
						0,
						e.scrollTop + self.virtualRemoveThreshold
					);

					// Low water is the pixel position of the last element to include (everything after
					// that will be removed)
					var lowWater = Math.min(
						scrollHeight,
						Math.abs(e.scrollTop) + viewportHeight + self.virtualAddThreshold
					);
					...
					self._virtualItemsToRemove = Array.prototype.slice.call(
						self.listEl.children,
						0,
						first
					);

					self.renderViewport &&
						self.renderViewport(highWater, lowWater, first, last);
				}
			},
     ...
     */
			...
			_startDrag: function(e) {
				...
				if (
					ionic.DomUtil.getParentOrSelfWithClass(
						e.target,
						ITEM_REORDER_BTN_CLASS
					) &&
				...
				if (
					lastDragOp &&
					self._dragOp &&
					!self._dragOp.isSameItem(lastDragOp) &&
					e.defaultPrevented
				) {
					lastDragOp.clean && lastDragOp.clean();
				}
   ...
   */
		ionic.views.SideMenu = ionic.views.View.inherit({
			initialize: function(opts) {
				this.el = opts.el;
				this.isEnabled =
					typeof opts.isEnabled === "undefined" ? true : opts.isEnabled;
							...
							if (options.continuous) {
								...
								if (options.bouncing) {
									...
									if (width * index - delta.x < 0) {
										//We are trying scroll past left boundary
									...
									if (Math.abs(delta.x) > width * (slides.length - index - 1)) {
										//We are trying to scroll past right bondary
										delta.x = Math.max(
											-width * (slides.length - index - 1),
											delta.x
										); //Set delta.x so we don't go past right screen
									}
								}
		...
		var Swiper = function(container, params, _scope, $compile) {
          ...
          ===========================*/
			s.updateClasses = function() {
				...
				if (s.bullets && s.bullets.length > 0) {
					...
					if (s.params.loop) {
						bulletIndex =
							Math.ceil(s.activeIndex - s.loopedSlides) /
							s.params.slidesPerGroup;
						if (bulletIndex > s.slides.length - 1 - s.loopedSlides * 2) {
							bulletIndex =
								bulletIndex - (s.slides.length - s.loopedSlides * 2);
						}
          ...
          ===========================*/
			...
			s.onTouchEnd = function(e) {
				...
				for (i = 0; i < s.slidesGrid.length; i += s.params.slidesPerGroup) {
					if (
						typeof s.slidesGrid[i + s.params.slidesPerGroup] !== "undefined"
					) {
						if (
          ...
          ===========================*/
			...
			s.updateLoop = function() {
				...
				if (
					angular.element(currentSlide).hasClass(s.params.slideDuplicateClass)
				) {
					// we're on a duplicate, so slide to the non-duplicate
					var swiperSlideIndex = angular
					...
					for (var i = 0; i < slides.length; i++) {
						if (
							...
							angular.element(slides[i]).attr("data-swiper-slide-index") ===
								swiperSlideIndex
						) {
          ...
          ===========================*/
			function handleKeyboard(e) {
				...
				if (kc === 37 || kc === 39 || kc === 38 || kc === 40) {
					...
					if (
						s.container.parents(".swiper-slide").length > 0 &&
						s.container.parents(".swiper-slide-active").length === 0
					) {
						return;
					}
					var windowScroll = {
	...
	(function(ionic) {
		...
		ionic.views.Toggle = ionic.views.View.inherit({
			initialize: function(opts) {
				...
				this.dragStartGesture = ionic.onGesture(
					"dragstart",
					this.dragStartHandler,
					this.el
				);
				this.dragGesture = ionic.onGesture("drag", this.dragHandler, this.el);
				this.dragHoldGesture = ionic.onGesture(
					"hold",
					this.holdHandler,
					this.el
				);
				this.dragReleaseGesture = ionic.onGesture(
					"release",
					this.releaseHandler,
					this.el
				);
			...
			setOpenPercent: function(openPercent) {
				...
				if (
					this.openPercent < 0 ||
					(openPercent < this.openPercent - 3 ||
						openPercent > this.openPercent + 3)
				) {
					...
					if (openPercent === 0) {
						...
						var openPixel = Math.round(
							openPercent / 100 * this.track.offsetWidth -
								this.handle.offsetWidth
						);
						openPixel = openPixel < 1 ? 0 : openPixel;
						this.handle.style[ionic.CSS.TRANSFORM] =
							"translate3d(" + openPixel + "px,0,0)";
					}
				}
			},
