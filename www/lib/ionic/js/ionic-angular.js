				...
				forEach(bindDefinition || {}, function(definition, scopeName) {
					...
					switch (mode) {
						...
						case "&":
							...
							if (
								attrs[attrName] &&
								attrs[attrName].match(RegExp(scopeName + "(.*?)"))
							) {
								throw new Error(
									'& expression binding "' +
										scopeName +
 ...
 */
			...
			function getHistory(scope) {
				...
				if (!viewHistory.histories[histObj.historyId]) {
					...
					viewHistory.histories[histObj.historyId] = {
						...
						parentHistoryId: getParentHistoryObj(histObj.scope.$parent)
							.historyId,
						stack: [],
						cursor: -1
					};
				...
				register: function(parentScope, viewLocals) {
					...
					if (forcedNav) {
						...
						viewHistory.views[viewId] = this.createView({
							...
							historyId: hist.historyId,
							backViewId:
								currentView && currentView.viewId ? currentView.viewId : null,
							forwardViewId: null,
					...
					if (
						...
						for (x = 0; x < hist.stack.length; x++) {
							if (hist.stack[x].viewId == viewId) {
								...
								viewHistory.currentView.backViewId =
									viewHistory.backView.backViewId;
     ...
     */
				currentHistoryId: function() {
					return viewHistory.currentView
						? viewHistory.currentView.historyId
						: null;
				},
     ...
     */
				backTitle: function(view) {
					var backView =
						(view && getViewById(view.backViewId)) || viewHistory.backView;
					...
					if (historyId) {
						...
						if (hist && hist.stack.length) {
							if (
								...
								viewHistory.currentView.viewId === hist.stack[0].viewId
							) {
								return;
							}
							forcedNav = {
     ...
     */
				goBack: function(backCount) {
					if (isDefined(backCount) && backCount !== -1) {
						...
						var clearStateIds = [];
						var fwdView = getViewById(
							currentHistory.stack[cursor].forwardViewId
						);
						while (fwdView) {
     ...
     */
					...
					if (arguments.length) {
						...
						if (opts === null) {
							...
							if (nextViewOptions.expire) {
								deregisterStateChangeListener = $rootScope.$on(
									"$stateChangeSuccess",
									function() {
				...
				isAbstractEle: function(ele, viewLocals) {
					if (
						viewLocals &&
						viewLocals.$$state &&
						viewLocals.$$state.self["abstract"]
					) {
						return true;
					}
					return !!(
						ele &&
						(isAbstractTag(ele) || isAbstractTag(ele.children()))
					);
				},
				...
				isActiveScope: function(scope) {
					...
					while (climbScope) {
						...
						if (currentHistoryId) {
							if (
								climbScope.hasOwnProperty("$historyId") &&
								currentHistoryId == climbScope.$historyId
							) {
								return true;
							}
						...
						if (
							foundHistoryId &&
							climbScope.hasOwnProperty("$activeHistoryId")
						) {
			...
			function isAbstractTag(ele) {
				return (
					ele && ele.length && /ion-side-menus|ion-tabs/i.test(ele[0].tagName)
				);
			}
			...
			function canSwipeBack(ele, viewLocals) {
				if (
					viewLocals &&
					viewLocals.$$state &&
					viewLocals.$$state.self.canSwipeBack === false
				) {
					return false;
				}
		...
		function(
			...
			$rootScope.$on("$ionicHistory.change", function(e, data) {
				...
				var hist = data.historyId
					? viewHistory.histories[data.historyId]
					: null;
 ...
 */
	...
	IonicModule.provider("$ionicConfig", function() {
		...
		// iOS Transitions
		// -----------------------
		provider.transitions.views.ios = function(
			enteringEle,
			leavingEle,
			direction,
			shouldAnimate
			...
			function setStyles(ele, opacity, x, boxShadowOpacity) {
				...
				if (boxShadowOpacity > -1) {
					css.boxShadow =
						"0 0 10px rgba(0,0,0," +
						(d.shouldAnimate ? boxShadowOpacity * 0.45 : 0.3) +
						")";
				}
			...
			var d = {
				...
				shouldAnimate:
					shouldAnimate && (direction == "forward" || direction == "back")
			};

			return d;
		};

		provider.transitions.navBar.ios = function(
			enteringHeaderBar,
			leavingHeaderBar,
			direction,
			shouldAnimate
			...
			function enter(ctrlA, ctrlB, step) {
				...
				var backTextX =
					(ctrlB &&
						(ctrlB.titleTextX() - ctrlA.backButtonTextLeft()) * (1 - step)) ||
					0;
			...
			function leave(ctrlA, ctrlB, step) {
				if (!ctrlA || !ctrlB) return;
				var titleX =
					(-(ctrlA.titleTextX() - ctrlB.backButtonTextLeft()) -
						ctrlA.titleLeftRight()) *
					step;
			...
			var d = {
				run: function(step) {
					...
					var leavingHeaderCtrl =
						leavingHeaderBar && leavingHeaderBar.controller();
					if (d.direction == "back") {
		...
		provider.transitions.views.android = function(
			enteringEle,
			leavingEle,
			direction,
			shouldAnimate
		...
		provider.transitions.navBar.android = function(
			enteringHeaderBar,
			leavingHeaderBar,
			direction,
			shouldAnimate
		...
		provider.transitions.views.none = function(enteringEle, leavingEle) {
			return {
				run: function(step) {
					provider.transitions.views
						.android(enteringEle, leavingEle, false, false)
						.run(step);
				},
				shouldAnimate: false
			};
		...
		provider.transitions.navBar.none = function(
			enteringHeaderBar,
			leavingHeaderBar
		) {
			return {
				run: function(step) {
					provider.transitions.navBar
						.ios(enteringHeaderBar, leavingHeaderBar, false, false)
						.run(step);
					provider.transitions.navBar
						.android(enteringHeaderBar, leavingHeaderBar, false, false)
						.run(step);
				},
				shouldAnimate: false
			};
		...
		function setPlatformConfig(platformName, platformConfigs) {
			...
			createConfig(
				configProperties.platform[platformName],
				provider.platform[platformName],
				""
			);
		}
				...
				if (angular.isObject(configObj[namespace])) {
					...
					providerObj[namespace] = function(newValue) {
						...
						if (configObj[namespace] == PLATFORM) {
							...
							return stringObj(
								configProperties.platform,
								"default" + platformPath + "." + namespace
							);
						}
     ...
     */
						...
						.then(function(self) {
							self.show = function(options) {
								...
								if (!self.isShown) {
									...
									self.hasBackdrop =
										!options.noBackdrop && options.showBackdrop !== false;
									if (self.hasBackdrop) {
										$ionicBackdrop.retain();
			...
			function showLoader(options) {
				...
				if (options.hideOnStateChange) {
					deregisterStateListener1 = $rootScope.$on(
						"$stateChangeSuccess",
						hideLoader
					);
					deregisterStateListener2 = $rootScope.$on(
						"$stateChangeError",
						hideLoader
					);
				}
   ...
   */
     ...
     */
				show: function(target) {
					...
					$timeout(function() {
						...
						self.scope.$parent &&
							self.scope.$parent.$broadcast(self.viewType + ".shown", self);
					...
					return $timeout(function() {
						...
						self.$el.on("touchmove", function(e) {
							...
							var isInScroll = ionic.DomUtil.getParentOrSelfWithClass(
								e.target,
								"scroll"
							);
							if (!isInScroll) {
						...
						self.$el.on("click", function(e) {
							if (
								self.backdropClickToClose &&
								e.target === self.el &&
								stack.isHighest(self)
							) {
								self.hide();
							}
						});
     ...
     */
				hide: function() {
					...
					self._isShown = false;
					self.scope.$parent &&
						self.scope.$parent.$broadcast(self.viewType + ".hidden", self);
     ...
     */
				remove: function() {
					var self = this,
						...
						promise;
					self.scope.$parent &&
						self.scope.$parent.$broadcast(self.viewType + ".removed", self);
     ...
     */
			...
			var createModal = function(templateString, options) {
				...
				var scope =
					(options.scope && options.scope.$new()) || $rootScope.$new(true);
 ...
 */
   ...
   */
	...
	IonicModule.service(
		"$ionicNavViewDelegate",
		ionic.DelegateService(["clearCache"])
	);
         ...
         */
						hardwareBackButtonClick: function(e) {
							...
							for (actionId in self.$backButtonActions) {
								if (
									!priorityAction ||
									self.$backButtonActions[actionId].priority >=
 ...
 */
		...
		function($ionicModal, $ionicPosition, $document, $window) {
			...
			function positionView(target, popoverEle) {
				...
				if (popoverCSS.left < POPOVER_BODY_PADDING) {
					...
					popoverCSS.left + popoverWidth + POPOVER_BODY_PADDING >
					bodyWidth
				) {
				...
				arrowEle.css({
					left:
						buttonOffset.left +
						buttonOffset.width / 2 -
						arrowEle.prop("offsetWidth") / 2 -
						popoverCSS.left +
						"px"
				});
   ...
   */
     ...
     */
				fromTemplate: function(templateString, options) {
					return $ionicModal.fromTemplate(
						templateString,
						ionic.Utils.extend({}, POPOVER_OPTIONS, options)
					);
				},
				/**
     * @ngdoc method
 ...
 */
     ...
     */
			...
			function createPopup(options) {
				...
				$q
					...
					.then(function(template) {
						var popupBody = jqLite(
							self.element[0].querySelector(".popup-body")
						);
						if (template) {
			...
			function showPrompt(opts) {
				...
				scope.data.placeholder = opts.inputPlaceholder
			...
			.config([
				...
				function($stateProvider, $ionicConfigProvider) {
					...
					$stateProvider.state = function(stateName, definition) {
						...
						return stateProviderState.call(
							$stateProvider,
							stateName,
							definition
						);
		...
		function(
			...
			var ionicViewSwitcher = {
				create: function(
					...
					var switcher = {
						...
						loadViewElements: function(registerData) {
							...
							var enteringEleIdentifier = getViewElementIdentifier(
								viewLocals,
								enteringView
							);
							...
							for (x = 0, l = viewElements.length; x < l; x++) {
								...
								if (
									viewEle.data(DATA_ELE_IDENTIFIER) === enteringEleIdentifier
								) {
									...
									if (viewEle.data(DATA_NO_CACHE)) {
										...
										viewEle.data(
											DATA_ELE_IDENTIFIER,
											enteringEleIdentifier + ionic.Utils.nextUid()
										);
									...
									isDefined(navViewActiveEleId) &&
									viewEle.data(DATA_ELE_IDENTIFIER) === navViewActiveEleId
								) {
									leavingEle = viewEle;
								}
							...
							if (!alreadyInDom) {
								...
								enteringEle =
									registerData.ele ||
									ionicViewSwitcher.createViewEle(viewLocals);
						...
						render: function(registerData, callback) {
							if (alreadyInDom) {
								...
								var enteringData = getTransitionData(
									viewLocals,
									enteringEle,
									registerData.direction,
									enteringView
								);
								var transitionFn =
									$ionicConfig.transitions.views[enteringData.transition] ||
									$ionicConfig.transitions.views.none;
								transitionFn(
									enteringEle,
									null,
								...
								var viewScope = navViewCtrl.appendViewElement(
									enteringEle,
									viewLocals
								);
						...
						transition: function(direction, enableBack, allowAnimate) {
							...
							var leavingData = extend(
								extend({}, enteringData),
								getViewData(leavingView)
							);
							...
							if (viewTransition.shouldAnimate) {
								...
								enteringEle.data(
									DATA_FALLBACK_TIMER,
									$timeout(transitionComplete, defaultTimeout)
								);
								...
								return {
									...
									cancel: function(shouldAnimate) {
										if (shouldAnimate) {
											enteringEle.on(
												TRANSITIONEND_EVENT,
												cancelOnTransitionEnd
											);
											enteringEle.data(
												DATA_FALLBACK_TIMER,
												$timeout(cancelTransition, defaultTimeout)
											);
											$ionicClickBlock.show(defaultTimeout);
										} else {
											cancelTransition();
							...
							function onReflow() {
								...
								navViewAttr(
									enteringEle,
									viewTransition.shouldAnimate ? "entering" : VIEW_STATUS_ACTIVE
								);
								navViewAttr(
									leavingEle,
									viewTransition.shouldAnimate ? "leaving" : VIEW_STATUS_CACHED
								);
							...
							function transitionComplete() {
								...
								$timeout.cancel(enteringEle.data(DATA_FALLBACK_TIMER));
								leavingEle &&
									$timeout.cancel(leavingEle.data(DATA_FALLBACK_TIMER));
								...
								if (transitionId === transitionCounter) {
									$q
										.all(transitionPromises)
										.then(ionicViewSwitcher.transitionEnd);
							...
							if (!enteringData.viewId || enteringData.abstractView) {
								...
								if (step == "after") {
									...
									if (leavingScope) {
										...
										prefixesAreEqual = compareStatePrefixes(
											enteringData.stateName,
											leavingData.stateName
										);
										if (prefixesAreEqual) {
								...
								if (enteringScope) {
									enteringScope.$emit(
										"$ionicView." + step + "Enter",
										enteringData
									);
									enteringScope.$broadcast(
										"$ionicParentView." + step + "Enter",
										enteringData
									);
								}
								...
								if (leavingScope) {
									leavingScope.$emit(
										"$ionicView." + step + "Leave",
										leavingData
									);
									leavingScope.$broadcast(
										"$ionicParentView." + step + "Leave",
										leavingData
									);
								} else if (
									enteringScope &&
						...
						cleanup: function(transData) {
							...
							if (
								leavingEle &&
								transData.direction == "back" &&
								!$ionicConfig.views.forwardCache()
							) {
							...
							var x, viewElement;
							var removeOldestAccess =
								viewElementsLength - 1 > $ionicConfig.views.maxCache();
							var removableEle;
							...
							for (x = 0; x < viewElementsLength; x++) {
								...
								if (
									removeOldestAccess &&
									viewElement.data(DATA_VIEW_ACCESSED) < oldestAccess
								) {
									...
									viewElement.data(DATA_DESTROY_ELE) &&
					...
					if (viewLocals && viewLocals.$template) {
						...
						if (containerEle.children.length === 1) {
							...
							if (
								...
								angular
									.element(containerEle.children[0])
									.attr("abstract", "true");
							} else {
								if (viewLocals.$$state && viewLocals.$$state.self) {
									angular
										.element(containerEle.children[0])
										.attr("state", viewLocals.$$state.self.name);
								}
							}
				...
				viewEleIsActive: function(viewEle, isActiveAttr) {
					navViewAttr(
						viewEle,
						isActiveAttr ? VIEW_STATUS_ACTIVE : VIEW_STATUS_CACHED
					);
				},
			...
			function getTransitionData(viewLocals, enteringEle, direction, view) {
				...
				direction =
					nextDirection ||
					cachedAttr(enteringEle, "view-direction") ||
					state.viewDirection ||
					direction ||
					"none";
				...
				return extend(getViewData(view), {
					...
					navBarTransition:
						navBarTransition === "view" ? viewTransition : navBarTransition,
					direction: direction,
			...
			function compareStatePrefixes(enteringStateName, exitingStateName) {
				...
				var enteringPrefix = enteringStateName.substring(
					0,
					enteringStateSuffixIndex
				);
				var exitingPrefix = exitingStateName.substring(
					0,
					exitingStateSuffixIndex
				);
			...
			$provide.decorator("$browser", [
				...
				function($delegate, $window) {
					...
					function isIOS9UIWebView(userAgent) {
						return (
							/(iPhone|iPad|iPod).* OS 9_\d/.test(userAgent) &&
							!/Version\/9\./.test(userAgent)
						);
					}
					...
					function applyIOS9Shim(browser) {
						...
						browser.url = function() {
							...
							return (
								pendingLocationUrl || originalUrlFn.apply(browser, arguments)
							);
						};
	...
	IonicModule.config([
		...
		function($provide) {
			$provide.decorator("$compile", [
				"$delegate",
				function($compile) {
					$compile.$$addScopeInfo = function $$addScopeInfo(
						$element,
						scope,
						isolated,
						noTemplate
						...
						var dataName = isolated
							? noTemplate ? "$isolateScopeNoTemplate" : "$isolateScope"
							: "$scope";
	...
	IonicModule.controller("$ionicHeaderBar", [
		...
		function($scope, $element, $attrs, $q, $ionicConfig, $ionicHistory) {
			...
			self.updateBackButton = function() {
				var ele;
				if (
					(isBackShown && isNavBackShown && isBackEnabled) !==
					isBackElementShown
				) {
			...
			self.align = function(textAlign) {
				...
				textAlign =
					textAlign || $attrs.alignTitle || $ionicConfig.navBar.alignTitle();
			...
			self.updatePositions = function(
				...
				if (titleEle) {
					...
					if (updateTitleRight !== titleRight) {
						titleEle.style.right = updateTitleRight
							? updateTitleRight + "px"
							: "";
						titleRight = updateTitleRight;
					}
				...
				ionic.requestAnimationFrame(function() {
					if (titleEle && titleEle.offsetWidth + 10 < titleEle.scrollWidth) {
						...
						var testRight =
							$element[0].offsetWidth - titleLeft - self.titleTextWidth() - 20;
		...
		function($scope, $attrs, $element, $timeout) {
			...
			function finishInfiniteScroll() {
				...
				$timeout(
					function() {
						...
						if (
							(self.jsScrolling &&
								self.scrollView.__container &&
								self.scrollView.__container.offsetHeight > 0) ||
							!self.jsScrolling
						) {
							self.checkBounds();
						}
			...
			function checkInfiniteBounds() {
				...
				if (self.jsScrolling) {
					...
					if (
						(maxScroll.left !== -1 &&
							self.scrollEl.scrollLeft >=
								maxScroll.left - self.scrollEl.clientWidth) ||
						(maxScroll.top !== -1 &&
							self.scrollEl.scrollTop >=
								maxScroll.top - self.scrollEl.clientHeight)
					) {
						onInfinite();
					}
				}
 ...
 */
   ...
   */
	...
	IonicModule.controller("$ionicNavBar", [
		...
		function(
			...
			var delegateHandle =
				$attrs.delegateHandle || "navBar" + ionic.Utils.nextUid();

			var deregisterInstance = $ionicNavBarDelegate._registerInstance(
				self,
				delegateHandle
			);
			...
			self.createHeaderBar = function(isActive) {
				...
				ionic.DomUtil.cachedAttr(
					containerEle,
					"nav-bar",
					isActive ? "active" : "cached"
				);

				var alignTitle = $attrs.alignTitle || $ionicConfig.navBar.alignTitle();
				var headerBarEle = jqLite("<ion-header-bar>")
					.addClass($attrs["class"])
					.attr("align-title", alignTitle);
				...
				function positionItem(ele, itemType) {
					...
					if (itemType === "title") {
						...
						(itemType == SECONDARY_BUTTONS &&
							$ionicConfig.navBar.positionSecondaryButtons() != "left") ||
						(itemType == PRIMARY_BUTTONS &&
			...
			self.update = function(viewData) {
				...
				var enteringHeaderBar = self.isInitialized
					? getOffScreenHeaderBar()
					: getOnScreenHeaderBar();
				var leavingHeaderBar = self.isInitialized
					? getOnScreenHeaderBar()
					: null;
				var enteringHeaderCtrl = enteringHeaderBar.controller();
			...
			self.transition = function(
				...
				var transitionFn =
					$ionicConfig.transitions.navBar[viewData.navBarTransition] ||
					$ionicConfig.transitions.navBar.none;
				...
				queuedTransitionStart = function() {
					...
					queuedTransitionEnd = function() {
						if (
							...
							!navBarTransition.shouldAnimate
						) {
							transitionEnd();
						}
					};
   ...
   */
			...
			self.hasBarSubheader = function(isBarSubheader) {
				$element[isBarSubheader ? "addClass" : "removeClass"](
	...
	IonicModule.controller("$ionicNavView", [
		...
		function(
			...
			self.init = function() {
				...
				var deregisterInstance = $ionicNavViewDelegate._registerInstance(
					self,
					$attrs.delegateHandle
				);
				$scope.$on("$destroy", function() {
			...
			self.update = function(registerData) {
				...
				var parentNavViewCtrl = $element
					.parent()
					.inheritedData("$ionNavViewController");
			...
			self.transitionEnd = function() {
				...
				for (x = 0, l = viewElements.length; x < l; x++) {
					...
					if (viewElement.data(DATA_ELE_IDENTIFIER) === activeEleId) {
						...
						navViewAttr(viewElement) === "leaving" ||
						navViewAttr(viewElement) === VIEW_STATUS_ACTIVE ||
						...
						if (
							viewElement.data(DATA_DESTROY_ELE) ||
							viewElement.data(DATA_NO_CACHE)
						) {
			...
			function onTabsLeave(ev, data) {
				...
				for (var x = 0, l = viewElements.length; x < l; x++) {
					...
					if (navViewAttr(viewElement) == VIEW_STATUS_ACTIVE) {
						...
						viewScope &&
							viewScope.$broadcast(ev.name.replace("Tabs", "ParentView"), data);
						break;
					}
				}
			}
   ...
   */
			self.enableBackButton = function(shouldEnable) {
				...
				associatedNavBarCtrl &&
					associatedNavBarCtrl.enableBackButton(shouldEnable);
			};
   ...
   */
			...
			self.initSwipeBack = function() {
				...
				function onDragStart(ev) {
					if (
						!isPrimary ||
						!$ionicConfig.views.swipeBackEnabled() ||
					...
					if (
						!backView ||
						backView.historyId !== currentView.historyId ||
					...
					viewTransition = switcher.transition(
						"back",
						$ionicHistory.enabledBack(backView),
						true
					);
				...
				function onRelease(ev) {
					if (
						...
						var isSwipingRight =
							releaseX >= dragPoints[dragPoints.length - 2].x;
						var releaseSwipeCompletion = getSwipeCompletion(releaseX);
						var velocity =
							Math.abs(startDrag.x - releaseX) / (now - startDrag.t);
						...
						disableAnimation =
							releaseSwipeCompletion < 0.03 || releaseSwipeCompletion > 0.97;

						if (
							isSwipingRight &&
							(releaseSwipeCompletion > 0.5 || velocity > 0.1)
						) {
							// complete view transition on release
							var speed =
								velocity > 0.5 || velocity < 0.05 || releaseX > windowWidth - 45
									? "fast"
									: "slow";
							...
							associatedNavBarCtrl &&
								associatedNavBarCtrl.activeTransition &&
								associatedNavBarCtrl.activeTransition.complete(
									!disableAnimation,
									speed
								);
						} else {
							...
							associatedNavBarCtrl &&
								associatedNavBarCtrl.activeTransition &&
								associatedNavBarCtrl.activeTransition.cancel(
									!disableAnimation,
									"fast",
									cancelData
								);
			...
			function onBarSubheader(ev, isBarSubheader) {
				var associatedNavBarCtrl = getAssociatedNavBarCtrl();
				associatedNavBarCtrl &&
					associatedNavBarCtrl.hasBarSubheader(isBarSubheader);
			}
			...
			function getAssociatedNavBarCtrl() {
				if (navBarDelegate) {
					for (var x = 0; x < $ionicNavBarDelegate._instances.length; x++) {
						if (
							$ionicNavBarDelegate._instances[x].$$delegateHandle ==
							navBarDelegate
						) {
	...
	IonicModule.controller("$ionicRefresher", [
		...
		function($scope, $attrs, $element, $ionicBind, $timeout) {
			...
			$scope.showSpinner =
				!isDefined($attrs.refreshingIcon) && $attrs.spinner != "none";
			...
			function overscroll(val) {
				scrollChild.style[ionic.CSS.TRANSFORM] =
					"translate3d(0px, " + val + "px, 0px)";
				lastOverscroll = val;
			}
 ...
 */
	IonicModule.controller("$ionicScroll", [
		...
		function(
			...
			var scrollView;
			if (self.isNative()) {
				scrollView = self.scrollView = new ionic.views.ScrollNative(
					scrollViewOptions
				);
			} else {
				scrollView = self.scrollView = new ionic.views.Scroll(
					scrollViewOptions
				);
			}
			...
			($element.parent().length ? $element.parent() : $element).data(
   ...
   */
			self.snapToRest = function(e) {
				...
				if (
					...
					direction == "right" &&
					ratio >= 0 &&
					(ratio >= 0.5 || velocityX > velocityThreshold)
				) {
			...
			self.exposeAside = function(shouldExposeAside) {
				if (
					!(self.left && self.left.isEnabled) &&
					!(self.right && self.right.isEnabled)
				)
					return;
				self.close();
			...
			self.isDraggableTarget = function(e) {
				...
				var shouldOnlyAllowEdgeDrag =
					self.edgeThresholdEnabled && !self.isOpen();
				var startX =
					e.gesture.startEvent &&
					e.gesture.startEvent.center &&
				...
				if (!menuEnabled) {
					var currentView = $ionicHistory.currentView() || {};
					return (
						dragIsWithinBounds && backView.historyId !== currentView.historyId
					);
				}
			...
			android: function(ele) {
				...
				function run() {
					...
					setSvgAttribute(
						circleEle,
						"da",
						Math.max(Math.min(dasharray, 188), 128)
					);
					setSvgAttribute(
						circleEle,
						"os",
						Math.max(Math.min(dashoffset, 182), 0)
					);
					setSvgAttribute(
						circleEle,
						"t",
			...
			function($element, $attrs, $ionicConfig) {
				...
				this.start = function() {
					animations[spinnerName] &&
						(anim = animations[spinnerName]($element[0])());
				};
		...
		function($scope, $ionicHistory, $attrs, $location, $state) {
			...
			this.hrefMatchesState = function() {
				return (
					...
					$location
						.path()
						.indexOf($attrs.href.replace(/^#/, "").replace(/\/$/, "")) === 0
				);
			};
			...
			this.navNameMatchesState = function() {
				return (
					this.navViewName &&
					$ionicHistory.isCurrentStateNavView(this.navViewName)
				);
			};

			this.tabMatchesState = function() {
				return (
					this.hrefMatchesState() ||
			...
			self.remove = function(tab) {
				...
				if (tab.$tabSelected) {
					...
					if (self.tabs.length === 1) {
						...
						var newTabIndex =
							tabIndex === self.tabs.length - 1 ? tabIndex - 1 : tabIndex + 1;
	...
	IonicModule.controller("$ionicView", [
		...
		function($scope, $element, $attrs, $compile, $rootScope) {
			...
			var deregIonNavBarInit = $scope.$on("ionNavBar.init", function(
				ev,
				delegateHandle
			) {
			...
			self.beforeEnter = function(ev, transData) {
				...
				if (transData && !transData.viewNotified) {
					...
					viewTitle = isDefined($attrs.viewTitle)
						? $attrs.viewTitle
						: $attrs.title;
			...
			function afterEnter() {
				...
				var viewTitleAttr =
					(isDefined($attrs.viewTitle) && "viewTitle") ||
					(isDefined($attrs.title) && "title");
				if (viewTitleAttr) {
			...
			return {
				...
				compile: function(element, attr) {
					...
					checkboxWrapper.classList.add(
						"checkbox-" + $ionicConfig.form.checkbox()
					);
				}
			};
		}
 ...
 */
	...
	function CollectionRepeatDirective(
		$ionicCollectionManager,
		$parse,
		$window,
		$$rAF,
		...
		function postLink(scope, element, attr, scrollCtrl, transclude) {
			...
			if (scrollView.options.scrollingX && scrollView.options.scrollingY) {
				throw new Error(
					"collection-repeat expected a parent x or y scrollView, not " +
						"an xy scrollView."
				);
			}

			var repeatExpr = attr.collectionRepeat;
			var match = repeatExpr.match(
			...
			function onResize() {
				if (
					changeValidator.resizeRequiresRefresh(
						scrollView.__clientWidth,
						scrollView.__clientHeight
					)
				) {
					refreshDimensions();
				}
			...
			scope.$watchCollection(listGetter, function(newValue) {
				...
				if (!angular.isArray(newValue)) {
					throw new Error(
						"collection-repeat expected an array for '" +
							listExpr +
							"', " +
							"but got a " +
							typeof value
			...
			function makeChangeValidator() {
				...
				return (self = {
					...
					dataChangeRequiresRefresh: function(newData) {
						var requiresRefresh =
							newData.length > 0 || newData.length < self.dataLength;
			...
			function getRepeatManager() {
				return (
					...
					(repeatManager = new $ionicCollectionManager({
						...
						widthData: widthData,
						forceRefreshImages: !!(
							isDefined(attr.forceRefreshImages) &&
			...
			function initAfterItemsContainer() {
				var container = angular.element(
					scrollView.__content.querySelector(
				...
				if (!container.length) {
					...
					var afterNodes = [].filter.call(
						scrollView.__content.childNodes,
						function(node) {
					...
					container = angular.element(
						'<span class="collection-repeat-after-container">'
					);
			...
			function parseDimensionAttr(attrValue, dimensionData) {
				...
				var isConstant =
					constantAttrValue.length &&
					!/([a-zA-Z]|\$|:|\?)/.test(constantAttrValue);
						...
						dimensionData === heightData
							? function heightGetter(scope, locals) {
									...
									if (
										...
										return Math.floor(
											parseFloat(result) / 100 * scrollView.__clientHeight
										);
									}
							...
							: function widthGetter(scope, locals) {
									...
									if (
										...
										return Math.floor(
											parseFloat(result) / 100 * scrollView.__clientWidth
										);
									}
									return parseInt(result);
								};
	...
	function RepeatManagerFactory($rootScope, $window, $$rAF) {
		...
		return function RepeatController(options) {
			...
			scrollView.options[contentSizeStr] = angular.bind(
				view,
				view.getContentSize
			);

			scrollView.__$callback = scrollView.__callback;
			scrollView.__callback = function(
				transformLeft,
				transformTop,
				zoom,
				wasResize
			...
			this.refreshLayout = function() {
				...
				var firstStyle =
					(afterItemsNode.firstElementChild &&
						getComputedStyle(afterItemsNode.firstElementChild)) ||
					{};
				var lastStyle =
					(afterItemsNode.lastElementChild &&
						getComputedStyle(afterItemsNode.lastElementChild)) ||
					{};
				repeaterAfterSize =
					(parseInt(style[isVertical ? "height" : "width"]) || 0) +
					((firstStyle &&
						parseInt(firstStyle[isVertical ? "marginTop" : "marginLeft"])) ||
						0) +
					((lastStyle &&
						parseInt(lastStyle[isVertical ? "marginBottom" : "marginRight"])) ||
						0);
				...
				do {
					repeaterBeforeSize +=
						current[isVertical ? "offsetTop" : "offsetLeft"];
				} while (
					ionic.DomUtil.contains(
						scrollView.__content,
						(current = current.offsetParent)
					)
				);

				var containerPrevNode = containerNode.previousElementSibling;
				var beforeStyle = containerPrevNode
					? $window.getComputedStyle(containerPrevNode)
					: {};
				var beforeMargin = parseInt(
					beforeStyle[isVertical ? "marginBottom" : "marginRight"] || 0
				);
				...
				containerNode.style[
					ionic.CSS.TRANSFORM
				] = TRANSLATE_TEMPLATE_STR.replace(PRIMARY, -beforeMargin).replace(
					SECONDARY,
					0
				);
			...
			function render(forceRerender) {
				...
				for (i = renderStartIndex; i <= renderEndIndex; i++) {
					...
					item =
						...
						(itemsShownMap[i] = itemsLeaving.length
							? itemsLeaving.pop()
							: itemsPool.length ? itemsPool.shift() : new RepeatItem());
					...
					if (
						item.secondaryPos !== dim.secondaryPos ||
						item.primaryPos !== dim.primaryPos
					) {
						item.node.style[
							ionic.CSS.TRANSFORM
						] = TRANSLATE_TEMPLATE_STR.replace(
							PRIMARY,
							(item.primaryPos = dim.primaryPos)
						).replace(SECONDARY, (item.secondaryPos = dim.secondaryPos));
					}
					if (
						item.secondarySize !== dim.secondarySize ||
						item.primarySize !== dim.primarySize
					) {
				...
				while (itemsLeaving.length) {
					...
					itemsPool.push(item);
					item.node.style[ionic.CSS.TRANSFORM] =
				...
				if (forceRefreshImages) {
					for (
						i = 0, ii = itemsEntering.length;
						i < ii && (item = itemsEntering[i]);
						i++
					) {
						if (!item.images) continue;
						for (
							var j = 0, jj = item.images.length, img;
							j < jj && (img = item.images[j]);
							j++
						) {
							var src = img.src;
			...
			function RepeatItem() {
				...
				transclude(this.scope, function(clone) {
					...
					self.node.style[ionic.CSS.TRANSFORM] =
						"translate3d(-9999px,-9999px,0)";
			...
			function GridViewType() {
				this.getEstimatedSecondaryPos = function(index) {
					return (
						(index % this.estimatedItemsAcross) * this.estimatedSecondarySize
					);
				};
				this.getEstimatedPrimaryPos = function(index) {
					return (
						Math.floor(index / this.estimatedItemsAcross) *
						this.estimatedPrimarySize
					);
				};
				this.getEstimatedIndex = function(scrollValue) {
					return (
						Math.floor(scrollValue / this.estimatedPrimarySize) *
						this.estimatedItemsAcross
					);
				};
			}
			...
			function StaticViewType() {
				...
				this.updateRenderRange = function(scrollValue, scrollValueEnd) {
					...
					renderEndIndex = Math.min(
						data.length - 1,
						this.getEstimatedIndex(scrollValueEnd) +
							this.estimatedItemsAcross -
							1
					);

					renderBeforeBoundary = Math.max(
						0,
						this.getEstimatedPrimaryPos(renderStartIndex)
					);
					renderAfterBoundary =
						this.getEstimatedPrimaryPos(renderEndIndex) +
						this.estimatedPrimarySize;
				};
			}
			...
			function DynamicViewType() {
				var self = this;
				var debouncedScrollViewSetDimensions = ionic.debounce(
					scrollViewSetDimensions,
					25,
					true
				);
				var calculateDimensions = isGridView
					? calculateDimensionsGrid
				...
				function calculateDimensionsList(toIndex) {
					...
					for (
						i = Math.max(0, dimensionsIndex);
						i <= toIndex && (dim = dimensions[i]);
						i++
					) {
						...
						dim.primaryPos =
							prevDimension.primaryPos + prevDimension.primarySize;
						dim.secondaryPos = 0;
					}
				}
				function calculateDimensionsGrid(toIndex) {
					...
					for (
						...
						dim.secondaryPos =
							prevDimension.secondaryPos + prevDimension.secondarySize;

						if (
							i === 0 ||
							dim.secondaryPos + dim.secondarySize > self.scrollSecondarySize
						) {
							dim.secondaryPos = 0;
							dim.primarySize = self.getItemPrimarySize(i, data[i]);
							dim.primaryPos =
								prevDimension.primaryPos + prevDimension.rowPrimarySize;
							...
							dimensions[
								dim.rowStartIndex
							].rowPrimarySize = dim.rowPrimarySize = Math.max(
								dimensions[dim.rowStartIndex].rowPrimarySize,
								dim.primarySize
							);
							dim.rowPrimarySize = Math.max(
								dim.primarySize,
								dim.rowPrimarySize
							);
						}
				...
				this.updateRenderRange = function(scrollValue, scrollValueEnd) {
					...
					if (oldRenderStartIndex === -1 || scrollValue === 0) {
						...
						for (i = oldRenderStartIndex, len = data.length; i < len; i++) {
							if (
								(dim = this.getDimensions(i)) &&
								dim.primaryPos + dim.rowPrimarySize >= scrollValue
							) {
								break;
							}
						}
					...
					renderBeforeBoundary =
						renderStartIndex !== -1
							? this.getDimensions(renderStartIndex).primaryPos
							: -1;
					...
					for (i = renderStartIndex + 1, len = data.length; i < len; i++) {
						if (
							(dim = this.getDimensions(i)) &&
							dim.primaryPos + dim.rowPrimarySize > scrollValueEnd
						) {
							...
							if (isGridView) {
								...
								while (
									i < len - 1 &&
									(dim = this.getDimensions(i + 1)).primaryPos ===
										lastRowDim.primaryPos
								) {
									i++;
								}
							}
				...
				compile: function(element, attr) {
					...
					var nativeScrolling =
						...
						(attr.overflowScroll === "true" ||
							!$ionicConfig.scrolling.jsScrolling());
						...
						if ($attr.scroll === "false") {
							...
							if (nativeScrolling) {
								...
								scrollViewOptions = {
									...
									scrollEventInterval:
										parseInt($scope.scrollEventInterval, 10) || 10,
			...
			return {
				...
				link: function($scope, $element, $attr, sideMenuCtrl) {
					...
					function checkAsideExpose() {
						var mq =
							$attr.exposeAsideWhen == "large"
								? "(min-width:768px)"
								: $attr.exposeAsideWhen;
				...
				return {
					...
					link: function($scope, $element, $attr) {
						...
						function onTap(e) {
							...
							var touch =
								(e.gesture && e.gesture.touches[0]) || e.detail.touches[0];
				...
				return {
					...
					compile: function(tElement) {
						...
						$timeout(function() {
							if (
								isHeader &&
								$document[0].getElementsByClassName("tabs-top").length
							)
			...
			return {
				...
				link: function($scope, $element, $attrs, ctrls) {
					...
					if (jsScrolling) {
						...
						var scrollEl = ionic.DomUtil.getParentOrSelfWithClass(
						...
						infiniteScrollCtrl.scrollEl.addEventListener(
							"scroll",
					...
					var doImmediateCheck = isDefined($attrs.immediateCheck)
						? $scope.$eval($attrs.immediateCheck)
						: true;
					if (doImmediateCheck) {
						$timeout(function() {
							infiniteScrollCtrl.checkBounds();
	...
	IonicModule.directive("ionDeleteButton", function() {
		...
		return {
			...
			compile: function($element, $attr) {
				...
				$attr.$set(
					"class",
					($attr["class"] || "") + " button icon button-icon",
					true
				);
						...
						function init() {
							...
							$scope.$watch(
								...
								function(isShown, wasShown) {
									...
									var deleteButton = jqLite(
										$element[0].getElementsByClassName("item-delete")
									);
							...
							$scope.$watch(
								...
								function(isShown, wasShown) {
									...
									$element
										.children()
										.toggleClass("list-right-editing", isShown);
									$element.toggleClass("disable-pointer-events", isShown);

									var reorderButton = jqLite(
										$element[0].getElementsByClassName("item-reorder")
									);
			...
			return {
				...
				link: function($scope, $element) {
					$element.bind("click", function() {
						var sideMenuCtrl = $element.inheritedData(
							"$ionSideMenusController"
						);
						if (sideMenuCtrl) {
							$ionicHistory.nextViewOptions({
								historyRoot: true,
		...
		function($ionicConfig, $document) {
			return {
				...
				compile: function(tElement, tAttrs) {
					...
					buttonEle.className =
						"button back-button hide buttons " + (tElement.attr("class") || "");
					...
					if (!hasIcon && defaultIcon && defaultIcon !== "none") {
						buttonEle.innerHTML =
							'<i class="icon ' + defaultIcon + '"></i> ' + buttonEle.innerHTML;
				...
				compile: function(tElement, tAttrs, transclude) {
					...
					return function($scope, $element, $attr, navViewCtrl) {
						...
						function updateView(firstTime) {
							...
							var viewLocals =
								$state.$current && $state.$current.locals[viewData.name];
					...
					function prelink($scope, $element, $attr, sideMenuCtrl) {
						...
						function onContentTap(gestureEvt) {
							if (sideMenuCtrl.getOpenAmount() !== 0) {
								...
								startCoord = ionic.tap.pointerCoord(
									gestureEvt.gesture.srcEvent
								);
							}
						}
						...
						function getPrimaryScrollAxis(gestureEvt) {
							...
							if (gestureEvt && gestureEvt.gesture) {
								if (!startCoord) {
									...
									var endCoord = ionic.tap.pointerCoord(
										gestureEvt.gesture.srcEvent
									);
						...
						var content = {
							...
							setMarginLeftAndRight: ionic.animationFrameThrottle(function(
								amountLeft,
								amountRight
							) {
						...
						var contentTapGesture = $ionicGesture.on(
							"tap",
							onContentTap,
							$element,
							gestureOpts
						);
						var dragRightGesture = $ionicGesture.on(
							"dragright",
							onDragX,
							$element,
							gestureOpts
						);
						var dragLeftGesture = $ionicGesture.on(
							"dragleft",
							onDragX,
							$element,
							gestureOpts
						);
						var dragUpGesture = $ionicGesture.on(
							"dragup",
							onDragY,
							$element,
							gestureOpts
						);
						var dragDownGesture = $ionicGesture.on(
							"dragdown",
							onDragY,
							$element,
							gestureOpts
						);
						var releaseGesture = $ionicGesture.on(
							"release",
							onDragRelease,
							$element,
							gestureOpts
				...
				return {
					...
					compile: function(element, attr) {
						...
						function prelink($scope, $element, $attrs, ctrl) {
							ctrl.enableMenuWithBackViews(
								$scope.$eval($attrs.enableMenuWithBackViews)
							);
	...
	IonicModule.directive("ionSlideBox", [
			...
			return {
				...
				controller: [
					...
					function($scope, $element, $attrs) {
						...
						var shouldAutoPlay = isDefined($attrs.autoPlay)
							? !!$scope.autoPlay
							: false;
						var slideInterval = shouldAutoPlay
							? $scope.$eval($scope.slideInterval) || 4000
							: 0;
		...
		.directive("ionPager", function() {
			return {
				...
				require: "^ionSlideBox",
				template:
					'<div class="slider-pager"><span class="slider-pager-page" ng-repeat="slide in numSlides() track by $index" ng-class="{active: $index == currentSlide}" ng-click="pagerClick($index)"><i class="icon ion-record"></i></span></div>',
 ...
 *
 */
	IonicModule.directive("ionSlides", [
		"$animate",
		"$timeout",
		"$compile",
			...
			return {
				...
				controller: [
					...
					function($scope, $element) {
						...
						$timeout(function() {
							var slider = new ionic.views.Swiper(
								$element.children()[0],
								newOptions,
								$scope,
								$compile
				...
				compile: function(element, attr) {
					...
					return function link($scope, $element, $attr, ctrls) {
						...
						$scope.$on("$ionicView.afterEnter", function() {
							$ionicViewSwitcher.viewEleIsActive(
								childElement,
								$scope.$tabSelected
							);
						});
			...
			return {
				...
				compile: function(tElement) {
					...
					tElement
						...
						.addClass(
							"tabs-" +
								$ionicConfig.tabs.position() +
								" tabs-" +
								$ionicConfig.tabs.style()
						);
					...
					function prelink($scope, $element, $attr, tabsCtrl) {
						...
						function emitLifecycleEvent(ev, data) {
							...
							if (previousSelectedTab) {
								previousSelectedTab.$broadcast(
									ev.name.replace("NavView", "Tabs"),
									data
								);
							}
						}
				...
				compile: function(element, attr) {
					...
					if (attr.toggleClass) {
						element[0]
							...
							.classList.add(attr.toggleClass);
					}
