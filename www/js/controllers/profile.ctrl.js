angular.module('brewskey.controllers').controller('ProfileCtrl', [
  '$scope',
  'Restangular',
  '$stateParams',
  '$localStorage',
  ...
  function($scope, rest, $stateParams, $storage, converter, achievements) {
    $scope.userName =
      ...
      ($storage.authDetails && $storage.authDetails.userName);
    $scope.canEdit =
      !$stateParams.userName ||
      $stateParams.userName ===
        ($storage.authDetails && $storage.authDetails.userName);
    ...
    rest
      .one('api/profile', escape($scope.userName))
      .get()
      .then(function(response) {
        response.totalPints = Math.round(
          converter.translateToPints(response.lifetimeTotal)
        );
