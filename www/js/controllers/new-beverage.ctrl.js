...
.controller('NewBeverageCtrl', ['$scope', 'Restangular', '$stateParams', '$state', 'utils',
function ($scope, rest, $stateParams, $state, utils) {
    ...
    rest.one('api/v2/beverage-styles').get({ '$skip': 100 }).then(function (response) {
        $scope.styles = $scope.styles.concat(response.value);
    });
    ...
    $scope.getBeerColor = function() {
        ...
        var srms = $scope.srms || [];
        var srm = srms.find(function(s) { return s.id === srmId; }) || { hex: 'FFE699' };
        return {
            'color': srmId > 9 ? '#fff' : '',
